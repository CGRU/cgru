...
ar_verbosity_list = [ 'none', 'fatal', 'error', 'warning', 'info', 'progress', 'debug', 'details' ]
#
# meArnoldRender
#
class meArnoldRender ( object ) :
  ...
  def __init__ ( self, selection = '' ) :
    ...
    self.def_scene_name   = '' # maya scene name used for deferred .ass generation
  ...
  #
  #
  #def __del__( self ): print( ">> meArnoldRender: Class deleted" )
  #
  #
  def initParameters ( self ) : 
    ...
    self.job_param [ 'job_cleanup_ass' ]  = getDefaultIntValue ( self_prefix, 'job_cleanup_ass', 0 ) is 1
    self.job_param [ 'job_cleanup_script' ] = getDefaultIntValue ( self_prefix, 'job_cleanup_script', 0 ) is 1
    #
    # .ass generation parameters
    #
    self.ass_param [ 'ass_reuse' ]         = getDefaultIntValue ( self_prefix, 'ass_reuse', 0 ) is 1
    self.ass_param [ 'ass_filename' ]      = 'data'
    self.ass_param [ 'ass_padding' ]       = getDefaultIntValue ( self_prefix, 'ass_padding', 4 )
    self.ass_param [ 'ass_perframe' ]      = getDefaultIntValue ( self_prefix, 'ass_perframe', 1 ) is 1
    self.ass_param [ 'ass_selection' ]     = getDefaultIntValue ( self_prefix, 'ass_selection', 0 ) is 1
    self.ass_param [ 'ass_binary' ]        = getDefaultIntValue ( self_prefix, 'ass_binary', 0 ) is 1
    self.ass_param [ 'ass_verbosity' ]     = getDefaultStrValue ( self_prefix, 'ass_verbosity', 'none' )
    
    self.ass_param [ 'ass_deferred' ]      = getDefaultIntValue ( self_prefix, 'ass_deferred', 0 ) is 1
    self.ass_param [ 'ass_local_assgen' ]   = getDefaultIntValue ( self_prefix, 'ass_local_assgen', 1 ) is 1
    self.ass_param [ 'ass_def_task_size' ] = getDefaultIntValue ( self_prefix, 'ass_def_task_size', 4 )
    self.ass_param [ 'ass_export_all_layers' ] = getDefaultIntValue ( self_prefix, 'ass_export_all_layers', 0 ) is 1
    self.ass_param [ 'ass_compression' ]   = getDefaultStrValue( self_prefix, 'ass_compression', 'Off' )
    #
    # Arnold parameters
    #
    self.ar_param [ 'ar_options' ]            = getDefaultStrValue ( self_prefix, 'ar_options', '' )
    self.ar_param [ 'ar_verbosity' ]          = getDefaultStrValue ( self_prefix, 'ar_verbosity', 'none' )
    self.ar_param [ 'ar_threads' ]            = getDefaultIntValue ( self_prefix, 'ar_threads', 0 )
    
    self.ar_param [ 'ar_distributed' ]   = getDefaultIntValue ( self_prefix, 'ar_distributed', 0 ) is 1
    self.ar_param [ 'ar_nomaster' ]      = getDefaultIntValue ( self_prefix, 'ar_nomaster', 0 ) is 1
    self.ar_param [ 'ar_hosts' ]         = getDefaultStrValue ( self_prefix, 'ar_hosts', '' ) # @AF_HOSTS@
    self.ar_param [ 'ar_port' ]          = getDefaultIntValue ( self_prefix, 'ar_port', 39010 )
    self.ar_param [ 'ar_hosts_min' ]     = getDefaultIntValue ( self_prefix, 'ar_hosts_min', 1 )
    self.ar_param [ 'ar_hosts_max' ]     = getDefaultIntValue ( self_prefix, 'ar_hosts_max', 4 )
    self.ar_param [ 'ar_threads_limit' ] = getDefaultIntValue ( self_prefix, 'ar_threads_limit', 4 )
    #
    # image parameters
    #
    ...
    self.afanasy_param [ 'af_service' ]            = getDefaultStrValue ( self_prefix, 'af_service', 'arnold' )
    self.afanasy_param [ 'af_deferred_service' ]   = getDefaultStrValue ( self_prefix, 'af_deferred_service', 'mayatoarnold' )
  ...
  def getDeferredCmd ( self, layer = None  ) :
    ...
    gen_cmd += ' -rt 1' # Render type (0 = render, 1 = export ass, 2 = export and kick)
  ...
  def getRenderCmd ( self, layer = None  ) :
    ...
    if ar_verbosity_level < 2     : ar_verbosity_level = 2
  ...
  def get_ass_file_names ( self, suffix = True, layer = None ) :
    ...
    if ass_deferred:
      ( scenename , ext ) = os.path.splitext( os.path.basename( self.def_scene_name))
    filename += '/' + scenename
    filename = cmds.workspace ( expandName = filename )

    if suffix:
      pad_str = getPadStr ( self.ass_param [ 'ass_padding' ], self.ass_param [ 'ass_perframe' ] )
      filename += '.@' + pad_str + '@.ass'
      if self.ass_param [ 'ass_compression' ] != 'Off' : filename += '.gz'
  ...
  def get_assgen_options ( self, layer = None ) :
    ...
    if ass_deferred :
      ...
      ass_compression_level = ass_compression_list.index ( self.ass_param [ 'ass_compression' ] )
      ...
      if ass_selection : assgen_cmd += ' -selected'
    ...
    if not skipExport:
      ...
      # with current render and .ass generation settings
        ...
        for layer in renderLayers :   
          ...
          assgen_cmd = self.assgenCommand + self.get_assgen_options ( layer )
  ...
  def submitJob( self, param=None ):
    ...
    if job_dispatcher == 'afanasy' :
      if ass_deferred and not ass_reuse:
        ...
        self.job.gen_block = AfanasyRenderBlock ( 'generate_ass', deferred_service, self.job, ass_local_assgen )
      ...
      for layer in renderLayers :
        ...
        frame_block.input_files = '"%s"' % self.get_ass_file_names ( True, layer_name )
  ...
  def setResolvedPath ( self ) :
    ...
    if self.ass_param[ 'ass_padding' ] > 0 and self.ass_param [ 'ass_perframe' ] == True :
      filename += '.'
      pad_str = getPadStr ( self.ass_param [ 'ass_padding' ], self.ass_param [ 'ass_perframe' ] )
      filename += pad_str
    
    ext = '.ass'
    if self.ass_param [ 'ass_compression' ] != 'Off' : ext += '.gz'
    filename += ext
    cmds.textFieldGrp ( self.winMain + '|f0|t0|tc1|fr2|fc2|' + 'ass_resolved_path', edit = True, text = filename )
  #
  # enable_range
  #
  ...
  def enable_deferred ( self, arg ) :
    ...
    setDefaultIntValue ( self_prefix, 'ass_deferred', self.ass_param, arg )
    cmds.checkBoxGrp ( ass_def + 'ass_local_assgen', edit = True, enable = arg )
    cmds.intFieldGrp ( ass_def + 'ass_def_task_size', edit = True, enable = arg )
    cmds.optionMenuGrp ( ass_compression, e = True, enable = (arg == False))
    
    bg_color = self.save_frame_bgc 
    if arg : bg_color = self.def_frame_bgc
    cmds.frameLayout ( ass_def_frame, edit = True, bgc = bg_color ) # , enableBackground=False
  ...
  def enable_distributed ( self, arg ) :
    ...
    setDefaultIntValue ( self_prefix, 'ar_distributed', self.ar_param, arg )
    cmds.checkBoxGrp ( ar_distr + 'ar_nomaster', edit = True, enable = arg )
    #cmds.intFieldGrp ( ar_distr + 'ar_port', edit = True, enable = arg )
    cmds.intFieldGrp ( ar_distr + 'ar_hosts_min', edit = True, enable = arg )
    cmds.intFieldGrp ( ar_distr + 'ar_hosts_max', edit = True, enable = arg )
    #cmds.intFieldGrp ( ar_distr + 'ar_threads_limit', edit = True, enable = arg )
    cmds.textFieldGrp ( ar_distr + 'ar_hosts', edit = True, enable = arg )
    bg_color = self.save_frame_bgc 
    if arg : bg_color = self.def_frame_bgc
    cmds.frameLayout ( ar_distr_frame, edit = True, bgc = bg_color ) # , enableBackground=False
  #
  ...
  def renderLayersSetup ( self, layers ) :
    ...
    for layer in layers :
      if layer != 'defaultRenderLayer' : 
        ...
        cmds.scriptJob ( nodeNameChanged = [ layer, 'import maya.OpenMaya; maya.cmds.evalDeferred ( "meArnoldRender.renderLayerRenamed()" , lowestPriority=True )' ], 
                         parent = selector, 
  ...
  def setupUI ( self ):
    ...
    #
    self.deleteUI ( True )
    self.winMain = cmds.window ( meArnoldRenderMainWnd,
                                title = 'meArnoldRender ver.' + meArnoldRenderVer + ' (' + self.os + ')' ,
                                menuBar = True,
                                retain = False,
                                widthHeight = ( 420, 460 ) )
    ...
    cmds.scriptJob ( attributeChange = [ 'renderLayerManager.currentRenderLayer', 
                                         'import maya.OpenMaya; maya.cmds.evalDeferred( "meArnoldRender.renderLayerSelected()" , lowestPriority = True )' ], 
                                          parent = self.winMain  )
    cmds.scriptJob (           event = [ 'renderLayerChange', 
                                         'import maya.OpenMaya; maya.cmds.evalDeferred( "meArnoldRender.renderLayerChanged()" , lowestPriority = True )' ], 
                                          parent = self.winMain ); 
    cw1 = 120
    cw2 = 60
    cw3 = 20
    ...
    cmds.checkBoxGrp ( 'ass_export_all_layers', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = 'Export All Renderable ',
                       value1 = self.ass_param [ 'ass_export_all_layers' ],
                       cc = partial( setDefaultIntValue, self_prefix, 'ass_export_all_layers', self.ass_param ) )
    cmds.setParent( '..' )
    cmds.setParent( '..' )
    #
    # setup tabs
    ...
    cmds.frameLayout ( 'fr1', label = ' Parameters ', borderVisible = True, borderStyle = 'etchedIn', marginHeight = ar_hi  )
    ...
    cmds.checkBoxGrp ( 'job_cleanup_ass', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = '', 
                       label1 = ' .ass files',
                       value1 = self.job_param [ 'job_cleanup_ass' ],
                       enable = False,
                       cc = partial ( setDefaultIntValue, self_prefix, 'job_cleanup_ass', self.job_param ) )
    ...
    #
    tab_assparam = cmds.columnLayout ( 'tc1', columnAttach = ( 'left', 0 ), rowSpacing = 0, adjustableColumn = True )
    cmds.frameLayout ( 'fr3', label = ' Deferred .ass generation ', 
                       borderVisible = True, 
                       borderStyle = 'etchedIn', 
                       marginHeight = ar_hi, 
                       cll = True, cl = True )
    cmds.columnLayout ( 'fc3', columnAttach = ( 'left', 0 ), rowSpacing = 0, adjustableColumn = True )
    cmds.checkBoxGrp ( 'ass_deferred', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = 'Use deferred ',
                       ann = 'Generate .ass files in background process',
                       value1 = self.ass_param [ 'ass_deferred' ],
                       cc = partial ( self.enable_deferred ) ) 
    cmds.checkBoxGrp ( 'ass_local_assgen', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = '', 
                       ...
                       cc = partial ( setDefaultIntValue, self_prefix, 'ass_local_assgen', self.ass_param ) )
    cmds.intFieldGrp ( 'ass_def_task_size', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
                       label = 'Task Size ',
                       value1 = self.ass_param [ 'ass_def_task_size' ],
                       enable = self.ass_param [ 'ass_deferred' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ass_def_task_size', self.ass_param ) )
    #self.save_frame_bgc = cmds.frameLayout ( 'fr3', query = True, bgc = True )
    #self.def_frame_bgc = [ 0.75, 0.5, 0 ]
    bg_color = self.save_frame_bgc 
    if self.ass_param [ 'ass_deferred' ] : bg_color = self.def_frame_bgc
    ...
    cmds.frameLayout ( 'fr1', label = ' Export Settings ', 
                       ...
                       marginHeight = ar_hi  )
    cmds.columnLayout ( 'fc1', columnAttach = ( 'left', 0 ), rowSpacing = 0, adjustableColumn = True )
    cmds.checkBoxGrp ( 'ass_reuse', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = 'Use existing .ass files ',
                       ann = 'Do not generate .ass files if they are exist',
                       value1 = self.ass_param [ 'ass_reuse' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ass_reuse', self.ass_param ) )
    cmds.text ( label = '' )
    #ass_dir = cmds.textFieldButtonGrp( cw=( 1, cw1 ), adj=2, label="Directory ", buttonLabel="...", text=self.ass_param['ass_dir'] )
    #cmds.textFieldButtonGrp( ass_dir, edit=True, bc=partial( browseDirectory, self.rootDir, ass_dir ), cc=partial( self.setDefaultStrValue, 'ass_dir' ) )
    ass_filename = cmds.textFieldButtonGrp ( 'ass_filename', cw = ( 1, cw1 ),
                       enable = False,
                       adj = 2,
                       label = 'File Name ', 
                       buttonLabel = '...',
    ...
    cmds.textFieldButtonGrp ( ass_filename, edit = True, bc = partial ( browseFile, self.rootDir, ass_filename, 'Arnold files (*.ass)' ) )
    cmds.intFieldGrp ( 'ass_padding', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
                       label = 'Frame Padding ',
                       value1 = self.ass_param [ 'ass_padding' ],
                       cc = partial ( self.assFileNameChanged, 'ass_padding' ) )
    cmds.checkBoxGrp ( 'ass_perframe', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = '', 
                       label1 = ' File Per Frame ',
                       value1 = self.ass_param [ 'ass_perframe' ],
                       cc = partial( self.assFileNameChanged, 'ass_perframe' ) )
    cmds.checkBoxGrp ( 'ass_selection', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = '', 
                       label1 = ' Export Only Selected Objects',
                       value1 = self.ass_param [ 'ass_selection' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ass_selection', self.ass_param ) )
    cmds.checkBoxGrp ( 'ass_binary', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = '', 
                       label1 = ' Binary',
                       value1 = self.ass_param [ 'ass_binary' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ass_binary', self.ass_param ) )
    ass_compression = cmds.optionMenuGrp ( 'ass_compression', cw=( (1, cw1), ),
                       cal=(1, 'right'),  
                       label = 'Compression ', 
                       cc = partial ( self.assFileNameChanged, 'ass_compression' ) )
    for name in ass_compression_list : cmds.menuItem ( label = name )
    cmds.optionMenuGrp ( ass_compression, e = True, value = self.ass_param [ 'ass_compression' ] )
    # ass_verbosity = none, fatal, error, warning, info, progress, and details
    #ass_verbosity = cmds.optionMenuGrp( 'ass_verbosity', cw=( (1, cw1), ), cal=(1, 'right'),
    #                                   label="Verbosity ",
    #                                   cc=partial( self.setDefaultStrValue, 'ass_verbosity' ) )
    #for name in ('none', 'fatal', 'error', 'warning', 'info', 'progress', 'details'):
    #  cmds.menuItem( label=name )
    #cmds.optionMenuGrp( ass_verbosity, e=True, value=self.ass_param['ass_verbosity'] )
    #
    cmds.setParent ( '..' )
    cmds.setParent ( '..' )
    cmds.frameLayout ( 'fr2', label = ' Resolved Path ', 
                       ...
                       marginHeight = ar_hi  )
    cmds.columnLayout ( 'fc2', columnAttach = ( 'left', 0 ), rowSpacing = 0, adjustableColumn = True )
    cmds.textFieldGrp ( 'ass_resolved_path', cw = ( 1, 0 ), adj = 2, 
                        label = '', 
                        text = '', 
                        editable = False )
    self.setResolvedPath ()
    ...
    fr2 = cmds.frameLayout ( 'fr2', label = ' Distributed render ', 
                       ...
                       cll = True, cl = True )
    cmds.columnLayout ( 'fc2', columnAttach = ( 'left', 0 ), rowSpacing = 0, adjustableColumn = True )
    cmds.checkBoxGrp ( 'ar_distributed', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
    ...
    cmds.checkBoxGrp ( 'ar_nomaster', cw = ( ( 1, cw1 ), ( 2, cw1 * 2 ) ),
                       label = '', 
                       label1 = " No Master ",
                       ...
                       cc = partial ( setDefaultIntValue, self_prefix, 'ar_nomaster', self.ar_param ) )
    #cmds.intFieldGrp ( 'ar_port', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
    #                   label = 'Port ',
    #                   value1 = self.ar_param [ 'ar_port' ],
    #                   enable = self.ar_param [ 'ar_distributed' ],
    #                   cc = partial ( setDefaultIntValue, self_prefix, 'ar_port', self.ar_param ) )
    cmds.intFieldGrp ( 'ar_hosts_min', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
                       label = 'Min Hosts ',
                       value1 = self.ar_param [ 'ar_hosts_min' ],
                       enable = self.ar_param [ 'ar_distributed' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ar_hosts_min', self.ar_param ) )
    cmds.intFieldGrp ( 'ar_hosts_max', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
                       label = 'Max Hosts ',
                       value1 = self.ar_param [ 'ar_hosts_max' ],
                       enable = self.ar_param [ 'ar_distributed' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ar_hosts_max', self.ar_param ) )
    #cmds.intFieldGrp ( 'ar_threads_limit', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
    #                   label = 'Threads Limit ',
    #                   ann = 'Max threads per host. All available threads will be used if 0',
    #                   value1 = self.ar_param [ 'ar_threads_limit' ],
    #                   enable = self.ar_param [ 'ar_distributed' ],
    #                   cc = partial ( setDefaultIntValue, self_prefix, 'ar_threads_limit', self.ar_param ) )
    cmds.textFieldGrp ( 'ar_hosts', cw = ( 1, cw1 ), adj = 2,
                       label = 'Remote Hosts ',
                       ann = 'Remote hosts names (if empty, will be filled by Render Manager)',
                       text = self.ar_param [ 'ar_hosts' ],
                       enable = self.ar_param [ 'ar_distributed' ],
                       cc = partial ( setDefaultStrValue, self_prefix, 'ar_hosts', self.ar_param ) )                       
    bg_color = self.save_frame_bgc 
    if self.ar_param [ 'ar_distributed' ] : bg_color = self.def_frame_bgc
    cmds.frameLayout ( fr2, edit = True, bgc = bg_color ) # , enableBackground=False
    cmds.setParent ( '..' )
    cmds.setParent ( '..' )
    cmds.frameLayout ( 'fr1', label = ' Arnold options ', 
                       borderVisible = True, 
                       borderStyle = 'etchedIn', 
                       marginHeight = ar_hi  )
    cmds.columnLayout ( 'fc1', columnAttach = ( 'left', 0 ), 
                        rowSpacing = 0, 
                        adjustableColumn = True )
    cmds.textFieldGrp ( 'ar_options', cw = ( 1, cw1 ), adj = 2,
                       label = 'Additional Options ',
                       text = self.ar_param [ 'ar_options' ],
                       cc = partial ( setDefaultStrValue, self_prefix, 'ar_options', self.ar_param ) )
    cmds.text ( label = '' )
    ar_verbosity = cmds.optionMenuGrp ( 'ar_verbosity', cw = ( ( 1, cw1 ), ), cal = ( 1, 'right' ),
                       label = 'Verbosity ',
                       cc = partial ( setDefaultStrValue, self_prefix, 'ar_verbosity', self.ar_param ) )
    for name in ar_verbosity_list : cmds.menuItem ( label = name )
    cmds.optionMenuGrp ( ar_verbosity, e = True, value = self.ar_param [ 'ar_verbosity' ] )
    cmds.intFieldGrp ( 'ar_threads', cw = ( ( 1, cw1 ), ( 2, cw2 ) ),
                       label = 'Threads ',
                       ann = 'The number of threads',
                       value1 = self.ar_param [ 'ar_threads' ],
                       cc = partial ( setDefaultIntValue, self_prefix, 'ar_threads', self.ar_param ) )
    cmds.setParent ( '..' )
    cmds.setParent ( '..' )
    cmds.setParent ( '..' )
    #
    ...
    cmds.setParent ( form )
    btn_sbm = cmds.button ( label = 'Submit', command = self.submitJob, ann = 'Generate .ass files and submit to dispatcher' )
    btn_gen = cmds.button ( label = 'Generate .ass', command = self.generate_ass, ann = 'Force .ass files generation' )

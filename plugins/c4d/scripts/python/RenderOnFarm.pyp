...
class RenderSceneWindow(c4d.gui.GeDialog):
    ...
    def CreateLayout(self):
        ...
        self.TabGroupBegin(GROUP_ID_TABS, c4d.BFH_SCALEFIT,
                           tabtype=c4d.TAB_TABS)

        # ------------------------------
        # START TAB SUBMISSION
        # ------------------------------
        self.GroupBegin(GROUP_ID_SUBMISSION, c4d.BFH_SCALEFIT,
                        title="Submission", cols=1)

        self.AddStaticText(STATIC_TEXT_JOB_NAME, c4d.BFH_LEFT,
                           name="Job Name:")
        self.AddEditText(TEXTBOX_JOBNAME, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_OUTPUT, c4d.BFH_LEFT,
                           name="Overwrite Output:")
        self.AddEditText(TEXTBOX_OUTPUT, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_PRIORITY, c4d.BFH_LEFT,
                           name="Priority:")
        ...
        self.AddStaticText(STATIC_TEXT_BY_FRAME, c4d.BFH_LEFT,
                           name="By Frame:")
        self.AddEditSlider(EDITSLIDER_BY_FRAME, c4d.BFH_SCALEFIT)

        self.AddCheckbox(CHECKBOX_BAKE_CONSTRAINTS, c4d.BFH_FIT,
                         name="Bake Constraints", initw=WINDOW_WIDTH, inith=15)
        self.AddCheckbox(CHECKBOX_DO_NOT_COPY_LOCALLY, c4d.BFH_FIT,
                         name="Do not copy Scene to Hosts", initw=WINDOW_WIDTH,
                         inith=15)
        self.AddCheckbox(CHECKBOX_CREATE_JOB_PAUSED, c4d.BFH_FIT,
                         name="Create Job Paused", initw=WINDOW_WIDTH,
                         inith=15)
        ...
        # START TAB SHEDULING
        #------------------------------
        self.GroupBegin(GROUP_ID_SHEDULING, c4d.BFH_SCALEFIT,
                        title="Sheduling", cols=1)

        self.AddStaticText(STATIC_TEXT_FRAMES_PER_TASK, c4d.BFH_LEFT,
                           name="Frames per Task:")
        self.AddEditSlider(EDITSLIDER_FRAMES_PER_TASK, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_MAX_HOSTS, c4d.BFH_LEFT,
                           name="Max Hosts (Max Running Tasks):")
        self.AddEditSlider(EDITSLIDER_MAX_HOSTS, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_MAX_RUNTIME, c4d.BFH_LEFT,
                           name="Max Runtime (min):")
        self.AddEditSlider(EDITSLIDER_MAX_RUNTIME, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_CAPACITY, c4d.BFH_LEFT,
                           name="Capacity:")
        ...
        # START TAB MASKS
        #------------------------------
        self.GroupBegin(GROUP_ID_MASKS, c4d.BFH_SCALEFIT, title="Masks",
                        cols=1)

        self.AddStaticText(STATIC_TEXT_HOSTS_MASK, c4d.BFH_LEFT,
                           name="Hosts Mask:")
        self.AddEditText(TEXTBOX_HOSTS_MASK, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_EXCLUDE_HOSTS_MASK, c4d.BFH_LEFT,
                           name="Exclude Hosts Mask:")
        self.AddEditText(TEXTBOX_EXCLUDE_HOSTS_MASK, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_DEPEND_MASK, c4d.BFH_LEFT,
                           name="Depend Mask:")
        self.AddEditText(TEXTBOX_DEPEND_MASK, c4d.BFH_SCALEFIT)

        self.AddStaticText(STATIC_TEXT_DEPEND_MASK_GLOBAL, c4d.BFH_LEFT,
                           name="Global Depend Mask:")
    ...
    def InitValues(self):
        ...
        self.SetReal(EDITSLIDER_STARTFRAME, self.parentClass.startFrame,
                     min=self.parentClass.startFrame,
                     max=self.parentClass.endFrame)
        self.SetReal(EDITSLIDER_ENDFRAME, self.parentClass.endFrame,
                     min=self.parentClass.startFrame,
                     max=self.parentClass.endFrame)
        self.SetReal(EDITSLIDER_PRIORITY, self.parentClass.priority, min=-1,
                     max=100)
        self.SetReal(EDITSLIDER_BY_FRAME, self.parentClass.byFrame, min=1,
                     max=100)
        self.SetReal(EDITSLIDER_FRAMES_PER_TASK,
                     self.parentClass.framesPerTask, min=1, max=100)
        self.SetReal(EDITSLIDER_MAX_HOSTS, self.parentClass.maxHosts, min=-1,
                     max=100)
        self.SetReal(EDITSLIDER_MAX_RUNTIME, self.parentClass.maxRuntime,
                     min=-1, max=1000)
        self.SetReal(EDITSLIDER_CAPACITY, self.parentClass.capacity, min=-1,
                     max=10000)
        ...
        self.SetString(TEXTBOX_EXCLUDE_HOSTS_MASK,
                       self.parentClass.excludeHostsMask)
        self.SetString(TEXTBOX_DEPEND_MASK, self.parentClass.dependMask)
        self.SetString(TEXTBOX_DEPEND_MASK_GLOBAL,
                       self.parentClass.dependMaskGlobal)

        self.SetBool(CHECKBOX_BAKE_CONSTRAINTS,
                     self.parentClass.bakeConstraints)
        self.SetBool(CHECKBOX_CREATE_JOB_PAUSED,
                     self.parentClass.createJobPaused)
        self.SetBool(CHECKBOX_DO_NOT_COPY_LOCALLY,
                     self.parentClass.doNotCopyLocally)
    ...
    def Command(self, id, msg):
        ...
        if id == BUTTON_CLOSE:
            ...
            self.parentClass.framesPerTask = self.GetReal(
                EDITSLIDER_FRAMES_PER_TASK)
...
class RenderOnFarm(plugins.CommandData):
    ...
    def Execute(self, thisDoc):
        ...
        if self.filePath == "":
            c4d.gui.MessageDialog(
                'Before you can render a Scene on the Farm it has to get '
                'saved!'
            )
            return True
        ...
        if self.startExport:
            ...
            if self.bakeConstraints:
                ...
                constrainedObjects = \
                    self.SelectConstraintTagObjects(
                        tempDoc,
                        tempDoc.GetFirstObject()
                    )

                # Key everything
                self.RecordSelectedObjects(
                    tempDoc,
                    self.startFrame,
                    self.endFrame
                )
            ...
            if self.bakeConstraints:
                ...
                c4d.documents.SaveDocument(
                    tempDoc, os.path.join(self.filePath, self.tempSceneName),
    ...
    def SelectConstraintTagObjects(self, thisDoc, obj, constrainedObjects=None):
        """Missing DocString

        :param thisDoc:
        :param obj:
        :param constrainedObjects:
        :return:
        """

        if constrainedObjects is None:
            constrainedObjects = []
        ...
        while obj:
            ...
            self.SelectConstraintTagObjects(thisDoc, obj.GetDown(),
                                            constrainedObjects)
            obj = obj.GetNext()
    ...
    def RecordSelectedObjects(self, thisDoc, startFrame, endFrame):
        ...
        for frame in range(startFrame, (endFrame + 1)):
            c4d.StatusSetBar(
                100 * (frame - startFrame) / (endFrame - startFrame))
    ...
    def DeactivateConstraintTags(self, constrainedObjects):
        ...
        for thisObject in constrainedObjects:
            ...
            print(
                "Constraint-Tag got deactivated on %s" % thisObject.GetName()
            )
    ...
    def startFarmRender(self):
        ...
        block.setNumeric(int(self.startFrame), int(self.endFrame),
                         int(self.framesPerTask), int(self.byFrame))
        ...
        if self.doNotCopyLocally:
            cmd = 'c4d'
        cmd += ' -nogui -render "' + scenePath + '" -frame @#@ @#@ ' + str(
            self.byFrame)
        if self.output != '':
        ...
        if not job.send()[0]:
            ...
            c4d.gui.MessageDialog(
                'There was a problem with submitting the job.'
            )

...
def main():
    ...
    if scene_file_path == "":
        error_exit("Scene to execute is not specified.")

    # Get Afanasy root directory:
    afroot = os.getenv("AF_ROOT")
    if afroot is None:
        error_exit("AF_ROOT is not defined.")

    # Get the scene-path and the scene-file-name
    (scene_folder, scene_file) = os.path.split(scene_file_path)

    # Create and check the c4d-temp directory:
    my_temp_directory = cgruTempFolder.cgruTempFolder(
        scene_file, service="c4d", type_="render", debug=debug
    )
    ...
    # Copy the scene-file to the temp-directory (copies only when newer then a
    # maybe already existing one)
    ...
    if not os.path.isdir(temp_output_folder):
        ...
        if not creation_status:
            error_exit(
                "The Temp-Output-Folder could not get created. So images can not get "
                "rendered and script stopped"
            )
    ...
    try:
        ...
        omultipass_overwrite_folder, omultipass_overwrite_filename = os.path.split(
            output_overwrite
        )

        parameters[parameters.index("-omultipass") + 1] = os.path.join(
            temp_output_folder, omultipass_overwrite_filename
        )
    except ValueError:
    ...
    print("Running Command: %s" % (" ".join(parameters)))
    sys.stdout.flush()

    # Now execute the command
    process = subprocess.Popen(parameters, shell=True, stdout=subprocess.PIPE)
    ...
    while True:
        ...
        # Now update some information like current frame & percent
    ...
    if output_overwrite:
        ...
        # When the output was not overwritten copy it in the Output-Folder of the
        # original scene-file
        output_folder = os.path.join(scene_folder, "Output")

    # To find the correct files to copy
    reg_match = ".*([a-zA-Z]|.)+(0?|0+)(%s).[a-zA-Z0-9]{1,5}$" % (
        "|".join(rendered_frames)
    )

    # Now actually copy and then delete the renderings
    copied_files = cgruutils.copy_directory(
        temp_output_folder,
        output_folder,
        reg_match=reg_match,
        delete_copied_files=True,
        debug=debug,
    )
    ...
    # Now lets tell the temp-folder that we do not need it anymore

...
def afRender(scene, start, end, step, simulate, renderPass='', attr='',
			 value=''):
	log('  ScenePath : %s' % scene)
	log('  StartFrame: %s' % start)
	log('  EndFrame  : %s' % end)
	log('  Step      : %s' % step)
	log('  Simulate  : %s' % simulate)
	...
	if renderPass != '':
		log(
			'Trying to render current pass "%s"...' % renderPass)
	if attr != '':
			...
			for ps in Application.ActiveProject.ActiveScene.Passes:
				...
				for fb in ps.Framebuffers:
					...
					Application.SetValue(
						"Passes.%s.%s.Filename" % (renderPass, fb.Name),
						outfile
					)

		if changeRenderOptionsOutputDir:
			outdir = \
				Application.GetValue('Passes.RenderOptions.ResolvedOutputDir')

...
def dailiesEvaluate(node):
	...
	if codec is None or codec == '' or not os.path.isfile(codec):
	...
	if lgspath is not None and lgspath != '' and not os.path.isfile(lgspath):
	...
	if lgfpath is not None and lgfpath != '' and not os.path.isfile(lgfpath):
	...
	if newNode:
		...
		if template is None or template == '' or not os.path.isfile(template):
		...
		if slate is None or slate == '' or not os.path.isfile(slate):
	...
	# Movie Name:
	if node.knob('movauto').value() \
			or node.knob('movname').value is None \
			or node.knob('movname').value == '':
		movname = movrule
...
def dailies(node):
	...
	if sys.platform.find('win') == 0:
		...
		cmd = 'python "%s" ' % (
		os.environ['CGRU_LOCATION'] + '/utilities/launchcmd.py') + cmd + ' &'

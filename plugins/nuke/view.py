...
if len(Args) < 1:
    Parser.error('Input is not provided.')
...
if os.path.isdir(InDir):
    ...
    for item in allitems:
        ...
        if len(Prefixes) == 0 \
           or Prefixes[-1] != prefix \
           or Suffixes[-1] != suffix \
           or Paddings[-1] != padding:
...
for i in range(len(Prefixes)):
    print('%s[%d-%d/%d]%s' % (
    Prefixes[i], Starts[i], Ends[i], Paddings[i], Suffixes[i]))

#!/bin/bash
#
# $ cd cgru
# $ afanasy/daemon/afdaemon.sh start afrender
#

action=$1

# Set Afanasy application:
afapp=$2
if [ -z "${afapp}" ]; then
	echo "ERROR: Afanasy application is not specified."
	exit 1
fi

# Set Afanasy location:
pushd `dirname $0`
cd ..
afroot=`pwd`
popd

# Afanasy "nonroot" user:
nonrootuser=render
if [ -f "${afroot}/nonrootuser" ]; then
	nonrootuser=`cat "${afroot}/nonrootuser"`
else
	echo "${nonrootuser}" > "${afroot}/nonrootuser"
fi
id "${nonrootuser}" || nonrootuser=""

# Temp directory:
tmpdir=/var/tmp/afanasy
if [ ! -d "$tmpdir" ]; then
	mkdir $tmpdir
	if [ ! -d "$tmpdir" ]; then
		echo "temp directory was not created"
		exit 1
	else
		chmod a+rwx $tmpdir
	fi
fi

#
# PID file:
#
export pidfile="${tmpdir}/${afapp}.pid"

#
# LOG file:
#
export logfile="${tmpdir}/${afapp}.log"
logrotate() {
	log=$1
	if [ -f $log ]; then
		[ -f $log.8 ] && /bin/mv -f $log.8 $log.9
		[ -f $log.7 ] && /bin/mv -f $log.7 $log.8
		[ -f $log.6 ] && /bin/mv -f $log.6 $log.7
		[ -f $log.5 ] && /bin/mv -f $log.5 $log.6
		[ -f $log.4 ] && /bin/mv -f $log.4 $log.5
		[ -f $log.3 ] && /bin/mv -f $log.3 $log.4
		[ -f $log.2 ] && /bin/mv -f $log.2 $log.3
		[ -f $log.1 ] && /bin/mv -f $log.1 $log.2
		[ -f $log.0 ] && /bin/mv -f $log.0 $log.1
		/bin/mv -f $log $log.0
	fi
	return 0
}

function checkNOFILE() {
	 nofile=10240
	 if (( `ulimit -n` < $nofile )); then
		  echo "Process file descriptors limit is `ulimit -n` too low."
		  echo "For high server load trying to increase it to $nofile."
		  ulimit -n $nofile
		  if [ $? != 0 ]; then
				echo "Can't raise this limit."
		  else
				echo "Process file descriptors limit changed to `ulimit -n`."
		  fi
	 else
		  echo "Process file descriptors limit is `ulimit -n`."
	 fi
}

execfile="$afroot/bin/$afapp"
startcmd="$afroot/daemon/afstart.sh $execfile $logfile"

function start(){
	echo "Starting $afapp..."
	if [ -f $pidfile ]; then
		kill `cat $pidfile`
		rm -fv $pidfile
	fi

	logrotate $logfile

	 if [ $afapp == "afserver" ]; then
		  checkNOFILE
	 fi

	startcmd="$startcmd $pidfile"
	if [ "$UID" == "0" ] && [ ! -z "${nonrootuser}" ]; then
		su - $nonrootuser -c "$startcmd"
	else
		$startcmd
	fi

	if [ $? != 0 ]; then
		echo "Can't execute process."
		exit 1
	fi

	if [ ! -f $pidfile ]; then
		echo "Pid file was not created."
		exit 1
	else
		chown root:root $pidfile
	fi
}

function stop(){
	echo "Stopping $afapp"
	if [ ! -f $pidfile ]; then
		echo "Application '$afapp' is not running (or pid file does not exist)."
		exit 1
	fi
	kill `cat $pidfile`
	if [ $? != 0 ]; then
		echo "Can't kill process."
		exit 1
	fi
	rm -f $pidfile
	if [ -f $pidfile ]; then
		echo "Can't delete pid file."
	fi
	echo $?
}

function _status(){
	if [ ! -f $pidfile ]; then
		echo "Application '$afapp' is not running (or pid file does not exist)."
		exit 1
	fi
	PID=`cat $pidfile`
	if [ -d "/proc/${PID}" ]; then
		echo "Application '$afapp' is running."
		exit 0
	else
		echo "Application '$afapp' is not running."
		exit 1
	fi
}

case $action in
	start)
		start
		;;

	stop)
		stop
		;;

	status)
		_status
		;;

	restart)
		stop
		sleep 10
		start
		;;

	launch)
		echo "Starting $afapp"
		if [ "$UID" == "0" ]; then
			echo "Error: Try not to launch under ROOT!"
			exit 1
		fi
		logrotate $logfile
		$startcmd
		;;

	*)
		echo "Usage: $afapp {start|stop|restart|launch|status}"
		exit 1
		;;
esac

exit 0

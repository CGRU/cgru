...
g_actions = {};
g_actions.logs_table = {
	'label': 'Logs Table',
	'func': 'g_Action_LogsTable'
};
g_actions.jobs_table = {
	'label': 'Jobs Tables',
	'func': 'g_Action_JobsTable'
};
g_actions.tasks_table = {
	'label': 'Tasks Tables',
	'func': 'g_Action_TasksTable'
};
g_actions.tasks_graph = {
	'label': 'Tasks Graphs',
	'func': 'g_Action_TasksGraph'
};
...
g_graph_intervals.minute = {
	'seconds': 60,
	'intervals': [1, 5, 10, 15, 20, 30, 60, 60 * 3, 60 * 6, 60 * 12, 60 * 24],
	'offsets': [[0]]
};
g_graph_intervals.hour = {
	'seconds': 60 * 60,
	'intervals': [1, 3, 6, 12, 24],
	'offsets': [[0]]
};
g_graph_intervals.day = {
	'seconds': 60 * 60 * 24,
	'intervals': [1, 7, 7, 7],
	'offsets': [[0], [1, 3, 5], [1, 5], [1]]
};
g_graph_intervals.week = {
	'seconds': 60 * 60 * 24 * 7,
	'intervals': [1],
	'offsets': [[0]]
};
...
function g_FoldersProcess(i_folder)
{
	...
	for (let f = 0; f < g_folders.length; f++)
	{
		...
		if (folder.indexOf(g_args.folder) != 0)
			continue;
...
function g_Action_JobsTable(i_args)
{
	...
	g_Request({
		'send': {'get_jobs_folders': i_args},
		'func': g_ShowTable,
		'args': i_args,
		'service': 'get_jobs_table'
	});
}
...
function g_Action_TasksTable(i_args)
{
	...
	i_args.select = 'folder';
	g_Request({
		'send': {'get_tasks_folders': i_args},
		'func': g_ShowTable,
		'args': i_args,
		'service': 'get_tasks_table'
	});
}
...
function g_ShowTable(i_data, i_args)
{
	...
	for (let r = 0; r < i_data.table.length; r++)
	{
		...
		for (let col in i_data.table[r])
		{
			...
			if (g_parm[col])
			{
				if (g_parm[col].percent)
					value = Math.round(100 * value) + '%';
				else if (g_parm[col].round)
					value = Math.round(value);
				else if (g_parm[col].time)
					value = g_SecToHMS(value);
				else if (g_parm[col].datetime)
					value = g_SecToStr(value);
			}

			if (col == 'folder')
				value = g_FolderStrip(value);
...
function g_ShowGraph(i_data, i_args)
{
	...
	// console.log("Count: " + count + ", interval: " + interval + ', type: "' + int_name + '", text interval:
	// ' + text_interval + ', offset: ' + text_interval_offset + ', offsets: ' +
	// text_interval_offsets.join(','));
	...
	let quantity = 0;
	let quantity_pow = 1;
	while (quantity_pow * 10 < quantity_max)
		quantity_pow *= 10;
	if (quantity_max / quantity_pow < 4)
		quantity_pow /= 2;
	...
	for (let s = 0; s < select_num; s++)
	{
		...
		if (select == 'folder')
			select_name = g_FolderStrip(select_name);
...
function g_Log(i_msg)
{
	...
	let date = new Date();
	let time = g_PadZero(date.getHours()) + ':' + g_PadZero(date.getMinutes()) + ':' +
		g_PadZero(date.getSeconds()) + '.' + g_PadZero(date.getMilliseconds(), 3);
...
g_parm.tasks_done_percent = {
	'label': 'Done',
	'percent': true
};
g_parm.tasks_quantity = {
	'label': 'Tasks Quantity'
};
g_parm.tasks_quantity_avg = {

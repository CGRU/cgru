#ifndef AF_BLOCKDATA_H
#define AF_BLOCKDATA_H
...
/// Called when some task started, to change state and to increment runnung tasks counter
   ...
   inline const int       getProgressPercentage()        const { return p_percentage;        }
   inline const int       getProgressErrorHostsNum()     const { return p_errorhostsnum;     }
   inline const int       getProgressAvoidHostsNum()     const { return p_avoidhostsnum;     }
   inline const int       getProgressTasksReady()        const { return p_tasksready;        }
   inline const int       getProgressTasksRunning()      const { return p_tasksrunning;      }
   inline const int       getProgressTasksDone()         const { return p_tasksdone;         }
   inline const int       getProgressTasksError()        const { return p_taskserror;        }
   inline const uint32_t  getProgressTasksSumRunTime()   const { return p_taskssumruntime;   }

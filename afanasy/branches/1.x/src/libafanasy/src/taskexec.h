#ifndef AF_TASKEXEC_H
#define AF_TASKEXEC_H
...
public:
   ...
   inline bool hasCmd()     const { return !cmd.isEmpty();     } ///< Whether command exists.
   ...
   inline bool hasCmdView() const { return !cmdview.isEmpty(); } ///< Whether preview command exists.
   ...
   inline const QString& getCmdView()  const { return cmdview; } ///< Get preview command.
   ...
   inline const uint32_t getTimeStart()  const { return time_start;   } ///< Get task started time.

   inline const bool equals( const MCTaskUp & up) const
      {return ((jobid == up.getNumJob()) && (blocknum == up.getNumBlock()) && (tasknum == up.getNumTask()) && (number == up.getNumber()));}

   inline const bool equals( const TaskExec & other) const
   ...
   inline void setCmd(       const QString & str) { cmd       = str;}   ///< Set task command.
...
private:
   ...
   QString cmd;            ///< Command.
   QString cmdview;        ///< Preview command.

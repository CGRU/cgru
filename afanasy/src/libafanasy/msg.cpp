...
Msg::Msg(const struct sockaddr_storage *ss) : m_address(ss)
{
	construct();
	set(TNULL);
}
	  ...
	  m_writing(false)
{
	...
	if (false == allocateBuffer(rawDataLen))
...
bool Msg::allocateBuffer(int i_size, int i_copy_len, int i_copy_offset)
{
	...
	AFINFA("Msg::allocateBuffer(%s): trying %d bytes ( %d written at %p)", TNAMES[m_type], i_size,
		   m_writtensize, old_buffer)
...
{
	if (checkZero(true) == false)
		return false;

	m_type = i_type;

	if ((i_size <= 0) || (i_size > Msg::SizeDataMax) || (i_msgData == NULL))
	{
...
void Msg::rw_header(bool write)
{
	...
	int offset = 0;
	rw_int32(m_version, m_buffer + offset, write);
	offset += int32_size;
	rw_int32(m_type, m_buffer + offset, write);
	offset += int32_size;
	rw_int32(m_int32, m_buffer + offset, write);
	offset += int32_size;
...
bool Msg::checkValidness()
{
	...
	if (m_type >= Msg::TDATA)
	{
		...
		{
			AFERRAR("Msg::checkValidness: message with data length > Msg::SizeDataMax ( %d > %d )", m_int32,
					Msg::SizeDataMax)
			setInvalid();
			return false;
		}
	...
	if (m_version != Msg::Version)
	{
		if (af::Environment::isClient())
			AFERRAR("Msg::checkValidness: Version mismatch: Received(%d) != Library(%d)", m_version,
					Msg::Version)
...
const char *Msg::TNAMES[] = {
	...
	"TRESERVED00", "TRESERVED01", "TRESERVED02", "TRESERVED03", "TRESERVED04", "TRESERVED05", "TRESERVED06",
	"TRESERVED07", "TRESERVED08", "TRESERVED09",
	...
	**/
	...
	"TRESERVED10", "TRESERVED11", "TRESERVED12", "TRESERVED13", "TRESERVED14", "TRESERVED15", "TRESERVED16",
	"TRESERVED17", "TRESERVED18", "TRESERVED19",

...
class Msg : public Af, public AfQueueItem
{
  public:
	...
	//	inline bool setJSON_headerBin( const std::ostringstream & i_str) { return setJSON_headerBin(
	// i_str.str());}
	...
	enum Type
	{
		...
		/**/ TMonitorUpdateId /**/,	  ///< Update Monitor with given id (No information for updating Monitor
									  ///< needed).
		...
		And when Render can't connect to Afanasy. Afanasy register new Render and send back it's id \c
		TRenderId. **/
		/**/ TRenderRegister /**/,
	...
	void readHeader(int bytes); ///< Read header from message buffer, \c bytes - number of already written
								///< bytes in it's buffer.

...
TaskData::TaskData(Msg *msg) { read(msg); }
...
TaskData::TaskData(const JSON &i_object) { jsonRead(i_object); }
...
void TaskData::v_generateInfoStream(std::ostringstream &o_str, bool full) const
{
	...
	else
	{
		...
		if (m_files.size())
			o_str << " F'" << af::strJoin(m_files, ";") << "'";
		if (m_depend_mask.size())
			o_str << " D'" << m_depend_mask << "'";
		if (m_custom_data.size())
			o_str << " cd'" << m_custom_data << "'";

...
MCJobsWeight::MCJobsWeight(Msg *msg) { read(msg); }

MCJobsWeight::~MCJobsWeight() {}

void MCJobsWeight::add(const std::string &Name, int Id, int WeightLog, int WeightBlack, int WeightProgress,
					   int Weight)
{
...
void MCJobsWeight::v_generateInfoStream(std::ostringstream &stream, bool full) const
{
	...
	for (unsigned j = 0; j < ids.size(); j++)
	{
		...
		if (full)
			stream << "\n"
				   << j + 1 << ids[j] << ": \tlog=" << float(weightLog[j]) / 1024
				   << " \tblack=" << float(weightBlack[j]) / 1024
				   << " \tprogress=" << float(weightProgress[j]) / 1024 << " \t : " << float(weight[j]) / 1024
				   << " kB"
				   << " \t - " << names[j];
	}

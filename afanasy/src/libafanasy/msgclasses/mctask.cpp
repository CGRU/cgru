...
const char *MCTask::TNAMES[] = {"TNULL", "TExec", "TOutput", "TLog", "TErrorHosts", "TListen", "TLAST"};
...
{
	if (m_type != TNULL)
		AF_ERR << "Type is not NULL.";
	if (m_exec != NULL)
		AF_ERR << "m_exec is not NULL.";
	if (i_exec == NULL)
		AF_ERR << "i_exec is NULL.";
...
{
	if (m_type != TExec)
		AF_ERR << "Type is not TExec.";
	if (m_exec == NULL)
		AF_ERR << "m_exec is NULL.";
...
void MCTask::v_generateInfoStream(std::ostringstream &o_str, bool i_full) const
{
	if (i_full)
	{
		...
		o_str << "Pos: " << m_pos;
		o_str << "\nProgress: " << m_progress;
		o_str << "\nJobName = '" << m_job_name << "', << BlockName = '" << m_block_name << "', TaskName = '"
			  << m_task_name << "'";

...
{
  public:
	MCTaskUp(int i_clientid,

			 int i_numjob, int i_numblock, int i_numtask, int i_number = -1,

			 int i_status = -1, int i_percent = -1, int i_frame = -1, int i_percent_frame = -1,

			 bool i_progress_changed = false,

			 const std::string &i_log = std::string(), const std::string &i_activity = std::string(),
			 const std::string &i_resources = std::string(), const std::string &i_report = std::string(),
			 const std::string &i_listened = std::string(), int i_datalen = 0, char *i_data = NULL);
	MCTaskUp(Msg *msg);
	~MCTaskUp();

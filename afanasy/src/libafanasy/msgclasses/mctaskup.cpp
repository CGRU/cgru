...
MCTaskUp::MCTaskUp(int i_clientid,

				   int i_numjob, int i_numblock, int i_numtask, int i_number,

				   int i_status, int i_percent, int i_frame, int i_percent_frame,

				   bool i_progress_changed,

				   const std::string &i_log, const std::string &i_activity, const std::string &i_resources,
				   const std::string &i_report, const std::string &i_listened,

				   int i_datalen, char *i_data)
	: m_clientid(i_clientid),

	  m_numjob(i_numjob), m_numblock(i_numblock), m_numtask(i_numtask), m_number(i_number),

	  m_status(i_status), m_percent(i_percent), m_frame(i_frame), m_percent_frame(i_percent_frame),

	  m_progress_changed(i_progress_changed),

	  m_log(i_log), m_activity(i_activity), m_resources(i_resources), m_report(i_report),

	  m_listened(i_listened),

	  m_datalen(i_datalen), m_data(i_data),
	  m_deleteData(false), // Don not delete data on client side, as it is not copied

	  m_files_num(0), m_files_data(NULL)
{
}

MCTaskUp::MCTaskUp(Msg *msg)
	: m_data(NULL), m_files_data(NULL),
...
{
	if ((m_numjob == i_other.m_numjob) && (m_numblock == i_other.m_numblock) &&
		(m_numtask == i_other.m_numtask) && (m_number == i_other.m_number))
		return true;
...
void MCTaskUp::v_generateInfoStream(std::ostringstream &stream, bool full) const
{
	if (full)
	{
		stream << "TaskUp: client=" << m_clientid << ", job=" << m_numjob << ", block=" << m_numblock
			   << ", task=" << m_numtask << ", number=" << m_number << ", activity=" << m_activity
			   << ", resources=" << m_resources << ", report=" << m_report << ", log=" << m_log
			   << ", datalen=" << m_datalen << ", files=" << m_files_num << ", status=" << int(m_status)
	...
	{
		stream << "TaskUp:"
			   << " C" << m_clientid << " J" << m_numjob << " B" << m_numblock << " T" << m_numtask << " #"
			   << m_number << " A" << m_activity << " R" << m_resources << " R" << m_report << " L" << m_log
			   << " D" << m_datalen << " F" << m_files_num << " S" << int(m_status) << " " << int(m_percent)
			   << "%";
	}
}

...
MCTaskPos::MCTaskPos(int job_id, int block_num, int task_num, int Number)
	: jobid(job_id), blocknum(block_num), tasknum(task_num), number(Number)
{
}

MCTaskPos::MCTaskPos(const MCTaskUp &i_tup)
	: jobid(i_tup.getNumJob()), blocknum(i_tup.getNumBlock()), tasknum(i_tup.getNumTask()),
	  number(i_tup.getNumber())
{
}

MCTaskPos::MCTaskPos(Msg *msg) { read(msg); }
...
{
	if ((jobid == i_other.jobid) && (blocknum == i_other.blocknum) && (tasknum == i_other.tasknum) &&

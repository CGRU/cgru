...
void Job::v_jsonWrite(std::ostringstream &o_str, int i_type) const
{
	...
	if (m_try_this_tasks_num.size())
		af::jw_int32vec("try_this_tasks_num", m_try_this_tasks_num, o_str);
	if (m_try_this_blocks_num.size())
		af::jw_int32vec("try_this_blocks_num", m_try_this_blocks_num, o_str);
...
BlockData *Job::newBlockData(Msg *msg) { return new BlockData(msg); }

BlockData *Job::newBlockData(const JSON &i_object, int i_num) { return new BlockData(i_object, i_num); }
...
int Job::v_calcWeight() const
{
	...
	for (int b = 0; b < m_blocks_num; b++)
		weight += m_blocks_data[b]->calcWeight();
...
const std::string Job::tryNextTasksToStr(int i_limit) const
{
	...
	{
		AF_ERR << "Try tasks and blocks size mismatch: " << m_try_this_tasks_num.size()
...
void Job::generateInfoStreamJob(std::ostringstream &o_str, bool full) const
{
	...
	if (m_depend_mask_global.notEmpty())
	...
	{
		o_str << "\nFolders:";
		for (std::map<std::string, std::string>::const_iterator it = m_folders.begin(); it != m_folders.end();
			 it++)
		{

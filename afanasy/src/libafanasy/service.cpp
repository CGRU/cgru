...
Service::Service(const TaskExec *i_task_exec, const std::string &i_store_dir)
	: m_name(i_task_exec->getServiceType()), m_parser_type(i_task_exec->getParserType()),
	  m_wdir(i_task_exec->getWDir())
{
	initialize(i_task_exec, i_store_dir);
}

Service::Service(const std::string &i_type, const std::string &i_wdir, const std::string &i_command_task)
	: m_name(i_type), m_parser_type("generic"), m_wdir(i_wdir)
{
	TaskExec *i_task_exec =
		new TaskExec("name", m_name, m_parser_type, 1, i_command_task, std::vector<std::string>(), 1, 1, 1, 1,
					 m_wdir, std::map<std::string, std::string>(), 1, 0, 0, 1);
	initialize(i_task_exec, "");
	delete i_task_exec;
}

Service::Service(const std::string &i_type, const std::string &i_parser_type)
	: m_name(i_type), m_parser_type(i_parser_type)
{
	TaskExec *i_task_exec = new TaskExec("name", m_name, m_parser_type, 1, "", std::vector<std::string>(), 1,
										 1, 1, 1, m_wdir, std::map<std::string, std::string>(), 1, 0, 0, 1);
	initialize(i_task_exec, "");
	delete i_task_exec;
}

Service::Service(const std::string &i_wdir) : m_name("generic"), m_parser_type("generic"), m_wdir(i_wdir)
{
	TaskExec *i_task_exec = new TaskExec("name", m_name, m_parser_type, 1, "", std::vector<std::string>(), 1,
										 1, 1, 1, m_wdir, std::map<std::string, std::string>(), 1, 0, 0, 1);
	initialize(i_task_exec, "");
...
Service::Service(const std::vector<std::string> &i_files_block, long long i_frame_start,
				 long long i_frame_end, long long i_frame_inc, const std::vector<std::string> &i_files_task)
	: m_name("generic"), m_parser_type("generic")
{
	...
	TaskExec *task_exec =
		new TaskExec("name", m_name, m_parser_type, 1, "", i_files_block, i_frame_start, i_frame_end,
					 i_frame_inc, 1, m_wdir, std::map<std::string, std::string>(), 1, 0, block_flags, 1);
...
Service::Service(const std::string &i_type, const std::string &i_command_block, long long i_frame_start,
				 long long i_frame_end)
	: m_name(i_type), m_parser_type("generic")
{
	TaskExec *i_task_exec = new TaskExec("name", m_name, m_parser_type, 1, i_command_block,
										 std::vector<std::string>(), i_frame_start, i_frame_end, 1, 1, m_wdir,
										 std::map<std::string, std::string>(), 1, 0, BlockData::FNumeric, 1);
	initialize(i_task_exec, "");
	delete i_task_exec;
}
...
void Service::initialize(const TaskExec *i_task_exec, const std::string &i_store_dir)
{
	...
	PyDict_SetItemString(task_info, "command_block",
						 PyBytes_FromString(i_task_exec->getCommandBlock().c_str()));
	PyDict_SetItemString(task_info, "command_task",
						 PyBytes_FromString(i_task_exec->getCommandTask().c_str()));
	...
	if (m_PyObj_FuncCheckRenderedFiles == NULL)
		return;

	m_PyObj_FuncDoPost = getFunction(AFPYNAMES::SERVICE_FUNC_DOPOST);
	if (m_PyObj_FuncDoPost == NULL)
		return;
...
void Service::parse(const std::string &i_mode, int i_pid, std::string &io_data, std::string &io_resources,
					int &o_percent, int &o_frame, int &o_percentframe, std::string &o_activity,
					std::string &o_report, bool &o_progress_changed, bool &o_warning, bool &o_error,
					bool &o_fatalerror, bool &o_badresult, bool &o_finishedsuccess) const
{
	...
	PyDict_SetItemString(dict, "resources",
						 PyBytes_FromStringAndSize(io_resources.data(), io_resources.size()));
	...
	if (pClass != NULL)
	{
		if (pClass != Py_None)
		{
			...
			af::PyGetAttrBool(pClass, "fatalerror", o_fatalerror, err);

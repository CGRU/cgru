...
/*!
	...
	\tparam Encoding	Encoding of the value. (Even non-string values need to have the same encoding in a
   document)
...
template <typename Encoding, typename Allocator = MemoryPoolAllocator<>> class GenericValue
{
  ...
  public:
	...
	*/
	...
	{
		static const unsigned defaultFlags[7] = {kNullFlag,
												 kFalseFlag,
												 kTrueFlag,
												 kObjectFlag,
												 kArrayFlag,
	...
	~GenericValue()
	{
		...
		{ // Shortcut by Allocator's trait
			switch (flags_)
			{
				...
				case kCopyStringFlag: Allocator::Free((void *)data_.s.str); break;
			}
		}
	}
	...
	*/
	template <typename T> GenericValue &operator=(T value)
	{
		this->~GenericValue();
	...
	*/
	GenericValue &AddMember(GenericValue &name, GenericValue &value, Allocator &allocator)
	{
		...
		if (o.size >= o.capacity)
		{
			...
			else
			{
				...
				o.capacity *= 2;
				o.members = (Member *)allocator.Realloc(o.members, oldCapacity * sizeof(Member),
	...
	template <typename T> GenericValue &AddMember(const Ch *name, T value, Allocator &allocator)
	{
		GenericValue n(name, internal::StrLen(name));
...
Value a(kArrayType);
a.PushBack(123);
int x = a[0].GetInt();				// Error: operator[ is ambiguous, as 0 also mean a null pointer of const
char* type. int y = a[SizeType(0)].GetInt();	// Cast to SizeType will work. int z = a[0u].GetInt();
// This works too.
\endcode
	*/
	...
	*/
	GenericValue &Reserve(SizeType newCapacity, Allocator &allocator)
	{
		...
		{
			data_.a.elements =
				(GenericValue *)allocator.Realloc(data_.a.elements, data_.a.capacity * sizeof(GenericValue),
	...
	/*! \param value		The value to be appended.
		...
		\note If the number of elements to be appended is known, calls Reserve() once first may be more
	   efficient.
	*/
	...
	template <typename T> GenericValue &PushBack(T value, Allocator &allocator)
	{
		GenericValue v(value);
	...
	/*! This version has better performance with supplied length, and also support string containing null
	   character.
	...
	*/
	template <typename Handler> GenericValue &Accept(Handler &handler)
	{
		switch (GetType())
		{
			...
			case kStringType: handler.String(data_.s.str, data_.s.length, false); break;
  ...
  private:
	template <typename, typename> friend class GenericDocument;
	...
	Member *FindMember(const Ch *name)
	{
		...
		for (Member *member = o.members; member != data_.o.members + data_.o.size; ++member)
			if (name[member->name.data_.s.length] == '\0' &&
				memcmp(member->name.data_.s.str, name, member->name.data_.s.length * sizeof(Ch)) == 0)
				return member;
...
class GenericDocument : public GenericValue<Encoding, Allocator>
{
  public:
	...
	*/
	...
	/*! \tparam parseFlags Combination of ParseFlag.
		...
		\return The document itself for fluent API.
	*/
	template <unsigned parseFlags, typename SourceEncoding> GenericDocument &ParseInsitu(Ch *str)
	{
	...
	*/
	template <unsigned parseFlags, typename SourceEncoding> GenericDocument &Parse(const Ch *str)
	{
  ...
  private:
	...
	{
		if (Allocator::kNeedFree)
			while (stack_.GetSize() > 0) // Here assumes all elements in stack array are GenericValue (Member
										 // is actually 2 GenericValue objects)

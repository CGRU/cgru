...
{
	if( false == i_object.IsObject()) return;
...
{
	if( i_str.empty()) return;

	unsigned int ip6[8];
	int fields6 = sscanf( i_str.c_str(), "%x:%x:%x:%x:%x:%x:%x:%x", &(ip6[0]),&(ip6[1]),&(ip6[2]),&(ip6[3]),&(ip6[4]),&(ip6[5]),&(ip6[6]),&(ip6[7]));
	if( fields6 == 8 )
	{
		m_family = IPv6;
		for( int i = 0; i < 8; i++)
	...
	{
		m_family = IPv4;
		for( int i = 0; i < 4; i++) m_addr[i] = ip4[i];
		return;
	}

...
HostRes::HostRes()
	: cpu_num(0), cpu_mhz(0), cpu_temp(0),

	  cpu_user(0), cpu_nice(0), cpu_system(0), cpu_idle(99), cpu_iowait(0), cpu_irq(0), cpu_softirq(0),

	  mem_total_mb(0), mem_free_mb(1), mem_cached_mb(0), mem_buffers_mb(0),

	  swap_total_mb(0), swap_used_mb(0),

	  hdd_total_gb(0), hdd_free_gb(1), hdd_rd_kbsec(0), hdd_wr_kbsec(0), hdd_busy(0),
...
void HostRes::jsonWrite(std::ostringstream &o_str) const
{
	...
	o_str << ",\n\"cpu_mhz\":" << cpu_mhz;
	o_str << ",\n\"cpu_temp\":" << cpu_temp;
	o_str << ",\n\"cpu_loadavg\":[" << int(cpu_loadavg[0]) << ',' << int(cpu_loadavg[1]) << ','
		  << int(cpu_loadavg[2]) << ']';
...
void HostRes::v_readwrite(Msg *msg)
{
	...
	for (int i = 0; i < custom_count; i++)
		if (msg->isWriting())
			...
			custom.push_back(new HostResCustom(msg));
}
...
void HostRes::v_generateInfoStream(std::ostringstream &stream, bool full) const
{
	...
	if (full)
	{
		...
		stream << "\n      " << int(cpu_user) << "% usr, " << int(cpu_nice) << "% nice, " << int(cpu_system)
			   << "% sys, " << int(cpu_idle) << "% idle, " << int(cpu_iowait) << "% iow, " << int(cpu_irq)
			   << "% irq, " << int(cpu_softirq) << "% sirq";
		stream << "\n      load average:   " << cpu_loadavg[0] / 10.0 << "   " << cpu_loadavg[1] / 10.0
			   << "   " << cpu_loadavg[2] / 10.0;
		...
		stream << "\n   Network: Received " << net_recv_kbsec << " Kb/sec, Send " << net_send_kbsec
			   << " Kb/sec",
			stream << "\n   IO: Read " << hdd_rd_kbsec << " Kb/sec, Write " << hdd_wr_kbsec
				   << " Kb/sec, Busy = " << int(hdd_busy) << "%";
		stream << "\n   HDD: " << hdd_total_gb << " GB / " << hdd_free_gb << " GB free";

		if (gpu_string.size())
		{
			stream << "\n   GPU = \"" << gpu_string << "\"";
			stream << "\n       Utilization: " << int(gpu_gpu_util) << "%, Temperature: " << int(gpu_gpu_temp)
				   << "C";
	...
	{
		stream << "la[" << cpu_loadavg[0] / 10.0 << "," << cpu_loadavg[1] / 10.0 << ","
			   << cpu_loadavg[2] / 10.0 << "]";
		stream << "; C" << cpu_mhz << "x" << cpu_num << " " << int(cpu_temp) << "C"
			   ...
			   << " r" << int(cpu_irq) << "%"
			   << " f" << int(cpu_softirq) << "%";
		stream << "; M" << mem_total_mb << " F" << mem_free_mb << " C" << mem_cached_mb << " B"
			   << mem_buffers_mb << " S" << swap_total_mb << "/" << swap_used_mb << " H" << hdd_total_gb
			   << "/" << hdd_free_gb << " N" << net_recv_kbsec << "/" << net_send_kbsec << " D"
			   << hdd_rd_kbsec << "\"" << hdd_wr_kbsec << ":" << int(hdd_busy) << "%";
	}

...
Render::Render(Client::Flags i_flags) : Client(i_flags, 0)
{
	construct();
...
Render::Render(Msg *msg) : Client(Client::DoNotGetAnyValues, 0)
{
	construct();
	read(msg);
}
...
void Render::v_readwrite(Msg *msg) // Thread-safe
{
	switch (msg->type())
	{
		...
		case Msg::TRendersResources:
			...
			break;
		default: AFERROR("Render::readwrite(): invalid type.\n");
	}
	...
	if (msg->type() == Msg::TRenderRegister)
	{
		...
		for (int i = 0; i < netIfs_size; i++)
			if (msg->isWriting())
				m_netIFs[i]->write(msg);
			else
				m_netIFs.push_back(new NetIF(msg));
	}
}

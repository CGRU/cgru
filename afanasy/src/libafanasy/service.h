...
class Service : public PyClass
{
  public:
	...
	Service(/* For afrender to generate WOL sleep command */
			const std::string &i_type, const std::string &i_wdir, const std::string &i_command_task);

	Service(/* For afwatch to parse task output: */
			const std::string &i_type, const std::string &i_parser_type);

	Service(/* For afwatch to paths map folder: */
			const std::string &i_wdir);

	Service(/* For afwatch to generate and paths map files: */
			const std::vector<std::string> &i_files_block, long long i_frame_start, long long i_frame_end,
			long long i_frame_inc, const std::vector<std::string> &i_files_task = std::vector<std::string>());

	Service(/* For afcmd to test service on a numeric command: */
			const std::string &i_type, const std::string &i_command_block, long long i_frame_start,
	...
	void parse(const std::string &i_mode, int i_pid, std::string &io_data, std::string &io_resources,

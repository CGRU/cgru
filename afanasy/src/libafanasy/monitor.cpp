...
/*
	Afanasy monitor.
	Libafanasy monitor needed for a data store/exchange only.
*/
#include "monitor.h"
...
Monitor::Monitor() : Client(Client::GetEnvironment, 0)
{
	construct();
	m_name = af::Environment::getUserName() + "@" + af::Environment::getHostName() + ":" +
			 m_address.generatePortString().c_str();
}

Monitor::Monitor(Msg *msg) : Client(Client::DoNotGetAnyValues, 0)
{
	if (construct() == false)
		return;
	read(msg);
...
Monitor::Monitor(const JSON &obj) : Client(Client::DoNotGetAnyValues, 0)
{
...
const char *Monitor::EVT_NAMES[] = {"jobs_add",		  "jobs_change",  "jobs_del",		 "JOBS_COUNT",
									"users_add",	  "users_change", "users_del",		 "renders_add",
									"renders_change", "renders_del",  "monitors_add",	 "monitors_change",
									"monitors_del",	  "branches_add", "branches_change", "branches_del",
									"pools_add",	  "pools_change", "pools_del",		 "COUNT"};

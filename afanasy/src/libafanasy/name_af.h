...
/*
	name_af.h - Header for functions that are common for an entire project (afserver, afrender, afwatch,
   afcmd).
*/
#pragma once
...
const std::string toKMG(long long i_number, int i_base = 1000, const std::string &i_separator = "");
...
const std::string strStrip(const std::string &i_str, Direction i_dir,
						   const std::string &i_characters = " \n");
...
bool setRegExp(RegExp &regexp, const std::string &str, const std::string &name,
			   std::string *errOutput = NULL);
...
char *fileRead(const std::string &i_filename, int *o_size = NULL, int i_maxfilesize = -1,
			   std::string *o_err = NULL);
...
bool launchProgram(PROCESS_INFORMATION *o_pinfo, const std::string &i_commandline,
				   const std::string &i_wdir = std::string(), char *i_environ = NULL, HANDLE *o_in = NULL,
				   HANDLE *o_out = NULL, HANDLE *o_err = NULL, DWORD i_flags = 0,
				   bool alwaysCreateWindow = false);
#else
char **processEnviron(const std::map<std::string, std::string> &i_env_map);
int launchProgram(const std::string &i_commandline, const std::string &i_wdir = std::string(),
				  char **i_environ = NULL, FILE **o_in = NULL, FILE **o_out = NULL, FILE **o_err = NULL);
#endif
...
/// Solve host address.
const af::Address solveNetName(const std::string &i_name, int i_port, int i_type = AF_UNSPEC,
							   VerboseMode i_verbose = VerboseOff);
...
// JSON:
char *jsonParseData(rapidjson::Document &o_doc, const char *i_data, int i_data_len,
					std::string *o_err = NULL);
...
bool jr_stringmap(const char *i_name, std::map<std::string, std::string> &o_attr, const JSON &i_object,
				  std::string *o_str = NULL);
bool jr_intmap(const char *i_name, std::map<std::string, int32_t> &o_map, const JSON &i_object,
			   std::string *o_str = NULL);
...
void jw_stringmap(const char *i_name, const std::map<std::string, std::string> &i_map,
...
void jsonActionStart(std::ostringstream &i_str, const std::string &i_type, const std::string &i_mask,
					 const std::vector<int> &i_ids = std::vector<int>());
void jsonActionFinish(std::ostringstream &i_str);

void jsonActionParamsStart(std::ostringstream &i_str, const std::string &i_type, const std::string &i_mask,
						   const std::vector<int> &i_ids = std::vector<int>());
...
void jsonActionOperationStart(std::ostringstream &i_str, const std::string &i_type,

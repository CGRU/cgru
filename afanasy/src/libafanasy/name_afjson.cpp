...
af::Msg *af::jsonMsgError(const std::string &i_str) { return af::jsonMsgInfo("error", i_str); }
...
af::Msg *af::jsonMsg(const std::ostringstream &i_stream) { return af::jsonMsg(i_stream.str()); }
...
af::Msg *af::jsonMsg(const std::string &i_type, const std::string &i_name,
					 const std::list<std::string> &i_list)
{
...
bool af::jr_intmap(const char *i_name, std::map<std::string, int32_t> &o_map, const JSON &i_object,
				   std::string *o_str)
{
	const JSON &map = i_object[i_name];
	if (false == map.IsObject())
...
void af::jw_stringmap(const char *i_name, const std::map<std::string, std::string> &i_map,
...
void af::jw_stateJob(const int64_t &i_state, std::ostringstream &o_str)
{
	...
	if (i_state & AFJOB::STATE_READY_MASK)
		o_str << " RDY";
	if (i_state & AFJOB::STATE_RUNNING_MASK)
		o_str << " RUN";
	if (i_state & AFJOB::STATE_DONE_MASK)
		o_str << " DON";
	if (i_state & AFJOB::STATE_ERROR_MASK)
		o_str << " ERR";
	if (i_state & AFJOB::STATE_SKIPPED_MASK)
		o_str << " SKP";
	if (i_state & AFJOB::STATE_OFFLINE_MASK)
		o_str << " OFF";
	if (i_state & AFJOB::STATE_WARNING_MASK)
		o_str << " WRN";
	if (i_state & AFJOB::STATE_PARSERERROR_MASK)
		o_str << " PER";
	if (i_state & AFJOB::STATE_PARSERBADRESULT_MASK)
		o_str << " PBR";
	if (i_state & AFJOB::STATE_PARSERSUCCESS_MASK)
		o_str << " PSC";
	if (i_state & AFJOB::STATE_WAITDEP_MASK)
		o_str << " WDP";
	if (i_state & AFJOB::STATE_WAITTIME_MASK)
		o_str << " WTM";
	if (i_state & AFJOB::STATE_STDOUT_MASK)
		o_str << " STO";
	if (i_state & AFJOB::STATE_STDERR_MASK)
		o_str << " STE";
	if (i_state & AFJOB::STATE_PPAPPROVAL_MASK)
		o_str << " PPA";
	if (i_state & AFJOB::STATE_ERROR_READY_MASK)
		o_str << " RER";
	if (i_state & AFJOB::STATE_WAITRECONNECT_MASK)
		o_str << " WRC";
	if (i_state & AFJOB::STATE_TRYTHISTASKNEXT_MASK)
		o_str << " TRY";
	if (i_state & AFJOB::STATE_SUSPENDED_MASK)
		o_str << " SUS";
...
void af::jw_statePool(const int64_t &i_state, std::ostringstream &o_str)
{
	...
	if (i_state & Pool::SBusy)
		o_str << " RUN";
	if (i_state & Pool::SPaused)
		o_str << " PAU";
...
void af::jw_stateRender(const int64_t &i_state, std::ostringstream &o_str)
{
	...
	if (i_state & Render::SOnline)
		o_str << " ONL";
	else
		o_str << " OFF";
	if (i_state & Render::Snimby)
		o_str << " NbY";
	if (i_state & Render::SNIMBY)
		o_str << " NBY";
	if (i_state & Render::SBusy)
		o_str << " RUN";
	if (i_state & Render::SDirty)
		o_str << " DRT";
	if (i_state & Render::SWOLFalling)
		o_str << " WFL";
	if (i_state & Render::SWOLSleeping)
		o_str << " WSL";
	if (i_state & Render::SWOLWaking)
		o_str << " WWK";
	if (i_state & Render::SPaused)
		o_str << " PAU";
	if (i_state & Render::SSick)
		o_str << " SIC";

	o_str << "\"";
}

void af::jsonActionStart(std::ostringstream &i_str, const std::string &i_type, const std::string &i_mask,
						 const std::vector<int> &i_ids)
{
	i_str << "{\"action\":{";
...
void af::jsonActionFinish(std::ostringstream &i_str) { i_str << "\n}}"; }
...
void af::jsonActionOperation(std::ostringstream &i_str, const std::string &i_type,
							 const std::string &i_operation, const std::string &i_mask,
							 const std::vector<int> &i_ids)
{
	af::jsonActionOperationStart(i_str, i_type, i_operation, i_mask, i_ids);
	af::jsonActionOperationFinish(i_str);
}
void af::jsonActionOperationStart(std::ostringstream &i_str, const std::string &i_type,
								  const std::string &i_operation, const std::string &i_mask,
								  const std::vector<int> &i_ids)
{
	af::jsonActionStart(i_str, i_type, i_mask, i_ids);

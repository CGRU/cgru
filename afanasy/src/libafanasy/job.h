...
class Job : public Work
{
  public:
	...
	**/
	...
	{
		return m_state & AFJOB::STATE_RUNNING_MASK;
	...
	{
		return m_state & AFJOB::STATE_OFFLINE_MASK;
	} ///< Whether a job is offline (paused).
	...
	inline void setPPAFlag(bool i_ppa = true)
	{
		if (i_ppa)
			m_flags = m_flags | FPPApproval;
		else
			m_flags = m_flags & (~FPPApproval);
	}

	inline bool isMaintenanceFlag() const { return (m_flags & FMaintenance); }
	inline void setMaintenanceFlag(bool i_on = true)
	{
		if (i_on)
			m_flags = m_flags | FMaintenance;
		else
			m_flags = m_flags & (~FMaintenance);
	}

	inline bool isIgnoreNimbyFlag() const { return (m_flags & FIgnoreNimby); }
	inline void setIgnoreNimbyFlag(bool i_on = true)
	{
		if (i_on)
			m_flags = m_flags | FIgnoreNimby;
		else
			m_flags = m_flags & (~FIgnoreNimby);
	}

	inline bool isIgnorePausedFlag() const { return (m_flags & FIgnorePaused); }
	inline void setIgnorePausedFlag(bool i_on = true)
	{
		if (i_on)
			m_flags = m_flags | FIgnorePaused;
		else
			m_flags = m_flags & (~FIgnorePaused);
	}
	...
	{
		return m_depend_mask_global.match(str);
	}

	//	const std::string & getTasksOutputDir() const { return m_tasks_output_dir; }

	/// Get block constant pointer.
	inline BlockData *getBlockData(int i_num) const
	{
		if (i_num < m_blocks_num)
			return m_blocks_data[i_num];
		else
			return NULL;
	}
  ...
  protected:
	...
	/// set in JobAf::refresh(): if job is running or done, but was not started, we set job header
	/// time_started

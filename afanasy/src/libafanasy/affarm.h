...
class Farm
{
  public:
	...
	struct Tiks
	{
		...
		Tiks(const int32_t &i_count, const int32_t &i_usage)
			: count(i_count), usage(i_usage), hosts(0), max_hosts(-1)
		{
		}
		Tiks(const int32_t &i_count, const int32_t &i_usage, const int32_t &i_hosts,
			 const int32_t &i_max_hosts)
  ...
  private:
	...
	static bool jr_Tickets(const char *i_name, std::unordered_map<std::string, Tiks> &o_map,
						   const JSON &i_object);
	static void jw_Tickets(const char *i_name, const std::unordered_map<std::string, Tiks> &i_map,

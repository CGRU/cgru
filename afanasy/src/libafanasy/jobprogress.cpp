...
JobProgress::JobProgress(Job *job, bool doConstruct) : m_job_id(job->getId())
{
	initProperties();
	if (doConstruct)
		construct(job);
...
bool JobProgress::construct(Job *job)
{
	...
	if (m_blocks_num < 1)
	{
		AFERRAR("JobProgress::JobProgress: Invalid number if blocks = %d (job name: '%s')", m_blocks_num,
				job->getName().c_str())
		return false;
	}
	...
	for (int b = 0; b < m_blocks_num; b++)
	{
		...
		if (tasksnum[b] < 1)
		{
			AFERRAR("JobProgress::JobProgress: Invalid number of tasks = %d (m_job_id=%d,block=%d)",
		...
		{
			AFERRAR("JobProgress::JobProgress: Tasks initalization failed ( block=%d, tasks number=%d).", b,
					tasksnum[b])
			return false;
		}
	}
...
TaskProgress *JobProgress::newTaskProgress() const { return new TaskProgress; }
...
void JobProgress::v_readwrite(Msg *msg)
{
	...
	for (int b = 0; b < m_blocks_num; b++)
	{
		...
		if (msg->isReading())
		{
			if (initTasks(b, tasksnum[b]) == false)
			{
				AFERRAR("JobProgress::readwrite: tasks initalization failed ( block=%d, tasks number=%d).", b,
						tasksnum[b])
				return;
			}
		}

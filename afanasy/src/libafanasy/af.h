...
class Af
{
  public:
	...
	friend std::ostream &operator<<(std::ostream &stream, const Af &obj)
	{
		stream << obj.v_generateInfoString();
		return stream;
	}
	friend std::ostream &operator<<(std::ostream &stream, const Af *obj_ptr)
	{
		stream << obj_ptr->v_generateInfoString();
		return stream;
	}

...
SysCmd::SysCmd(const std::string &i_command, const std::string &i_working_directory,
			   const std::string &i_user_name, const std::string &i_job_name, const std::string &i_task_name)
	: command(i_command), working_directory(i_working_directory), user_name(i_user_name),
	  job_name(i_job_name), task_name(i_task_name)
{
	if (task_name.empty())
	{
...
SysTask::SysTask(af::TaskExec *i_taskexec, SysCmd *i_system_command, Block *i_block, int i_task_number)
	: Task(i_block, &m_taskProgress, i_task_number), m_syscmd(i_system_command), m_birthtime(0)
{
...
void SysTask::v_refresh(time_t currentTime, RenderContainer *renders, MonitorContainer *monitoring,
						int &errorHostId)
{
	...
	if ((currentTime - m_birthtime > af::Environment::getSysJobTaskLife()) && (isReady()))
	{
		...
		std::string message = std::string("Error: Task age(") + af::itos(currentTime - m_birthtime) + ") > " +
							  af::itos(af::Environment::getSysJobTaskLife());
...
void SysTask::v_updateState(const af::MCTaskUp &taskup, RenderContainer *renders,
							MonitorContainer *monitoring, bool &errorHost)
{
	...
	if (taskup.getClientId() != 0)
	{
		...
		if (render)
			message += std::string(" on \"") + render->getName() + "\"";
	}
...
SysBlock::~SysBlock()
{
	for (std::list<SysCmd *>::iterator it = m_commands.begin(); it != m_commands.end(); it++)
		delete *it;
...
bool SysBlock::v_startTask(af::TaskExec *taskexec, RenderAf *render, MonitorContainer *monitoring)
{
	...
	if (monitoring)
		m_tasks[0]->v_monitor(monitoring);
...
SysTask *SysBlock::addTask(af::TaskExec *taskexec)
{
	...
	if (false == found)
	{
		std::string message = std::string("Can't find task number (max=") +
							  af::itos(af::Environment::getSysJobTasksMax()) + ")";
...
void SysBlock::v_errorHostsAppend(int task, int hostId, RenderContainer *renders)
{
	...
	if (render == NULL)
		return;
	if (Block::v_errorHostsAppend(render->getName()))
		appendJobLog(render->getName() + " - AVOIDING HOST !");
...
void SysBlock::updateTaskState(const af::MCTaskUp &taskup, RenderContainer *renders,
							   MonitorContainer *monitoring)
{
	...
	if (errorHost)
		v_errorHostsAppend(taskup.getNumTask(), taskup.getClientId(), renders);
}
...
bool SysBlock::v_refresh(time_t currentTime, RenderContainer *renders, MonitorContainer *monitoring)
{
	...
	for (std::list<SysTask *>::iterator it = m_systasks.begin(); it != m_systasks.end(); it++)
	{
		...
		if (errorHostId != -1)
			v_errorHostsAppend(0, errorHostId, renders);
		if ((*it)->isRunning())
			m_taskprogress->state |= AFJOB::STATE_RUNNING_MASK;
		if ((*it)->isReady())
		{
	...
	// Check changes:
	if (taskstate_old != m_taskprogress->state)
		taskchanged = true;
	if ((blockstate_old != blockstate_new) || (tasksready_old != tasksready_new) ||
		(tasksdone_old != tasksdone_new) || (taskserror_old != taskserror_new))
		blockProgress_changed = true;

	if (taskchanged && monitoring)
		m_tasks[0]->v_monitor(monitoring);
...
{
	for (std::list<SysTask *>::iterator it = m_systasks.begin(); it != m_systasks.end(); it++)
		if (tasknum == (*it)->getNumber())
			return *it;
	if (errorMessage)
		AFCommon::QueueLogError(std::string("SysJob::getTask: ") + errorMessage +
								": Invalid task number = " + af::itos(tasknum));
	else
		AFCommon::QueueLogError(std::string("SysJob::getTask: Invalid task number = ") + af::itos(tasknum));
	return NULL;
}
...
SysJob::SysJob(const std::string &i_folder) : JobAf(i_folder, true)
{
...
Block *SysJob::v_newBlock(int numBlock)
{
	...
	switch (numBlock)
	{
		...
		default: AFERRAR("SysJob::createBlock: Invalid block number = %d", numBlock)
	}
	return NULL;
}

void SysJob::AddPostCommand(const std::string &i_cmd, const std::string &i_wdir,
...
void SysJob::AddWOLCommand(const std::string &i_cmd, const std::string &i_wdir,
						   const std::string &i_user_name, const std::string &i_job_name)
{
	ms_block_wol->addCommand(new SysCmd(i_cmd, i_wdir, i_user_name, i_job_name));
}
void SysJob::AddEventCommand(const std::string &i_cmd, const std::string &i_wdir,
							 const std::string &i_user_name, const std::string &i_job_name,
...
void SysJob::v_updateTaskState(const af::MCTaskUp &taskup, RenderContainer *renders,
							   MonitorContainer *monitoring)
{
	...
	{
		AFCommon::QueueLogError("SysJob::updateTaskState: Invalid block number = " +
								af::itos(taskup.getNumBlock()));
		return;
	}
...
{
	o_error =
		"This is an empty dummy task in a system job block.\nError tasks output are stored in this task log.";
}

void SysJob::appendSysJobLog(const std::string &message) { ms_sysjob->appendLogInfo(message); }
...
SysBlockData::SysBlockData(int BlockNum, int JobId) : af::BlockData(BlockNum, JobId)
{
	//   initDefaults();
...
SysTaskData::SysTaskData() : af::TaskData() { m_name = "Dummy task. See all tasks logs here."; }

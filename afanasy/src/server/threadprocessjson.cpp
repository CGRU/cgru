...
af::Msg *threadProcessJSON(ThreadArgs *i_args, af::Msg *i_msg)
{
	...
	if (getObj.IsObject())
	{
		...
		if (type == "jobs")
		{
			...
			else if (mode == "output")
			{
				...
				if ((ids.size() == 1) && (block_ids.size() == 1) && (task_ids.size() == 1))
				{
					...
					if (mctask.hasOutput()) // Reading output from file
					{
						int readsize = -1;
						char *data =
							af::fileRead(mctask.getOutput(), &readsize, af::Msg::SizeDataMax, &error);
						if (data)
						{
			...
			else
			{
				...
				if (o_msg_response == NULL)
				{
					...
					o_msg_response = i_args->jobs->generateList(full ? af::Msg::TJob : af::Msg::TJobsList,
																type, ids, mask, json);
				}
			}
		}
		else if (type == "renders")
		{
			...
			if (o_msg_response == NULL)
			{
				if (mode == "resources")
					o_msg_response =
						i_args->renders->generateList(af::Msg::TRendersResources, type, ids, mask, json);
				else
					o_msg_response =
						i_args->renders->generateList(af::Msg::TRendersList, type, ids, mask, json);
			}
		}
		else if (type == "users")
		{
		...
		else if (type == "branches")
		{
			...
			if (o_msg_response == NULL)
				o_msg_response =
					i_args->branches->generateList(af::Msg::TBranchesList, type, ids, mask, json);
		}
		else if (type == "pools")
		{
		...
		else if (type == "monitors")
		{
			...
			if (NULL == o_msg_response)
				o_msg_response =
					i_args->monitors->generateList(af::Msg::TMonitorsList, type, ids, mask, json);
		}
		else if (type == "files")
		{
			std::string path;
	...
	else if (document.HasMember("job"))
	{
		...
		else
		{
			...
			o_msg_response = i_args->jobs->registerJob(document["job"], i_msg->getAddress(), i_args->branches,
													   i_args->users, i_args->monitors);
		}
	}
	...
	else if (document.HasMember("reload_config"))
	{
		...
		if (af::Environment::reload())
		{
			...
			o_msg_response = af::jsonMsgStatus(true, "reload_config", "Reloaded successfully.");
		}
		else
		{
			printf("\n	========= CONFIG RELOADING FAILED =========\n\n");
			o_msg_response = af::jsonMsgStatus(false, "reload_config", "Failed, see server logs fo details.");
		}
	}
...
af::Msg *jsonSaveObject(rapidjson::Document &i_obj)
{
	...
	str << "{\"save\":\n";
	str << "\t\"path\":\"" << path << "\",\n";
	if (af::pathFileExists(path))
		str << "\t\"overwrite\":true,\n";
	str << "\t\"size\":" << text.size() << "\n";
	str << "}}";

	if (false == AFCommon::writeFile(text, path))
		return af::jsonMsgError(std::string("Unable to write to \"") + path +

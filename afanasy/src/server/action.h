...
class Action
{
  public:
	...
	inline void setAnwer(AnswerType i_type, const std::string &i_answer)
	{
		m_answer_type = i_type;
		m_answer = i_answer;
	}

	inline void answerError(const std::string &i_msg) { setAnwer(ATError, i_msg); }
	inline void answerLog(const std::string &i_msg)
	{
		setAnwer(ATLog, i_msg);
		log.info = i_msg;
	}
	inline void answerInfo(const std::string &i_msg)
	{
		setAnwer(ATInfo, i_msg);
		log.info = i_msg;
	}

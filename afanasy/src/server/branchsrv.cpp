...
BranchSrv::BranchSrv(BranchSrv *i_parent, const std::string &i_path)
	: af::Branch(i_path), m_parent(i_parent), AfNodeSolve(this, "branch", "")
{
	appendLogInfo("Created from job.");
}

BranchSrv::BranchSrv(const std::string &i_store_dir)
	: af::Branch(), m_parent(NULL), AfNodeSolve(this, "branch", i_store_dir)
{
	int size;
...
bool BranchSrv::initialize()
{
	...
	else
	{
		...
		else
		{
			...
			if (m_parent->isSolveCapacity())
				setSolveCapacity();
			if (m_parent->isSolveTasksNum())
				setSolveTasksNum();

			if (m_parent->isSolvePriority())
				setSolvePriority();
			if (m_parent->isSolveOrder())
				setSolveOrder();
...
void BranchSrv::v_refresh(time_t i_currentTime, AfContainer *i_container, MonitorContainer *i_monitoring)
{
	...
	if ((_branches_total != m_branches_total) ||
		...
		(_jobs_total != m_jobs_total) || (_jobs_running != m_jobs_running) || (_jobs_done != m_jobs_done) ||
		(_jobs_ready != m_jobs_ready) || (_jobs_error != m_jobs_error) ||

		(_tasks_ready != m_tasks_ready) || (_tasks_error != m_tasks_error) ||
...
bool BranchSrv::v_canRunOn(RenderAf *i_render) { return true; }
...
void BranchSrv::v_postSolve(time_t i_curtime, MonitorContainer *i_monitoring)
{
	...
	for (std::list<BranchUserData *>::iterator it = m_active_users_list.begin();
	...
	for (; uIt != m_users.end(); uIt++)
	{
		...
		m_active_users_list.push_back(new BranchUserData((*uIt).first, (*uIt).second->running_tasks_num,
														 (*uIt).second->running_capacity_total));
	}
}

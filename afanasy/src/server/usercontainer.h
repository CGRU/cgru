...
class UserContainer : public AfContainer
{
  public:
	...
	/// only.
	UserAf *addUser(const std::string &i_username, const std::string &i_hostname,
	...
	/// Generate MCJobs message for users with provided ids:
	af::Msg *generateJobsList(const std::vector<int32_t> &ids, const std::string &i_type_name,
							  bool i_json = false);
	/// Generate MCJobs message for users with provided names:
	af::Msg *generateJobsList(const std::vector<std::string> &i_names, const std::string &i_type_name,
							  bool i_json = false);
...
class UserContainerIt : public AfContainerIt
{
  public:
	...
	inline UserAf *getUser(int i_id, const af::Msg *i_msg = NULL)
	{
		return static_cast<UserAf *>(get(i_id, i_msg));
	}

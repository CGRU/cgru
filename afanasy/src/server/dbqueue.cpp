...
DBQueue::DBQueue(const std::string &i_name, MonitorContainer *i_monitorcontainer)
	: af::AfQueue(i_name, af::AfQueue::e_start_thread), m_monitors(i_monitorcontainer), m_working(false),
	  m_conn(NULL)
{
...
void DBQueue::connectionEstablished() { AFINFA("DBQueue::connectionEstablished: %s", name.c_str()) }
...
void DBQueue::addTask(const af::TaskExec *i_exec, const af::TaskProgress *i_progress, const af::Job *i_job,

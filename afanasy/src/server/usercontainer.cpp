...
UserContainer::UserContainer() : AfContainer("Users", AFUSER::MAXCOUNT) { UserAf::setUserContainer(this); }

UserContainer::~UserContainer() { AFINFO("UserContainer::~UserContainer:\n"); }

UserAf *UserContainer::addUser(const std::string &i_usernmae, const std::string &i_hostname,
							   MonitorContainer *i_monitoring)
{
		...
		for (UserAf *user = usersIt.user(); user != NULL; usersIt.next(), user = usersIt.user())
		{
			if (i_usernmae == user->getName())
			{
				if (user->getHostName() != i_hostname)
				{
					...
					if (i_monitoring)
						i_monitoring->addEvent(af::Monitor::EVT_users_change, user->getId());
				}
				return user;
			}
		}
	...
	if (i_monitoring)
		i_monitoring->addEvent(af::Monitor::EVT_users_add, user->getId());
...
af::Msg *UserContainer::addUser(UserAf *i_user, MonitorContainer *i_monitoring)
{
	...
	if (addUser(i_user) == 0)
	{
		AFERRAR("UserContainer::addUser: Can't add user \"%s\" node to container.",
				i_user->getName().c_str());
		delete i_user;
	...
	i_user->store();
	if (i_monitoring)
		i_monitoring->addEvent(af::Monitor::EVT_users_add, i_user->getId());
...
af::Msg *UserContainer::generateJobsList(const std::vector<int32_t> &ids, const std::string &i_type_name,
										 bool i_json)
{
	return generateJobsList(ids, std::vector<std::string>(), i_type_name, i_json);
}

af::Msg *UserContainer::generateJobsList(const std::vector<std::string> &i_names,

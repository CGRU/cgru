...
Auth::Auth(const JSON &i_obj, const af::Address &i_address) : m_address(i_address), m_time(0), m_nc(0)
{
	std::string response;
		...
		if (af::jr_string("user_name", m_user_name, i_obj))
			if (af::jr_string("nonce", m_nonce, i_obj))
				if (af::jr_string("response", response, i_obj))
				{
					...
					if (m_digest.empty())
					{
						AFCommon::QueueLogError(std::string("AUTH: No such user: " + m_user_name + ": " +
															i_address.v_generateInfoString()));
						return;
					}
					...
					if (response == calcResponse())
						...
						AFCommon::QueueLogError(
							std::string("AUTH: Invalid digest: " + i_address.v_generateInfoString()));
...
bool Auth::process(const af::Msg *i_msg, af::Msg **o_msg)
{
	*o_msg = NULL;
	...
	{
		AFCommon::QueueLogError(
			std::string("AUTH: Not allowed message: " + i_msg->getAddress().v_generateInfoString()));
		return false;
	}
	...
	if (af::jr_int("nc", nc, i_obj))
		if (af::jr_string("user_name", user_name, i_obj))
			if (af::jr_string("response", response, i_obj))
			{
				...
				{
					AFCommon::QueueLogError(
						std::string("AUTH: Invalid IP: " + i_address.v_generateInfoString()));
					return false;
				}
				...
				{
					AFCommon::QueueLogError(
						std::string("AUTH: Invalid user name: " + i_address.v_generateInfoString()));
					return false;
				}
				...
				{
					AFCommon::QueueLogError(
						std::string("AUTH: Invalid nonce count: " + i_address.v_generateInfoString()));
					return false;
				}
				...
				{
					AFCommon::QueueLogError(
						std::string("AUTH: Invalid response: " + i_address.v_generateInfoString()));
					return false;
				}

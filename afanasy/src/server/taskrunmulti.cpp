...
TaskRunMulti::TaskRunMulti(Task *i_runningTask, af::TaskExec *i_taskExec, af::TaskProgress *i_taskProgress,
	...
	: TaskRun(i_runningTask,
			  ...
			  i_taskProgress, i_taskBlock, i_render, i_monitoring),
	  m_master_running(false), m_stopping(false), m_time_last_host_added(0), m_time_services_started(0),
	  m_time_services_stopped(0)
{
	...
	m_task->appendTaskLog("Starting to capture hosts:");
...
{
	AFINFA("TaskRunMulti:: ~ TaskRunMulti: %s[%d][%d]:", m_block->m_job->getName().c_str(),
		   m_block->m_data->getBlockNum(), m_tasknum)
	if (m_execs.size() != 0)
	{
		AFERRAR("TaskRunMulti:: ~ TaskRunMulti: %s[%d][%d]:", m_block->m_job->getName().c_str(),
				m_block->m_data->getBlockNum(), m_tasknum)
	}
	for (std::list<af::TaskExec *>::iterator it = m_execs.begin(); it != m_execs.end(); it++)
		delete *it;
...
{
	m_task->appendTaskLog(std::string("Host \"") + render->getName() + "\" added.");
...
void TaskRunMulti::startServices(RenderContainer *renders)
{
	...
	if (m_has_service == false)
		return;

	m_task->appendTaskLog("Starting services on slave hosts.");
...
void TaskRunMulti::startMaster(RenderContainer *renders, MonitorContainer *monitoring)
{
	...
	m_task->v_monitor(monitoring);
	m_task->v_store();
	m_task->appendTaskLog(std::string("Starting master on \"") + render->getName() + "\"");
}
...
void TaskRunMulti::update(const af::MCTaskUp &taskup, RenderContainer *renders, MonitorContainer *monitoring,
						  bool &errorHost)
{
	// printf("TaskRunMulti::update: [%d][%d][%d](%d):\n", taskup.getNumJob(), taskup.getNumBlock(),
	// taskup.getNumTask(), taskup.getNumber());
	...
	if (isZombie())
	{
		AFERRAR("TaskRunMulti::update: ZOMBIE %s[%d][%d]", m_block->m_job->getName().c_str(),
				m_block->m_data->getBlockNum(), m_tasknum)
		return;
	}
	...
	switch (taskup.getStatus())
	{
		...
		case af::TaskExec::UPLAST: return;

		case af::TaskExec::UPFinishedSuccess:
			m_task->appendTaskLog("Slave host service finished.");
		...
		case af::TaskExec::UPFailedToStart:
			m_task->appendTaskLog("Failed to start slave host service.");
			releaseHost(renders, monitoring, &taskup);
			errorHost = true;
			break;

		case af::TaskExec::UPFinishedKilled:
			m_task->appendTaskLog("Slave host service was killed.");
			releaseHost(renders, monitoring, &taskup);
			if (m_stopping == false)
				errorHost = true;
			break;
		...
		case af::TaskExec::UPFinishedError:
			m_task->appendTaskLog("Slave host service finished with error.");
			releaseHost(renders, monitoring, &taskup);
			if (m_stopping == false)
				errorHost = true;
			break;
		...
		case af::TaskExec::UPEject:
			m_task->appendTaskLog("Host owner ejected slave.");
			...
			else
			{
				...
				m_task->appendTaskLog("Ignoring, master task should be stopped to free slaves.");
				/*
		...
		case af::TaskExec::UPRenderDeregister:
			m_task->appendTaskLog("Slave deregistered.");
			releaseHost(renders, monitoring, &taskup);
			break;

		case af::TaskExec::UPRenderExit:
			m_task->appendTaskLog("Slave exited.");
			releaseHost(renders, monitoring, &taskup);
			break;

		case af::TaskExec::UPRenderZombie:
			m_task->appendTaskLog("Slave became a zombie.");
			releaseHost(renders, monitoring, &taskup);
			errorHost = true;
			break;
	...
	if (m_time_services_started && (m_stopping == false))
	{
		...
		if (m_block->m_data->isSlaveLostIgnore())
		{
			...
			if (numslaves)
			{
				m_task->appendTaskLog("Ignoring slave lost, current slaves count = " + af::itos(numslaves));
				return;
			}
		}
						   ...
						   int &errorHostId)
{
		...
		if (m_time_services_started == false)
				 ...
				 (currentTime - m_time_last_host_added >= m_block->m_data->getMultiHostWaitMax())))
			{
				if (m_progress->state & AFJOB::STATE_READY_MASK)
					m_progress->state = m_progress->state & (~AFJOB::STATE_READY_MASK);
				setMasterTask();
				m_task->appendTaskLog(std::string("Finished waiting for hosts.\nCaptured slaves:\n") +
	...
	if (m_master_running == false)
		return changed;

	if (TaskRun::refresh(currentTime, renders, monitoring, errorHostId))
		changed = true;
						...
						uint32_t i_state)
{
	...
	// block->data->getBlockNum(), tasknum, message.toUtf8().data());
	...
	while (tIt != end)
	{
		...
		if (m_has_service && m_time_services_started)
		{
			...
			m_task->appendTaskLog(std::string("Stopping service[") + af::itos((*tIt)->getNumber()) +
		...
		else
		{
			...
			m_task->appendTaskLog(std::string("Finished task[") + af::itos((*tIt)->getNumber()) +
								  "] on host \"" + render->getName() + "\"");
			delete *tIt;
...
void TaskRunMulti::stopSlaveService(RenderContainer *renders, MonitorContainer *monitoring,
									const af::MCTaskUp &taskup)
{
	RenderContainerIt rendersIt(renders);
	std::list<int>::iterator hIt = m_hostids.begin();
	std::list<af::TaskExec *>::iterator tIt = m_execs.begin();
...
void TaskRunMulti::finish(const std::string &message, RenderContainer *renders, MonitorContainer *monitoring)
{
	...
	if (m_stopping == false)
		stop(message, renders, monitoring);
	else if (m_execs.size() == 0)
	{
		// Finish master if it was not running and there are no service on slaves
...
void TaskRunMulti::releaseHost(RenderContainer *renders, MonitorContainer *monitoring,
							   const af::MCTaskUp *taskup)
{
	// printf("TaskRunMulti::releaseHost: %s[%d][%d](%d)\n", block->job->getName().toUtf8().data(),
	// block->data->getBlockNum(), tasknum, taskup ? taskup->getNumber() : -1);
	...
	{
		if ((taskup != NULL) && ((*tIt)->equals(*taskup) == false))
			continue;

		RenderAf *render = rendersIt.getRender(*hIt);
		if (render == NULL)
		{
			AFERRAR("TaskRunMulti::releaseHost: Render[%d] is NULL %s[%d][%d](%d)", *hIt,
					(*tIt)->getJobName().c_str(), (*tIt)->getBlockNum(), (*tIt)->getTaskNum(),
					(*tIt)->getNumber())
		}
		else
		{
			render->taskFinished(*tIt, m_progress->state, monitoring);
			m_task->appendTaskLog(std::string("Releasing task[") + af::itos((*tIt)->getNumber()) +
								  "] on host \"" + render->getName() + "\"");
		}
		delete *tIt;
		tIt = m_execs.erase(tIt);
...
void TaskRunMulti::stdOut(bool full) const
{
	...
	if (m_stopping)
		printf("Stp, ");
	printf("LHA=%d, SStarted=%d, SStopped=%d", m_time_last_host_added, m_time_services_started,
		   m_time_services_stopped);
	printf("\n");
	...
	for (; tIt != end; hIt++, tIt++, nIt++)
	{
		if (full)
			...
			printf("Exec=[%d][%d][%d](%d) host='%s'[%d]\n", (*tIt)->getJobId(), (*tIt)->getBlockNum(),
				   (*tIt)->getTaskNum(), (*tIt)->getNumber(), (*nIt).c_str(), *hIt);
	}
}

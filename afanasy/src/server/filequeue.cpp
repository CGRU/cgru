...
FileData::FileData(const std::ostringstream &i_str, const std::string &i_file_name,
				   const std::string &i_folder_name)
	: m_file_name(i_file_name), m_folder_name(i_folder_name), m_data(NULL)
{
	m_str = i_str.str();
	m_length = m_str.size();
}
...
FileData::FileData(const char *i_data, int i_length, const std::string &i_file_name,
				   const std::string &i_folder_name)
	: m_file_name(i_file_name), m_folder_name(i_folder_name), m_length(i_length), m_data(NULL)
{
...
FileData::FileData(const AfNodeSrv *i_node) : m_length(0), m_data(NULL)
{
...
FileQueue::FileQueue(const std::string &QueueName) : af::AfQueue(QueueName, af::AfQueue::e_start_thread) {}

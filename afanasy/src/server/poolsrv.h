...
class PoolSrv : public af::Pool, public AfNodeFarm
{
  public:
	...
	void taskAcuire(const af::TaskExec *i_taskexec, const std::list<std::string> &i_new_tickets,
					MonitorContainer *i_monitoring);
	void taskRelease(const af::TaskExec *i_taskexec, const std::list<std::string> &i_exp_tickets,
					 MonitorContainer *i_monitoring);
	...
	{
		if (!m_new_nimby && m_parent)
			return m_parent->newNimby();
		else
			return m_new_nimby;
	}
	inline bool newPaused() const
	{
		if (!m_new_paused && m_parent)
			return m_parent->newPaused();
		else
			return m_new_paused;
	}
	...
	{
		if (m_sick_errors_count < 0 && m_parent)
			return m_parent->getSickErrorsCount();
		else
			return m_sick_errors_count;
	}

	inline int calcPriority() const
	{
		if (m_parent)
			return (m_priority + m_parent->calcPriority()) / 2;
		else
			return m_priority;
	}
	...
	{
		if (m_heartbeat_sec <= 0 && m_parent)
			return m_parent->getHeartBeatSec();
		else
	...
	{
		if (m_resources_update_period <= 0 && m_parent)
			return m_parent->getResourcesUpdatePeriod();
		else
	...
	{
		if (m_zombie_time <= 0 && m_parent)
			return m_parent->getZombieTime();
		else
			return m_zombie_time;
	}

	inline int32_t getExitNoTaskTime() const
	{
		if (m_exit_no_task_time <= 0 && m_parent)
			return m_parent->getExitNoTaskTime();
		else
			return m_exit_no_task_time;
	}

	inline int32_t getNoTaskEventTime() const
	{
		if (m_no_task_event_time <= 0 && m_parent)
			return m_parent->getNoTaskEventTime();
		else
			return m_no_task_event_time;
	}

	inline int32_t getOverloadEventTime() const
	{
		if (m_overload_event_time <= 0 && m_parent)
			return m_parent->getOverloadEventTime();
		else
			return m_overload_event_time;
	}

	// Idle & Busy
	inline int get_idle_wolsleep_time() const
	{
		if (m_idle_wolsleep_time < 0 && m_parent)
			return m_parent->get_idle_wolsleep_time();
		else
			return m_idle_wolsleep_time;
	}
	inline int get_idle_free_time() const
	{
		if (m_idle_free_time < 0 && m_parent)
			return m_parent->get_idle_free_time();
		else
			return m_idle_free_time;
	}
	inline int get_busy_nimby_time() const
	{
		if (m_busy_nimby_time < 0 && m_parent)
			return m_parent->get_busy_nimby_time();
		else
			return m_busy_nimby_time;
	}
	inline int get_idle_cpu() const
	{
		if (m_idle_cpu < 0 && m_parent)
			return m_parent->get_idle_cpu();
		else
			return m_idle_cpu;
	}
	inline int get_busy_cpu() const
	{
		if (m_busy_cpu < 0 && m_parent)
			return m_parent->get_busy_cpu();
		else
			return m_busy_cpu;
	}
	inline int get_idle_mem() const
	{
		if (m_idle_mem < 0 && m_parent)
			return m_parent->get_idle_mem();
		else
			return m_idle_mem;
	}
	inline int get_busy_mem() const
	{
		if (m_busy_mem < 0 && m_parent)
			return m_parent->get_busy_mem();
		else
			return m_busy_mem;
	}
	inline int get_idle_swp() const
	{
		if (m_idle_swp < 0 && m_parent)
			return m_parent->get_idle_swp();
		else
			return m_idle_swp;
	}
	inline int get_busy_swp() const
	{
		if (m_busy_swp < 0 && m_parent)
			return m_parent->get_busy_swp();
		else
			return m_busy_swp;
	}
	inline int get_idle_hddgb() const
	{
		if (m_idle_hddgb < 0 && m_parent)
			return m_parent->get_idle_hddgb();
		else
			return m_idle_hddgb;
	}
	inline int get_busy_hddgb() const
	{
		if (m_busy_hddgb < 0 && m_parent)
			return m_parent->get_busy_hddgb();
		else
			return m_busy_hddgb;
	}
	inline int get_idle_hddio() const
	{
		if (m_idle_hddio < 0 && m_parent)
			return m_parent->get_idle_hddio();
		else
			return m_idle_hddio;
	}
	inline int get_busy_hddio() const
	{
		if (m_busy_hddio < 0 && m_parent)
			return m_parent->get_busy_hddio();
		else
			return m_busy_hddio;
	}
	inline int get_idle_netmbs() const
	{
		if (m_idle_netmbs < 0 && m_parent)
			return m_parent->get_idle_netmbs();
		else
			return m_idle_netmbs;
	}
	inline int get_busy_netmbs() const
	{
		if (m_busy_netmbs < 0 && m_parent)
			return m_parent->get_busy_netmbs();
		else
			return m_busy_netmbs;
	}

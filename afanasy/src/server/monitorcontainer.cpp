...
MonitorContainer::MonitorContainer()
	: AfContainer("Monitors", AFMONITOR::MAXCOUNT), m_events(NULL), m_jobEvents(NULL), m_jobEventsUids(NULL)
{
...
MonitorContainer::~MonitorContainer()
{
	...
	if (m_jobEvents != NULL)
		delete[] m_jobEvents;
	if (m_jobEventsUids != NULL)
		delete[] m_jobEventsUids;
}
...
void MonitorContainer::addEvent(int i_type, int i_nodeId)
{
	...
	{
		AFCommon::QueueLogError(std::string("MonitorContainer::addEvent: Invalid event type number: ") +
								af::itos(i_type));
		return;
	}
...
void MonitorContainer::addJobEvent(int i_type, int i_jid, int i_uid)
{
	...
	{
		AFCommon::QueueLogError(std::string("MonitorContainer::addJobEvent: Invalid job event type = ") +
								af::itos(i_type));
		return;
	}
...
void MonitorContainer::outputsReceived(const std::vector<af::MCTaskPos> &i_outspos,
									   const std::vector<std::string> &i_outputs)
{
	...
	for (MonitorAf *monitor = monitorsIt.monitor(); monitor != NULL;
		 monitorsIt.next(), monitor = monitorsIt.monitor())
...
void MonitorContainer::dispatch(RenderContainer *i_renders)
{
		...
		if (m_jobEvents[e].size() != m_jobEventsUids[e].size())
		{
			AFCommon::QueueLogError(std::string("MonitorContainer::dispatch: \
				jobEvents and jobEventsUids has different sizes: ") +
									af::itos(m_jobEvents[e].size()) +
									" != " + af::itos(m_jobEventsUids[e].size()));
			continue;
		}
	...
	for (int i = 0; i < m_listens.size(); i++)
	{
		...
		if (false == founded)
		{
			AFCommon::QueueLog(std::string("Removing not listening task: ") + m_listens[i].m_task_name);

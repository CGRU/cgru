...
AfContainerLock::AfContainerLock(AfContainer *afcontainer, LockType locktype)
	: m_container(afcontainer), m_type(locktype)
{
	switch (m_type)
	{
		case READLOCK:
		...
		default: AF_ERR << "invalid lock type.";
	}
}
...
AfContainerIt::AfContainerIt(AfContainer *af_container, bool skip_zombies)
	: m_container(af_container), m_by_pass_zombies(skip_zombies)
{
	reset();
}
...
AfNodeSrv *AfContainerIt::get(int id, const af::Msg *i_msg)
{
	...
	{
		AFCommon::QueueLogError(m_container->m_name + ".get(): id=" + af::itos(id) +
								" >= size=" + af::itos(m_container->m_capacity) + ": " +
								(i_msg ? i_msg->v_generateInfoString() : ""));
		return NULL;
	}

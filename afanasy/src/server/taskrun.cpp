...
TaskRun::TaskRun(Task *runningTask, af::TaskExec *taskExec, af::TaskProgress *taskProgress, Block *taskBlock,
				 RenderAf *render, MonitorContainer *monitoring)
	: m_task(runningTask), m_block(taskBlock), m_exec(taskExec), m_progress(taskProgress), m_tasknum(0),
	  m_hostId(0), m_stopTime(0), m_zombie(false)
{
	AF_DEBUG << "TaskRun::TaskRun: " << m_block->m_job->getName() << "[" << m_block->m_data->getBlockNum()
			 << "][" << m_tasknum << "]:";
	...
	m_task->v_store();
	m_task->appendTaskLog(std::string("SESSION #") + af::itos(m_progress->starts_count) + ": Starting on \"" +
						  render->getName() + "\"");
}

TaskRun::~TaskRun()
{
	AFINFA("TaskRun:: ~ TaskRun: %s[%d][%d]:", m_block->m_job->getName().c_str(),
	...
	if (m_progress->state & AFJOB::STATE_ERROR_MASK)
		return;
	if (m_progress->state & AFJOB::STATE_SKIPPED_MASK)
		return;
...
void TaskRun::update(const af::MCTaskUp &taskup, RenderContainer *renders, MonitorContainer *monitoring,
					 bool &o_error_host)
{
	if (m_zombie)
	{
	...
	{
		AF_ERR << m_block->m_job->getName() << "[" << m_block->m_data->getBlockNum() << "][" << m_tasknum
			   << "] task is not running.";
		return;
	}
	if (NULL == m_exec)
	...
	if (taskup.hasActivity())
		m_progress->activity = taskup.getActivity();
	if (taskup.hasResources())
		m_progress->resources = taskup.getResources();
	...
	switch (taskup.getStatus())
	{
		...
		case af::TaskExec::UPFinishedSuccess:
		{
			...
			// UPFinishedFailedPost (finished itself with a good exit status, but post task command was
			// failed). And this check definitely should be skipped in UPSkip (when user skipped running
			// task). Also it should be skipped on UPFinishedParserSuccess, as in parser we already made a
			// decision that render was good.
			if ((m_stopTime == 0) && (m_block->m_data->getTaskMinRunTime() > 0))
			{
					...
					(taskup.getStatus() == af::TaskExec::UPFinishedFailedPost))
				{
					...
					if (run_time < m_block->m_data->getTaskMinRunTime())
					{
						...
						message = "Task running time (" + af::itos(run_time) + "s) is less than a minimum (" +
								  af::itos(m_block->m_data->getTaskMinRunTime()) + "s)";
		...
		case af::TaskExec::UPFinishedError:
		{
			...
			else
			{
				...
				m_progress->errors_count++;
				// In Finish procedure task updates it's progress database (and write there new number of
				// errors)
				finish(message, renders, monitoring);
				o_error_host = true;
			}
		...
		default: AFERRAR("TaskRun::update: Unknown task update status = %d", taskup.getStatus())
	}
}
					  ...
					  int &errorHostId)
{
	...
	if (NULL == m_exec)
	{
		AF_ERR << "TaskRun::refresh: " << m_block->m_job->getName() << "[" << m_block->m_data->getBlockNum()
			   << "][" << m_tasknum << "] Task executable is NULL.";
		return false;
	}
	// printf("TaskRun::refresh: %s[%d][%d]\n", block->job->getName().toUtf8().data(),
	// block->data->getBlockNum(), tasknum);
	bool changed = false;
	...
	if ((timeout > 0) && (no_progress_for > timeout))
	{
		m_progress->state = m_progress->state | AFJOB::STATE_ERROR_MASK;
		m_progress->errors_count++;
		stop("Task run time without progress reached (no progress for " + af::itos(no_progress_for) + "s).",
			 renders, monitoring);
		errorHostId = m_hostId;
	}
...
void TaskRun::stop(const std::string &message, RenderContainer *renders, MonitorContainer *monitoring,
				   uint32_t i_state)
{
	// printf("TaskRun::stop: %s[%d][%d] HostID=%d: %s\n", m_block->m_job->getName().c_str(),
	// m_block->m_data->getBlockNum(), m_tasknum, m_hostId, message.c_str());
	if (m_zombie)
		return;
	if (m_stopTime)
		return;
	if (NULL == m_exec)
	{
		AFERRAR("TaskRun::stop: %s[%d][%d] Task executable is NULL.", m_block->m_job->getName().c_str(),
				m_block->m_data->getBlockNum(), m_tasknum)
		return;
	}

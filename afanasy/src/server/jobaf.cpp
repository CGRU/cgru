...
JobAf::JobAf(JSON &i_object) : af::Job(), AfNodeSolve(this, "job", "")
{
	initializeValues();
...
JobAf::JobAf(const std::string &i_store_dir, bool i_system) : af::Job(), AfNodeSolve(this, "job", i_store_dir)
{
...
void JobAf::initStoreDirs() { m_store_dir_tasks = getStoreDir() + AFGENERAL::PATH_SEPARATOR + "tasks"; }
...
Block *JobAf::v_newBlock(int numBlock) { return new Block(this, m_blocks_data[numBlock], m_progress); }
	...
	if (isFromStore() == false)
	{
		...
		for (int b = 0; b < m_blocks_num; b++)
		{
			if (m_blocks_data[b]->hasCmdPre())
			{
				...
				appendLogTypeInfo("pre command", std::string("Block[") + m_blocks_data[b]->getName() +
...
void JobAf::checkStates()
{
	...
	for (int b = 0; b < m_blocks_num; b++)
	{
		...
		for (int t = 0; t < numtasks; t++)
		{
			...
			else if (taskstate & AFJOB::STATE_RUNNING_MASK && (false == m_blocks_data[b]->isMultiHost()))
			{
				...
				m_blocks[b]->m_tasks[t]->appendTaskLog(
	...
	if ((m_state & AFJOB::STATE_DONE_MASK) == false)
...
void JobAf::deleteNode(RenderContainer *renders, MonitorContainer *monitoring)
{
	...
	if (false == m_deletion)
	{
		...
		if (getRunningTasksNum() && (renders != NULL) && (monitoring != NULL))
		{
			...
			if (monitoring)
				monitoring->addJobEvent(af::Monitor::EVT_jobs_change, getId(), getUid());
			return;
		}
	}
	...
	{
		SysJob::AddPostCommand(m_command_post, m_blocks_num > 0 ? m_blocks_data[0]->getWDir() : "",
							   m_user_name, m_name);
	...
	for (int b = 0; b < m_blocks_num; b++)
	{
		...
		{
			SysJob::AddPostCommand(m_blocks_data[b]->getCmdPost(), m_blocks_data[b]->getWDir(), m_user_name,
								   m_name);
			appendLogTypeInfo("post command", std::string("Block[") + m_blocks_data[b]->getName() +
...
void JobAf::v_action(Action &i_action)
{
	...
	if (i_action.data->HasMember("block_ids") || i_action.data->HasMember("block_mask"))
	{
		...
		if (block_ids.empty())
		{
			i_action.answerError(
				"\"block_ids\" array does not contain any integers or invalid \"block_mask\"");
			return;
		}
	...
	if (operation.IsObject())
	{
		...
		{
			// restartAllTasks("Job stopped by " + i_action.author, i_action.renders, i_action.monitors,
			// AFJOB::STATE_RUNNING_MASK);
			restartAllTasks(i_action.log, i_action.renders, i_action.monitors, AFJOB::STATE_RUNNING_MASK);
			m_state = m_state | AFJOB::STATE_OFFLINE_MASK;
		}
		else if (type == "restart_warnings")
		{
			// restartAllTasks("Job restart warnings by " + i_action.author,  i_action.renders,
			// i_action.monitors, AFJOB::STATE_WARNING_MASK);
			restartAllTasks(i_action.log, i_action.renders, i_action.monitors, AFJOB::STATE_WARNING_MASK);
		}
		else if (type == "restart_running")
		{
			// restartAllTasks("Job restart running by " + i_action.author,  i_action.renders,
			// i_action.monitors, AFJOB::STATE_RUNNING_MASK);
			restartAllTasks(i_action.log, i_action.renders, i_action.monitors, AFJOB::STATE_RUNNING_MASK);
		}
		else if (type == "restart_skipped")
		{
			// restartAllTasks("Job restart skipped by " + i_action.author,  i_action.renders,
			// i_action.monitors, AFJOB::STATE_SKIPPED_MASK);
			restartAllTasks(i_action.log, i_action.renders, i_action.monitors, AFJOB::STATE_SKIPPED_MASK);
		}
		else if (type == "restart_done")
		{
			// restartAllTasks("Job restart done by " + i_action.author,  i_action.renders, i_action.monitors,
		...
		{
			// restartAllTasks("Job errors restarted by " + i_action.author,  i_action.renders,
			// i_action.monitors, AFJOB::STATE_ERROR_MASK);
			restartAllTasks(i_action.log, i_action.renders, i_action.monitors, AFJOB::STATE_ERROR_MASK);
		}
		else if (type == "restart_pause")
		{
			// restartAllTasks("Job restarted ( and paused ) by " + i_action.author,  i_action.renders,
			// i_action.monitors);
			restartAllTasks(i_action.log, i_action.renders, i_action.monitors);
			checkDepends();
			m_state = m_state | AFJOB::STATE_OFFLINE_MASK;
...
bool JobAf::checkTryTasksNext()
{
	...
	AF_ERR << "Try next tasks num and blocks num mismatch: " << m_try_this_tasks_num.size()
		   << " != " << m_try_this_blocks_num.size();
...
bool JobAf::solveOnRender(RenderAf *i_render, MonitorContainer *i_monitoring)
{
	...
	for (int b = 0; b < m_blocks_num; b++)
	{
		if (false == (m_blocks_data[b]->getState() & AFJOB::STATE_READY_MASK))
			continue;
...
bool JobAf::solveTaskOnRender(RenderAf *i_render, int i_block_num, int i_task_num,
...
af::TaskExec *JobAf::genTask(RenderAf *i_render, int i_block, int i_task)
{
	...
	{
		AF_ERR << "block[" << i_block << "] '" << m_blocks_data[i_block]->getName() << "' : " << i_task
			   << " >= number of tasks = " << m_blocks_data[i_block]->getTasksNum();
		return NULL;
	}
...
void JobAf::v_updateTaskState(const af::MCTaskUp &taskup, RenderContainer *renders,
							  MonitorContainer *monitoring)
{
	if (false == checkBlockTaskNumbers(taskup.getNumBlock(), taskup.getNumTask(), "updateTaskState"))
		return;
	bool errorHost = false;
	m_blocks[taskup.getNumBlock()]->m_tasks[taskup.getNumTask()]->v_updateState(taskup, renders, monitoring,
																				errorHost);
	if (errorHost)
		m_blocks[taskup.getNumBlock()]->v_errorHostsAppend(taskup.getNumTask(), taskup.getClientId(),
														   renders);
...
void JobAf::v_refresh(time_t currentTime, AfContainer *pointer, MonitorContainer *monitoring)
{
	...
	if (m_id != AFJOB::SYSJOB_ID) // skip system job
	{
		...
		if (result_lifetime < 0)
			result_lifetime = m_user->getJobsLifeTime(); // get default value from user
		if ((result_lifetime > 0) && ((currentTime - m_time_creation) > result_lifetime))
		{
			af::Log log("jobs", getName() + "@" + getUserName(),
						"Life time finished: " + af::time2strHMS(result_lifetime, true));
	...
	// check wait time
	{
		...
		if (m_time_wait > currentTime)
			m_state = m_state | AFJOB::STATE_WAITTIME_MASK;
		else
			m_state = m_state & (~AFJOB::STATE_WAITTIME_MASK);
		bool nowWaining = m_state & AFJOB::STATE_WAITTIME_MASK;
		if (wasWaiting != nowWaining)
	...
	// Compare changes
	if ((_tasks_ready != m_tasks_ready) || (_tasks_error != m_tasks_error))
		jobchanged = af::Monitor::EVT_jobs_change;
	...
	if (m_state & AFJOB::STATE_WAITDEP_MASK)
		m_state = m_state & (~AFJOB::STATE_READY_MASK);
	if (m_state & AFJOB::STATE_WAITTIME_MASK)
		m_state = m_state & (~AFJOB::STATE_READY_MASK);
	...
	if ((m_time_started != 0) && (false == (m_state & AFJOB::STATE_RUNNING_MASK)) &&
	...
	if ((monitoring) && (jobchanged))
		monitoring->addJobEvent(jobchanged, getId(), getUid());
...
void JobAf::restartAllTasks(const af::Log &i_log, RenderContainer *i_renders, MonitorContainer *i_monitoring,
							uint32_t i_with_state)
{
	for (int b = 0; b < m_blocks_num; b++)
	{
		int numtasks = m_blocks_data[b]->getTasksNum();
		for (int t = 0; t < numtasks; t++)
		{
			m_blocks[b]->m_tasks[t]->operation(i_log, i_renders, i_monitoring, i_with_state,
...
af::Msg *JobAf::writeErrorHosts(int b, int t) const
{
	...
	{
		return af::jsonMsgError(std::string("Job '") + m_name + "' invalid task number: " + af::itos(b) +
								", " + af::itos(t) + ".");
	}
...
bool JobAf::checkBlockTaskNumbers(int i_b, int i_t, const char *o_str) const
{
	...
	{
		if (o_str)
			AF_ERR << o_str << ": numtask >= numTasks ( " << i_t
				   << " >= " << m_blocks_data[i_b]->getTasksNum() << " )";
		else
...
int JobAf::v_calcWeight() const
{
	...
	if (m_progress != NULL)
		progressWeight = m_progress->calcWeight();

...
int main(int argc, char *argv[])
{
	...
	if (af::pathMakePath(ENV.getStoreFolder(), af::VerboseOn) == false)
		return 1;
	if (af::pathMakeDir(ENV.getStoreFolderBranches(), af::VerboseOn) == false)
		return 1;
	if (af::pathMakeDir(ENV.getStoreFolderJobs(), af::VerboseOn) == false)
		return 1;
	if (af::pathMakeDir(ENV.getStoreFolderRenders(), af::VerboseOn) == false)
		return 1;
	if (af::pathMakeDir(ENV.getStoreFolderUsers(), af::VerboseOn) == false)
		return 1;
	if (af::pathMakeDir(ENV.getStoreFolderPools(), af::VerboseOn) == false)
		return 1;

// Server for windows can be me more simple and not use signals at all.
// Windows is not a server platform, so it designed for individual tests or very small companies with easy
// load.
#ifndef _WIN32
	...
	if (false == branches.isInitialized())
		return 1;

	JobContainer jobs;
	if (false == jobs.isInitialized())
		return 1;

	UserContainer users;
	if (false == users.isInitialized())
		return 1;

	PoolsContainer pools;
	if (false == pools.isInitialized())
		return 1;

	RenderContainer renders;
	if (false == renders.isInitialized())
		return 1;

	MonitorContainer monitors;
	if (false == monitors.isInitialized())
		return 1;

	af::RenderUpdatetQueue rupQueue("RenderUpdatetQueue");
	if (false == rupQueue.isInitialized())
		return 1;
	...
	*/
	...
	bool hasSystemJob = false;
	{
		...
		std::string sysjob_folder =
			AFCommon::getStoreDir(ENV.getStoreFolderJobs(), AFJOB::SYSJOB_ID, AFJOB::SYSJOB_NAME);

...
MonitorAf::MonitorAf(af::Msg *msg, UserContainer *i_users) : af::Monitor(msg), AfNodeSrv(this, "monitor", "")
{
...
MonitorAf::MonitorAf(const JSON &i_obj, UserContainer *i_users)
	: AfNodeSrv(this, "monitor", ""), af::Monitor(i_obj)
{
	UserAf *user = i_users->getUser(m_user_name);
	if (user)
	{
...
void MonitorAf::v_refresh(time_t currentTime, AfContainer *pointer, MonitorContainer *monitoring)
{
	...
	{
		if (monitoring)
			monitoring->addEvent(af::Monitor::EVT_monitors_del, m_id);
		{
...
void MonitorAf::v_action(Action &i_action)
{
	...
	if (operation.IsObject())
	{
		...
		else if (optype == "watch")
		{
			...
			// appendLog("Operation '" + optype + "' class '" + opclass + "' status '" + opstatus + "' by " +
			// i_action.author);
...
{
	// printf("MonitorAf::addJobIds:[%d]",getId());for(int i=0;i<i_ids.size();i++)printf("
	// %d",i_ids[i]);printf("\n");
	for (int i = 0; i < i_ids.size(); i++)
	{
...
{
	// printf("MonitorAf::delJobIds:[%d]",getId());for(int i=0;i<i_ids.size();i++)printf("
	// %d",i_ids[i]);printf("\n");
	for (int i = 0; i < i_ids.size(); i++)
		m_jobsIds.remove(i_ids[i]);
}
...
{
	// std::ostringstream str;af::jw_state( i_tp->state, str);printf("MonitorAf::addTaskProgress():j=%d b=%d
	// t=%d s='%s'\n", i_j, i_b, i_t, str.str().c_str());

	for (int j = 0; j < m_e.m_tp.size(); j++)
	{
		if (m_e.m_tp[j].job_id != i_j)
			continue;

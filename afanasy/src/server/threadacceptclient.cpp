...
void threadAcceptPort(void *i_arg, int i_port)
{
	...
	for (struct addrinfo *ai = res; ai != NULL; ai = ai->ai_next)
	{
		switch (ai->ai_family)
		{
			...
			case AF_INET6:
			{
				...
				const char *addr_str =
					inet_ntop(AF_INET6, &(((sockaddr_in6 *)(ai->ai_addr))->sin6_addr), buffer, buffer_len);
				printf("IPv6 = '%s'\n", addr_str);
				break;
			}
			default: printf("Unsupported address family, skipping.\n"); continue;
		}
	}
	freeaddrinfo(res);
	...
	switch (protocol)
	{
		case AF_INET: printf("Using IPv4 addresses family.\n"); break;
		case AF_INET6:
...
void threadAcceptClient(void *i_arg) { threadAcceptPort(i_arg, af::Environment::getServerPort()); }

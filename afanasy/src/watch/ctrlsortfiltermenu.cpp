...
CtrlSortFilterMenu::CtrlSortFilterMenu(QWidget *i_parent, const int *i_checked_type)
	: QLabel(i_parent), m_checked_type(i_checked_type)
{
...
void CtrlSortFilterMenu::mouseDoubleClickEvent(QMouseEvent *i_event) { emit sig_dblclick(); }

void CtrlSortFilterMenu::slot_changed(int i_type) { emit sig_changed(i_type); }

...
WndTask::WndTask(const af::MCTaskPos &i_tp, ListTasks *i_parent)
	: Wnd("Task"), m_parent(i_parent), m_pos(i_tp), m_log_te(NULL), m_errhosts_te(NULL), m_output_te(NULL),
	  m_outputs_count(-1), m_output_current(-1), m_tab_current(NULL), m_listening(false)
{
...
void WndTask::updateProgress(const af::TaskProgress &i_progress)
{
	...
	if (m_progress.state & AFJOB::STATE_READY_MASK)
		str << ' ' << AFJOB::STATE_READY_NAME;
	if (m_progress.state & AFJOB::STATE_RUNNING_MASK)
		str << ' ' << AFJOB::STATE_RUNNING_NAME;
	if (m_progress.state & AFJOB::STATE_DONE_MASK)
		str << ' ' << AFJOB::STATE_DONE_NAME;
	if (m_progress.state & AFJOB::STATE_SKIPPED_MASK)
		str << ' ' << AFJOB::STATE_SKIPPED_NAME;
	if (m_progress.state & AFJOB::STATE_ERROR_MASK)
		str << ' ' << AFJOB::STATE_ERROR_NAME;
	str << "</b>";

	if (m_progress.state & AFJOB::STATE_RUNNING_MASK)
		str << " <b>" << int(m_progress.percent) << "%</b>";
	if (m_progress.starts_count)
		str << " Starts: <b>" << m_progress.starts_count << "</b>";
	if (m_progress.errors_count)
		str << " (<b>" << m_progress.errors_count << "</b> errors)";
	if (m_progress.hostname.size())
		str << " Last host: <b>" << m_progress.hostname << "</b>";

	if (m_progress.time_start && m_progress.time_done &&
		(false == (m_progress.state & AFJOB::STATE_RUNNING_MASK)))
	...
	if ((m_progress.state & AFJOB::STATE_READY_MASK) || (m_progress.state & AFJOB::STATE_RUNNING_MASK))
		m_btn_skip->setEnabled(true);
	else
		m_btn_skip->setEnabled(false);

	if ((m_progress.state & AFJOB::STATE_RUNNING_MASK) || (m_progress.state & AFJOB::STATE_DONE_MASK) ||
...
ButtonMenu::ButtonMenu(const QString &i_file, const QString &i_wdir,
	...
	: QPushButton("Launch", i_parent), m_file(i_file), m_wdir(i_wdir), m_env(i_env)
{

...
LabelVersion::LabelVersion(QWidget *parent) : QWidget(parent), m_status(SS_None)
{
	...
	m_tooltip += QString("\nServer: %1:%2")
					 ...
					 .arg(af::Environment::getServerPort());
...
void LabelVersion::paintEvent(QPaintEvent *event)
{
	...
	if (m_status != SS_None)
	{
		...
		{
			p.drawText(rect().x() + offset_cur, rect().y(), rect().width() - border.x(), rect().height(),
					   Qt::AlignVCenter | Qt::AlignLeft, m_message, &border);
...
void LabelVersion::showMessage(const std::string &str)
{
	...
	if ((m_status != SS_None) && (m_message == afqt::stoq(str)))
		return;
	phase = 0;
...
{
	if (str.find("AFANASY") == 0)
		return SS_Info;
	if (str.find("ALARM") == 0)
		return SS_Alarm;
	if (str.find("ACHTUNG") == 0)
		return SS_Alarm;
	return SS_None;
}

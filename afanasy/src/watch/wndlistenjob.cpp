...
WndListenJob::WndListenJob(int i_job_id, const QString &i_name)
	: WndText("Listen Job"), m_job_id(i_job_id), m_name(i_name)
{
...
WndListenJob::~WndListenJob() { Watch::listenJob_rem(m_job_id); }

void WndListenJob::closeEvent(QCloseEvent *event) { subscribe(false); }
...
void WndListenJob::v_connectionLost() { close(); }

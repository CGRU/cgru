...
ItemJobTask::ItemJobTask(ListTasks *i_list, ItemJobBlock *i_block, int i_numtask,
						 const af::BlockData *i_bdata)
	: Item(afqt::stoq(i_bdata->genTaskName(i_numtask)), 0, TTask), m_list(i_list), m_job_id(i_block->job_id),
	  m_blocknum(i_bdata->getBlockNum()), m_tasknum(i_numtask), m_block(i_block), m_files_ready(false)
{
...
ItemJobTask::~ItemJobTask() { thumbsCLear(); }
...
const QString ItemJobTask::v_getSelectString() const { return m_name; }

void ItemJobTask::upProgress(const af::TaskProgress &tp) { taskprogress = tp; }

void ItemJobTask::v_paint(QPainter *i_painter, const QRect &i_rect,
						  const QStyleOptionViewItem &i_option) const
{
	...
	QString rightString;
	if (taskprogress.state & AFJOB::STATE_WARNING_MASK)
		rightString += "Warning! ";
	if (taskprogress.state & AFJOB::STATE_PARSERERROR_MASK)
		rightString += "Bad Output! ";
	if (taskprogress.state & AFJOB::STATE_PARSERFATALERROR_MASK)
		rightString += "FATAL ERROR! ";
	if (taskprogress.state & AFJOB::STATE_PARSERBADRESULT_MASK)
		rightString += "Bad Result! ";
	if (taskprogress.state & AFJOB::STATE_PARSERSUCCESS_MASK)
		rightString += "Parser Success. ";
	if (false == taskprogress.hostname.empty())
		rightString += afqt::stoq(taskprogress.hostname);
	if (false == taskprogress.activity.empty())
		rightString += QString(": ") + afqt::stoq(taskprogress.activity);
	...
	if (false == taskprogress.resources.empty())
	{
		...
		for (int i = 0; i < pairs.size(); i++)
		{
			...
			if (res[0] == "mem_peak_mb")
				...
				resources += QString(" %1:%2GB")
								 .arg(res[0].left(res[0].size() - 3))
								 .arg(res[1].toFloat() / 1024.0, 0, 'f', 2);
			else if (res[0].endsWith("_gb"))
				resources +=
					QString(" %1:%2GB").arg(res[0].left(res[0].size() - 3)).arg(res[1].toFloat(), 0, 'f', 2);
		}
		if (resources.size())
			rightString += QString(":") + resources;
	}

	if (taskprogress.state & (AFJOB::STATE_DONE_MASK | AFJOB::STATE_SKIPPED_MASK))
		percent = 100;
	else if (taskprogress.state & (AFJOB::STATE_READY_MASK | AFJOB::STATE_ERROR_MASK))
		percent = 0;
	...
	if (taskprogress.state & AFJOB::STATE_RUNNING_MASK)
	{
		...
		if (frames_num > 1)
		{
			if (m_block->numeric)
				leftString =
					QString("%1 - f%2/%3-%4%").arg(leftString).arg(frames_num).arg(frame).arg(percentframe);
			else
				leftString =
					QString("f%1/%2-%3% %4").arg(frames_num).arg(frame).arg(percentframe).arg(leftString);
		}
	}
	else
		setNotRunning();
	...
	if (taskprogress.state & AFJOB::STATE_RUNNING_MASK)
	{
		...
		drawPercent(i_painter, x, y, w - WidthInfo, Height - 1, 100, percent, 0, 0,
					!(taskprogress.state & AFJOB::STATE_DONE_MASK),
					((taskprogress.state & AFJOB::STATE_WARNING_MASK)
						 ? &afqt::QEnvironment::clr_itemjobwarning.c
						 : NULL));
	}
	else if (taskprogress.state &
			 (AFJOB::STATE_ERROR_MASK | AFJOB::STATE_WAITRECONNECT_MASK | AFJOB::STATE_WAITDEP_MASK |
			  AFJOB::STATE_SKIPPED_MASK | AFJOB::STATE_DONE_MASK | AFJOB::STATE_ERROR_READY_MASK |
			  AFJOB::STATE_TRYTHISTASKNEXT_MASK | AFJOB::STATE_SUSPENDED_MASK))
	{
		i_painter->setPen(Qt::NoPen);
	...
	if ((taskprogress.state & AFJOB::STATE_RUNNING_MASK && (taskprogress.percent > 0) &&
		 (taskprogress.percent < 100)))
		i_painter->drawText(x + 1, y + 1, text_x - 10, Height, Qt::AlignVCenter | Qt::AlignRight,
							QString("%1%").arg(taskprogress.percent));
	text_x -= ItemJobBlock::WPct;

	i_painter->drawText(x + 1, y + 1, text_x - 10, Height, Qt::AlignVCenter | Qt::AlignRight,
						QString("e%1").arg(taskprogress.errors_count));
	...
	i_painter->drawText(x + 1, y + 1, text_x - 10, Height, Qt::AlignVCenter | Qt::AlignRight,
						QString("s%1").arg(taskprogress.starts_count));
	if (false == rightString.isEmpty())
	{
		text_x -= ItemJobBlock::WStarts;
		QRect rect;
		i_painter->drawText(x + 1, y + 1, text_x - 10, Height, Qt::AlignVCenter | Qt::AlignRight, rightString,
							&rect);
		text_x -= rect.width();
	}
	...
	for (int i = 0; i < m_thumbs_imgs.size(); i++)
	{
		...
		i_painter->drawText(x, y + 3, m_thumbs_imgs[i]->width(), 16, Qt::AlignRight | Qt::AlignTop,
							m_thumbs_names[i]);
...
bool ItemJobTask::compare(int type, const ItemJobTask &other, bool ascending) const
{
	...
	switch (type)
	{
		case ItemJobBlock::SPct:
			if ((taskprogress.percent > other.taskprogress.percent) == ascending)
				result = true;
			break;
		case ItemJobBlock::SErrors:
			if ((taskprogress.errors_count > other.taskprogress.errors_count) == ascending)
				result = true;
			break;
		case ItemJobBlock::SHost:
			if ((taskprogress.hostname > other.taskprogress.hostname) == ascending)
				result = true;
			break;
		case ItemJobBlock::SStarts:
			if ((taskprogress.starts_count > other.taskprogress.starts_count) == ascending)
				result = true;
			break;
		case ItemJobBlock::SState:
			if ((taskprogress.state > other.taskprogress.state) == ascending)
				result = true;
			break;
		case ItemJobBlock::STime:
			if ((taskprogress.time_done - taskprogress.time_start >
				 other.taskprogress.time_done - other.taskprogress.time_start) == ascending)
				result = true;
			break;
		default: AFERROR("ItemJobTask::compare: Invalid sort type.\n");
	}
	return result;
}
...
void ItemJobTask::taskFilesReceived(const af::MCTaskUp &i_taskup)
{
	...
	for (int i = 0; i < i_taskup.getFilesNum(); i++)
	{
		...
		if (false ==
			img->loadFromData((const unsigned char *)i_taskup.getFileData(i), i_taskup.getFileSize(i)))
		{
			AF_ERR << "Can't constuct an image " << i_taskup.getFileName(i)
				   << ", size = " << i_taskup.getFileSize(i);

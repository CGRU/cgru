...
ListTasks::ListTasks(QWidget *parent, int JobId, const QString &JobName)
	: ListItems(parent, "tasks"), m_job_id(JobId), m_job_name(JobName), constructed(false)
{
...
void ListTasks::generateMenu(QMenu &o_menu, Item *i_item)
{
	...
	switch (i_item->getType())
	{
				...
				if (preview_cmds.size())
				{
					...
					for (int i = 0; i < files.size(); i++)
					{
						...
						for (int p = 0; p < preview_cmds.size(); p++)
						{
							...
							ActionIdId *actionid =
								new ActionIdId(p, i, QString("    " + cmdSplit.first()), this);
							connect(actionid, SIGNAL(triggeredId(int, int)), this,
									SLOT(actBlockPreview(int, int)));
		...
		case Item::TTask:
		{
			...
			if (itemTask->taskprogress.state &
	...
	switch (msg->type())
	{
		...
		case af::Msg::TBlocksProgress:
		{
			...
			for (int b = 0; b < count; b++)
			{
				...
				{
					AF_ERR << "ListTasks::v_caseMessage: block->getJobId() != m_job_id: " << block->getJobId()
						   << "!=" << m_job_id;
					continue;
				}
				...
				if (blocknum >= m_blocks.size())
				{
					...
					{
						AF_ERR << "ListTasks::v_caseMessage: blocknum >= m_blocks.size(): " << blocknum
							   << ">=" << m_blocks.size();
					}
				}
				else
...
int ListTasks::getRow(int i_block, int i_task)
{
	...
	if (i_block < m_blocks.size())
	{
		...
		{
			AF_ERR << "ListTasks::getRow: i_task >= m_tasks[i_block].size(): " << i_task
				   << ">=" << m_tasks[i_block].size();
		}
		else
		{
			row = 1;
		...
		AF_ERR << "ListTasks::getRow: i_block >= m_blocks.size(): " << i_block << ">=" << m_blocks.size();
...
bool ListTasks::updateProgress(const af::JobProgress *i_job_progress /*bool blocksOnly = false*/)
{
	...
	{
		AF_ERR << "ListTasks::updateProgress: Blocks number mismatch: " << m_blocks.size()
			   << "!=" << i_job_progress->getBlocksNum();
		return false;
	}
	...
	for (int b = 0; b < m_blocks.size(); b++)
	{
		...
		{
			AF_ERR << "ListTasks::updateProgress: Tasks number mismatch in block #" << b << ": "
				   << m_tasks[b].size() << "!=" << i_job_progress->getTasksNum(b);
			return false;
		}
...
bool ListTasks::updateTasks(const std::vector<int32_t> &i_blocks, const std::vector<int32_t> &i_tasks,
							const std::vector<af::TaskProgress> &i_tps)
{
	if ((i_tps.size() != i_blocks.size()) && (i_tps.size() != i_tasks.size()))
	{
		AF_ERR << "ListTasks::updateTasks: input sizes mismatch: " << int(i_tasks.size()) << ", "
			   << int(i_blocks.size()) << ", " << int(i_tps.size());
		return false;
	}
	...
	for (int i = 0; i < i_tps.size(); i++)
	{
		...
		{
			AF_ERR << "ListTasks::updateTasks: block >= m_blocks.size(): " << i_blocks[i]
				   << ">=" << m_blocks.size();
			return false;
		}
		if (i_tasks[i] >= m_tasks[i_blocks[i]].size())
		{
			AF_ERR << "ListTasks::updateTasks: i_tasks[i] >= m_tasks[i_blocks[i]].size():" << i_tasks[i]
				   << ">=" << m_tasks[i_blocks[i]].size();
			return false;
		}
...
void ListTasks::actBlockWorkingDir()
{
	...
	QString str = QInputDialog::getText(this, "Change Working Directory", "Enter Directory",
...
{
	bool ok;
	QString str = QInputDialog::getText(this, "Set Environment", "Enter name=value pair:", QLineEdit::Normal,
										QString(), &ok);
	if (!ok)
		return;
	str = afqt::stoq(af::strEscape(afqt::qtos(str)));
...
void ListTasks::actBlockCmdPost()
{
	...
	QString str =
		QInputDialog::getText(this, "Change Post Command", "Enter Command", QLineEdit::Normal, cur, &ok);
	if (!ok)
		return;
	str = afqt::stoq(af::strEscape(afqt::qtos(str)));
...
void ListTasks::sortBlock(int i_block_num)
{
	...
	{
		AF_ERR << "ListTasks::sortBlock: i_block_num >= m_blocks.size(): " << i_block_num
			   << ">=" << m_blocks.size();
		return;
	}
...
bool ListTasks::v_filesReceived(const af::MCTaskUp &i_taskup)
{
	...
	{
		AF_ERR << "ListTasks::taskFilesReceived: i_taskup.getNumTask() >= "
				  "m_tasks[i_taskup.getNumBlock()].size():"

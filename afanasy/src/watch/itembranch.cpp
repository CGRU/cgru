...
ItemBranch::ItemBranch(ListWork *i_list_work, af::Branch *i_branch, const CtrlSortFilter *i_ctrl_sf)
	: ItemWork(i_list_work, i_branch, TBranch, i_ctrl_sf), m_paused(false), m_empty(false)
{
	// Add buttons:
...
void ItemBranch::v_updateValues(af::Node *i_afnode, int i_msgType)
{
	...
	if (Watch::isPadawan())
	{
		// Counts:
		if (branches_total)
			strCounts += QString(" Branches:%1").arg(branches_total);
		if (jobs_total)
			strCounts += QString(" Jobs:%1").arg(jobs_total);
		if (jobs_running)
			strCounts += QString(" Running:%1").arg(jobs_running);
		if (jobs_ready)
			strCounts += QString(" Ready:%1").arg(jobs_ready);
		if (jobs_done)
			strCounts += QString(" Done:%1").arg(jobs_done);
		if (jobs_error)
			strCounts += QString(" Error:%1").arg(jobs_error);
		if (tasks_ready)
			strCounts += QString(" TReady:%1").arg(tasks_ready);
		if (tasks_error)
			strCounts += QString(" TError:%1").arg(tasks_error);
		...
		// Parameters:
		if (branch->isCreateChilds())
			strParameters += " AutoCreateChilds";
		strParameters += " Solving:";
		if (branch->isSolveJobs())
			strParameters += "Jobs:";
		else
			strParameters += "Users:";
		if (branch->isSolvePriority())
			strParameters += "Priority";
		else
			strParameters += "Ordered";
		if (branch->isSolveCapacity())
			strParameters += ",Capacity";
		else
			strParameters += ",RunTasks";
		if (max_running_tasks != -1)
			strParameters += QString(" MaxRuningTasks:%1").arg(max_running_tasks);
		if (max_running_tasks_per_host != -1)
			strParameters += QString(" MaxRunTasksPerHost:%1").arg(max_running_tasks_per_host);
	...
	else if (Watch::isJedi())
	{
		...
		if (jobs_total)
			strCounts += QString(" Jobs:%1").arg(jobs_total);
		if (jobs_running)
			strCounts += QString(" Run:%1").arg(jobs_running);
		if (jobs_ready)
			strCounts += QString(" Rdy:%1").arg(jobs_ready);
		if (jobs_done)
			strCounts += QString(" Done:%1").arg(jobs_done);
		if (jobs_error)
			strCounts += QString(" Err:%1").arg(jobs_error);
		if (tasks_ready)
			strCounts += QString(" TRdy:%1").arg(tasks_ready);
		if (tasks_error)
			strCounts += QString(" TErr:%1").arg(tasks_error);
		...
		if (branch->isSolveCapacity())
			...
			strParameters += ",MaxTasks";
		if (max_running_tasks != -1)
			strParameters += QString(" MaxTasks:%1").arg(max_running_tasks);
		if (max_running_tasks_per_host != -1)
			strParameters += QString(" MaxPerHost:%1").arg(max_running_tasks_per_host);
		if (false == hostsmask.isEmpty())
		{
	...
	else
	{
		// Counts:
		if (branches_total)
			strCounts += QString(" b:%1").arg(branches_total);
		if (jobs_total)
			strCounts += QString(" j:%1").arg(jobs_total);
		if (jobs_running)
			strCounts += QString(" R:%1").arg(jobs_running);
		if (jobs_ready)
			strCounts += QString(" r:%1").arg(jobs_ready);
		if (jobs_done)
			strCounts += QString(" d:%1").arg(jobs_done);
		if (jobs_error)
			strCounts += QString(" E:%1").arg(jobs_error);
		if (tasks_ready)
			strCounts += QString(" T:%1").arg(tasks_ready);
		if (tasks_error)
			strCounts += QString(" e:%1").arg(tasks_error);
		...
		if (branch->isSolveCapacity())
			...
			strParameters += ",mt";
		if (max_running_tasks != -1)
			strParameters += QString(" m%1").arg(max_running_tasks);
		if (max_running_tasks_per_host != -1)
			strParameters += QString(" mph%1").arg(max_running_tasks_per_host);
		if (false == hostsmask.isEmpty())
		{
	...
	if (isLocked())
		strName = "(LOCKED) " + strName;

	if (m_paused)
		strParameters += " PAUSED";
...
void ItemBranch::updateInfo(af::Branch *i_branch)
{
	...
	if (m_empty)
	{
		...
		m_info_text += QString("<br>Empty for: <b>%1</b>")
						   .arg(afqt::stoq(af::time2strHMS(time(NULL) - i_branch->getTimeEmpty())));
	}
...
void ItemBranch::v_paint(QPainter *i_painter, const QRect &i_rect, const QStyleOptionViewItem &i_option) const
{
	...
	QRect rect_name;
	i_painter->drawText(x + m_buttons_width, y, w - m_buttons_width, h, Qt::AlignLeft | Qt::AlignTop, strName,
						&rect_name);
	...
	if (false == m_empty)
	{
		i_painter->drawText(x + m_buttons_width + rect_name.width(), y + 1,
							w - rect_name.width() - m_buttons_width, height_branch,
...
void ItemBranch::v_setSortType(int i_type1, int i_type2)
{
	...
	switch (i_type2)
	{
		...
		case CtrlSortFilter::TNUMJOBS: m_sort_int2 = jobs_total; break;
...
void ItemBranch::v_setFilterType(int i_type)
{
	...
	switch (i_type)
	{
		...
		case CtrlSortFilter::TUSERNAME: break;

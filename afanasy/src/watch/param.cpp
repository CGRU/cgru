...
Param::Param(EType i_type, Item::EType i_itemtype, const QString &i_name, const QString &i_label,
			 const QString &i_tip, int i_min, int i_max)
	: type(i_type), itemtype(i_itemtype), name(i_name), label(i_label), tip(i_tip), m_min(i_min), m_max(i_max)
{
}
...
bool Param::getInputDialog(const QVariant &i_var, QString &o_str, QWidget *i_parent) const
{
	...
	switch (type)
	{
		...
		default: AF_ERR << "Unknown parameter '" << name.toUtf8().data() << "' type: " << type;
	}

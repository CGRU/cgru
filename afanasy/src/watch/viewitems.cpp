...
ItemDelegate::ItemDelegate(QWidget *parent) : QAbstractItemDelegate(parent) {}
...
void ItemDelegate::emitSizeHintChanged(const QModelIndex &index) { emit sizeHintChanged(index); }

ViewItems::ViewItems(ListItems *parent) : QListView(parent), m_listitems(parent)
{
	setSpacing(2);
...
void ViewItems::loadImage()
{
	...
	else
	{
		...
		{
			if (false == m_back_pixmap.load(afqt::stoq(af::Environment::getAfRoot()) + "/icons/watch/" +
											afqt::QEnvironment::theme.str + "/" + m_back_filename))
				m_back_pixmap = QPixmap();
		}
		...
		if (false == m_back_pixmap.isNull())
		{
			...
			m_back_pixmap =
				m_back_pixmap.transformed(QTransform().rotate(m_back_angle), Qt::SmoothTransformation);
		}
	}
#endif
}
...
void ViewItems::emitSizeHintChanged(const QModelIndex &index) { itemDelegate->emitSizeHintChanged(index); }
...
void ViewItems::repaintViewport() { viewport()->repaint(); }
...
void ViewItems::paintEvent(QPaintEvent *event)
{
	...
	if (m_back_pixmap.isNull() || verticalScrollBar()->isVisible())
	{

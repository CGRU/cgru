...
ListJobs::ListJobs(QWidget *i_parent, bool i_listwork, const std::string &i_name)
	: ListNodes(i_parent, i_name), m_listwork(i_listwork), m_all_blocks_menu_shown(false)
{
	if (af::Environment::VISOR() || m_listwork)
		m_ctrl_sf = new CtrlSortFilter(this, &ms_SortType1_SU, &ms_SortAscending1_SU, &ms_SortType2_SU,
									   &ms_SortAscending2_SU, &ms_FilterType_SU, &ms_FilterInclude_SU,
									   &ms_FilterMatch_SU, &ms_FilterRegEx_SU, &ms_FilterString_SU);
	else
		m_ctrl_sf = new CtrlSortFilter(this, &ms_SortType1, &ms_SortAscending1, &ms_SortType2,
									   &ms_SortAscending2, &ms_FilterType, &ms_FilterInclude, &ms_FilterMatch,
									   &ms_FilterRegEx, &ms_FilterString);
	...
	if (af::Environment::VISOR() || (false == m_listwork))
	{
		...
		bp = addButtonPanel(Item::TJob, "STOP", "jobs_stop", "Stop selected jobs tasks and pause jobs.", "",
							true);
		...
		bp = addButtonPanel(Item::TJob, "ALL TASKS", "jobs_restart",
							"Restart all tasks of all selected jobs.", "", true);
		connect(bp, SIGNAL(sigClicked()), this, SLOT(actRestart()));

		bp = addButtonPanel(Item::TJob, "AND PAUSE", "jobs_restart_pause",
							"Restart tasks and pause selected jobs.", "", true);
		connect(bp, SIGNAL(sigClicked()), this, SLOT(actRestartPause()));

		bp = addButtonPanel(Item::TJob, "WARNINGS", "jobs_restart_warning", "Restart tasks with warnings.",
							"", true);
		connect(bp, SIGNAL(sigClicked()), this, SLOT(actRestartWarnings()));

		bp = addButtonPanel(Item::TJob, "RUNNING", "jobs_restart_running", "Restart runnning tasks.", "",
							true);
		connect(bp, SIGNAL(sigClicked()), this, SLOT(actRestartRunning()));

		bp =
			addButtonPanel(Item::TJob, "SKIPPED", "jobs_restart_skipped", "Restart skipped tasks.", "", true);
		...
		bp = addButtonPanel(Item::TJob, "RETRY TASKS", "jobs_restart_error_tasks", "Restart error tasks.",
							"R");
		...
		bp = addButtonPanel(Item::TAny, "FIND", "users_hosts_mask_find",
							"Use simple find algorithm for hosts mask.");
		connect(bp, SIGNAL(sigClicked()), this, SLOT(actHostsMaskFind()));
		bp = addButtonPanel(Item::TAny, "REGEX", "users_hosts_mask_regex",
		...
		if (m_listwork)
		{
			bp = addButtonPanel(Item::TAny, "DELETE", "work_delete", "Delete selected jobs or branches.", "",
								true);
	...
	addParam_separator();
	addParam_Num(Item::TAny, "max_running_tasks", "Maximum Running", "Maximum running tasks number", -1,
				 1 << 20);
	addParam_Num(Item::TAny, "max_running_tasks_per_host", "Max Run Per Host",
				 "Max run tasks on the same host", -1, 1 << 20);
	addParam_separator();
	addParam_REx(Item::TAny, "hosts_mask", "Hosts Mask", "Host names pattern that job can run on");
	addParam_REx(Item::TAny, "hosts_mask_exclude", "Hosts Mask Exclude",
	...
	addParam_separator();
	addParam_Num(Item::TBranch, "max_tasks_per_second", "Max Tasks Per Second",
				 "Maximum tasks starts per second", -1, 1 << 20);
	addParam_Hrs(Item::TJob, "time_life", "Life Time", "Time to be deleted after creation");
	addParam_separator();
	addParam_Num(Item::TJob, "need_power", "Power Needed", "Job need client that has at least this power", -1,
				 1 << 30);
	addParam_REx(Item::TJob, "need_os", "OS Needed", "Job will run only on this OS");
	addParam_MiB(Item::TJob, "need_memory", "Memory Needed",
				 "Job need client to have at least this free memory (GB)", -1, 1 << 30);
	addParam_GiB(Item::TJob, "need_hdd", "HDD Needed",
				 "Job need client to have at least this free HDD space (GB)", -1, 1 << 30);
	addParam_REx(Item::TJob, "need_properties", "Properties Needed",
				 "Job need client that has such properties");
...
void ListJobs::v_showFunc()
{
	...
	else
	{
		...
		else if (m_parentWindow != (QWidget *)Watch::getDialog())
			close();
	}
}
...
void ListJobs::contextMenuEvent(QContextMenuEvent *event)
{
	...
	if (selectedItemsCount == 1)
		action->setEnabled(jobitem->state & AFJOB::STATE_ERROR_MASK);
	menu.addAction(action);
	...
	if (selectedItemsCount == 1)
		action->setEnabled(jobitem->state & AFJOB::STATE_OFFLINE_MASK);
	submenu->addAction(action);

	action = new QAction("Pause Job", this);
	connect(action, SIGNAL(triggered()), this, SLOT(actPause()));
	if (selectedItemsCount == 1)
		action->setEnabled(false == (jobitem->state & AFJOB::STATE_OFFLINE_MASK));
	submenu->addAction(action);

	action = new QAction("Stop Job", this);
	connect(action, SIGNAL(triggered()), this, SLOT(actStop()));
	if (selectedItemsCount == 1)
		action->setEnabled(jobitem->state & AFJOB::STATE_RUNNING_MASK);
	submenu->addAction(action);

	action = new QAction("Restart Job", this);
	connect(action, SIGNAL(triggered()), this, SLOT(actRestart()));
	if (selectedItemsCount == 1)
		action->setEnabled((jobitem->time_started != 0) || (jobitem->state & AFJOB::STATE_SKIPPED_MASK));
	submenu->addAction(action);
	...
	if (selectedItemsCount == 1)
		action->setEnabled(jobitem->state & AFJOB::STATE_WARNING_MASK);
	submenu->addAction(action);
	...
	if (selectedItemsCount == 1)
		action->setEnabled(jobitem->state & AFJOB::STATE_SKIPPED_MASK);
	submenu->addAction(action);
	...
	switch (msg->type())
	{
		case af::Msg::TJobsList:
		{
			if (updateItems(msg, Item::TJob))
			{
				...
				if ((af::Environment::VISOR() == false) && (false == m_listwork) &&
					(m_ctrl_sf->isSortDisabled()))
					getUserJobsOrder();
			}
...
void ListJobs::getUserJobsOrder()
{
	...
	str << "{\"get\":{\"type\":\"users\",\"ids\":[" << MonitorHost::getUid()
		<< "],\"mode\":\"jobs_order\",\"binary\":true}}";
...
void ListJobs::calcTotals()
{
	...
	for (int i = 0; i < numjobs; i++)
	{
		...
		if (itemjob->state & AFJOB::STATE_RUNNING_MASK)
			running++;
		if (itemjob->state & AFJOB::STATE_ERROR_MASK)
			error++;
	}
...
void ListJobs::actSetUser()
{
	...
	bool ok;
	QString text =
		QInputDialog::getText(this, "Change Owner", "Enter New User Name", QLineEdit::Normal, current, &ok);
	if (!ok)
		return;
...
void ListJobs::actPostCommand()
{
	...
	bool ok;
	QString cmd =
		QInputDialog::getText(this, "Change Post Command", "Enter Command", QLineEdit::Normal, current, &ok);
	if (!ok)
		return;
...
void ListJobs::actListenJob()
{
	...
	if (Watch::isConnected())
		Watch::listenJob(item->getId(), item->getName());
}

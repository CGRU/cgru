...
ButtonDblClick::ButtonDblClick(const QString &i_label, QWidget *i_parent)
	: QWidget(i_parent), m_label(i_label), m_text(i_label), m_enabled(true), m_hovered(false),
	  m_clicked(false), m_activated(false), m_warning(false)
{
	m_width = 96;
	m_height = 24;
...
void ButtonDblClick::enterEvent(QEvent *i_evt)
{
	if (false == m_enabled)
		return;
	m_hovered = true;
...
void ButtonDblClick::leaveEvent(QEvent *i_evt)
{
	if (false == m_enabled)
		return;
	m_hovered = false;

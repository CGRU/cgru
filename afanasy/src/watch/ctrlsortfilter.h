...
class CtrlSortFilter : public QFrame
{
	Q_OBJECT
  public:
	CtrlSortFilter(ListItems *i_parent, int *i_sorttype1, bool *i_sortascending1, int *i_sorttype2,
				   bool *i_sortascending2, int *i_filtertype, bool *i_filterinclude, bool *i_filtermatch,
				   bool *i_filterregex, std::string *i_filterstring);
	~CtrlSortFilter();

	inline void addSortType(int i_type)
	{
		if (i_type < TLAST)
			m_sort_types.push_back(i_type);
	}
	inline void addFilterType(int i_type)
	{
		if (i_type < TLAST)
			m_filter_types.push_back(i_type);
	}

...
Item::Item(const QString &i_name, int i_id, EType i_type)
	: m_name(i_name), m_height(Height), m_margin_left(0), m_locked(false), m_running(false), m_id(i_id),
	  m_type(i_type), m_hidden(false), m_depth(0)
{
}
...
QSize Item::sizeHint(const QStyleOptionViewItem &option) const { return QSize(Width, m_height); }
...
bool Item::v_isSelectable() const { return true; }
...
{
	if (on)
		return (option.state & QStyle::State_Selected) ? afqt::QEnvironment::clr_textmuted.c
													   : afqt::QEnvironment::clr_textbright.c;
	else
		return (option.state & QStyle::State_Selected) ? afqt::QEnvironment::clr_textbright.c
													   : afqt::QEnvironment::clr_textmuted.c;
}
...
void Item::printfState(const uint32_t state, int posx, int posy, QPainter *painter,
...
void Item::drawPercent(QPainter *painter, int posx, int posy, int width, int height, int whole, int part_a,
					   int part_b, int part_c, bool drawBorder, QColor *i_done_color)
{
	int x = posx;
	int y = posy;
	int h = height;
	...
	if (part_a)
	{
		...
		painter->setBrush(
			QBrush(i_done_color ? *i_done_color : afqt::QEnvironment::clr_done.c, Qt::SolidPattern));
...
int Item::drawTicket(QPainter *i_painter, const QPen &i_text_pen, int i_x, int i_y, int i_w, int i_h,
					 int i_opts, const QString &i_name, int i_count, int i_usage, int i_hosts,
					 int i_max_hosts)
{
	...
	if (i_usage > 0)
		text += QString(":%1").arg(i_usage);
	if (i_hosts > 0)
		text += QString("/%1").arg(i_hosts);
	if (i_max_hosts >= 0)
		text += QString("/%1").arg(i_max_hosts);
	...
	else
	{
		...
		{
			i_painter->drawText(i_x, i_y, i_w - tk_width, 15, Qt::AlignRight | Qt::AlignTop, i_name,
								&tk_rect);
			tk_width += tk_rect.width();
		}

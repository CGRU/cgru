...
WndText::WndText(const QString &i_name) : Wnd(i_name) { construct(); }

WndText::WndText(const QString &i_name, af::Msg *i_msg) : Wnd(i_name)
{
	construct();
	caseMsg(i_msg);
}

WndText::WndText(const QString &i_name, const std::string &i_str) : Wnd(i_name)
{
	construct();
	insertText(i_str);
}

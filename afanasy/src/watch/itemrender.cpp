...
ItemRender::ItemRender(ListRenders *i_list_renders, af::Render *i_render, const CtrlSortFilter *i_ctrl_sf)
	: ItemFarm(i_list_renders, i_render, TRender, i_ctrl_sf), m_online(false), m_taskstartfinishtime(0),
	  m_plotCpu(2), m_plotMem(2), m_plotSwp(1), m_plotHDD(1), m_plotNet(2, -1), m_plotIO(2, -1),
	  m_plot_GPU_gpu(1), m_plot_GPU_mem(1), m_update_counter(0)
{
...
bool ItemRender::calcHeight()
{
	...
	switch (afqt::QEnvironment::render_item_size.n)
	{
		...
		case ListRenders::ENormalSize: m_height = m_plots_height; break;
...
void ItemRender::v_updateValues(af::Node *i_afnode, int i_msgType)
{
	...
	switch (i_msgType)
	{
		...
		case af::Msg::TRendersResources:
		{
			...
			QString cpu_label =
				QString("C %1*%2").arg(m_hres.cpu_num).arg(double(m_hres.cpu_mhz) / 1000.0, 0, 'f', 1);
			if ((m_hres.cpu_temp > 0) && notVirtual())
				cpu_label = QString("%1 %2C").arg(cpu_label).arg(m_hres.cpu_temp);
			m_plotCpu.setLabel(cpu_label);
			m_plotCpu.addValue(0,
							   m_hres.cpu_system + m_hres.cpu_iowait + m_hres.cpu_irq + m_hres.cpu_softirq);
			...
			int plotIO_r_r =
				m_plotIO_rn_r * (100 - m_hres.hdd_busy) / 100 + m_plotIO_rh_r * m_hres.hdd_busy / 100;
			int plotIO_r_g =
				m_plotIO_rn_g * (100 - m_hres.hdd_busy) / 100 + m_plotIO_rh_g * m_hres.hdd_busy / 100;
			int plotIO_r_b =
				m_plotIO_rn_b * (100 - m_hres.hdd_busy) / 100 + m_plotIO_rh_b * m_hres.hdd_busy / 100;
			int plotIO_w_r =
				m_plotIO_wn_r * (100 - m_hres.hdd_busy) / 100 + m_plotIO_wh_r * m_hres.hdd_busy / 100;
			int plotIO_w_g =
				m_plotIO_wn_g * (100 - m_hres.hdd_busy) / 100 + m_plotIO_wh_g * m_hres.hdd_busy / 100;
			int plotIO_w_b =
				m_plotIO_wn_b * (100 - m_hres.hdd_busy) / 100 + m_plotIO_wh_b * m_hres.hdd_busy / 100;
			m_plotIO_rn_g = 200;
			m_plotIO_rn_b = 20;
			...
			int plot_GPU_red =
				(m_hres.gpu_gpu_temp - plot_GPU_red_min) * 100 / (plot_GPU_red_max - plot_GPU_red_min);
			if (plot_GPU_red < 0)
				plot_GPU_red = 0;
			if (plot_GPU_red > 100)
				plot_GPU_red = 100;
			...
			for (unsigned i = 0; i < m_plots.size(); i++)
			{
				...
				m_plots[i]->setColor(m_hres.custom[i]->graphr, m_hres.custom[i]->graphg,
									 m_hres.custom[i]->graphb);
				m_plots[i]->setLabelColor(m_hres.custom[i]->labelr, m_hres.custom[i]->labelg,
										  m_hres.custom[i]->labelb);
				...
				m_plots[i]->setBGColor(m_hres.custom[i]->bgcolorr, m_hres.custom[i]->bgcolorg,
									   m_hres.custom[i]->bgcolorb);
			}
			...
			m_info_text_hres += QString("<br>CPU: <b>%1</b>x<b>%2</b> MHz <b>%3</b>C")
									.arg(m_hres.cpu_mhz)
									.arg(m_hres.cpu_num)
									.arg(m_hres.cpu_temp);
			...
			if (m_hres.gpu_string.size())
				m_info_text_hres += QString("<br>GPU: <b>%1</b> Mem: <b>%2</b> GB")
										.arg(afqt::stoq(m_hres.gpu_string))
										.arg(m_hres.gpu_mem_total_mb / 1000);
	...
	if (m_wolWaking)
		m_offlineState = "Waking Up";
	else if (m_wolSleeping || m_wolFalling)
		m_offlineState = "Sleeping";
	else
		m_offlineState = "Offline";
...
{
	const int x = i_rect.x();
	const int y = i_rect.y();
	const int w = i_rect.width();
	const int h = i_rect.height();
	...
	if (m_online == false)
		itemColor = &(afqt::QEnvironment::clr_itemrenderoff.c);
	else if (m_sick)
		itemColor = &(afqt::QEnvironment::clr_itemrendersick.c);
	else if (m_paused)
		itemColor = &(afqt::QEnvironment::clr_itemrenderpaused.c);
	else if (m_NIMBY)
		itemColor = &(afqt::QEnvironment::clr_itemrenderNIMBY.c);
	else if (m_nimby)
		itemColor = &(afqt::QEnvironment::clr_itemrendernimby.c);
	else if (m_busy)
		itemColor = &(afqt::QEnvironment::clr_itemrenderbusy.c);
	...
	if (m_wol_operation_time > 0)
		offlineState_time =
			m_offlineState + " " + afqt::stoq(af::time2strHMS(time(NULL) - m_wol_operation_time));
	...
	if (false == m_online)
	{
		...
		i_painter->drawText(x + 5, y_cur, w - 10 - rect.width() - 10, HeightOffline,
							Qt::AlignVCenter | Qt::AlignRight, 'v' + m_engine);

		i_painter->setFont(afqt::QEnvironment::f_info);
		QRect rect_center;
		i_painter->drawText(x + 5, y_cur, w - 10, HeightOffline, Qt::AlignVCenter | Qt::AlignHCenter,
							offlineState_time, &rect_center);
		i_painter->drawText(x + 5, y_cur, (w >> 1) - 10 - (rect_center.width() >> 1), HeightOffline,
							Qt::AlignVCenter | Qt::AlignLeft, m_name, &rect);

		i_painter->setFont(afqt::QEnvironment::f_thin);
		i_painter->drawText(x + 5 + rect.width() + 10, y_cur,
							(w >> 1) - 10 - (rect_center.width() >> 1) - rect.width() - 10, HeightOffline,
		...
		if (m_annotation.size())
		{
			i_painter->setPen(afqt::QEnvironment::qclr_black);
			i_painter->setFont(afqt::QEnvironment::f_info);
			i_painter->drawText(x + 5, y_cur, w - 10, HeightAnnotation - 2,
								Qt::AlignVCenter | Qt::AlignHCenter, m_annotation);
		}
	...
	switch (afqt::QEnvironment::render_item_size.n)
	{
		case ListRenders::ESmallSize:
			...
			i_painter->setFont(afqt::QEnvironment::f_info);
			i_painter->drawText(left_text_x, y_cur, left_text_w, h, Qt::AlignTop | Qt::AlignLeft, m_name,
								&rect);
			i_painter->setFont(afqt::QEnvironment::f_thin);
			i_painter->drawText(left_text_x + rect.width() + 5, y_cur - 1, left_text_w, h,
								Qt::AlignTop | Qt::AlignLeft,
								m_capacity_usage + ' ' + m_loggedin_users + ' ' + m_hw_info);

			i_painter->setPen(clrTextInfo(i_option));
			i_painter->setFont(afqt::QEnvironment::f_info);
			i_painter->drawText(right_text_x, y_cur, right_text_w, h, Qt::AlignTop | Qt::AlignRight,
								ann_state, &rect);

			i_painter->setFont(afqt::QEnvironment::f_thin);
			i_painter->drawText(right_text_x, y_cur + 1, right_text_w - rect.width() - 5, h,
								Qt::AlignTop | Qt::AlignRight, 'v' + m_engine);
		...
		default:
			...
			i_painter->setFont(afqt::QEnvironment::f_thin);
			i_painter->drawText(left_text_x + rect.width() + 5, y_cur, left_text_w, h,
								Qt::AlignTop | Qt::AlignLeft, m_hw_info);

			i_painter->setPen(afqt::QEnvironment::qclr_black);
			i_painter->setFont(afqt::QEnvironment::f_info);
			i_painter->drawText(right_text_x, y_cur + 1, right_text_w, h, Qt::AlignTop | Qt::AlignRight,
								ann_state, &rect);

			i_painter->setFont(afqt::QEnvironment::f_thin);
			i_painter->drawText(right_text_x, y_cur, right_text_w - rect.width() - 5, h,
								Qt::AlignTop | Qt::AlignRight, 'v' + m_engine);

			i_painter->setPen(clrTextInfo(i_option));
			i_painter->setFont(afqt::QEnvironment::f_info);
			i_painter->drawText(left_text_x, y_cur, left_text_w, base_height + 2,
								Qt::AlignBottom | Qt::AlignLeft, m_capacity_usage + ' ' + m_loggedin_users);
	}
	...
	switch (afqt::QEnvironment::render_item_size.n)
	{
		case ListRenders::ESmallSize: break;
		case ListRenders::ENormalSize:
			if (m_annotation.isEmpty() && m_tasks_users_counts.isEmpty())
				break;
			i_painter->drawText(right_text_x, y, right_text_w, base_height + 2,
								Qt::AlignBottom | Qt::AlignRight, m_annotation + ' ' + m_tasks_users_counts);
			break;
		default:
			i_painter->drawText(right_text_x, y, right_text_w, base_height + 2,
								Qt::AlignBottom | Qt::AlignRight, m_props_state);
	}
	...
	switch (afqt::QEnvironment::render_item_size.n)
	{
		...
		case ListRenders::EBigSize:
		{
			i_painter->drawText(x + 5, y_cur + 4, w - 10, HeightAnnotation, Qt::AlignBottom | Qt::AlignLeft,
								m_tasks_users_counts);
			...
			if (m_annotation.size())
			{
				i_painter->setPen(afqt::QEnvironment::qclr_black);
				i_painter->setFont(afqt::QEnvironment::f_info);
				i_painter->drawText(x + 5, y_cur, w - 10, HeightAnnotation,
									Qt::AlignVCenter | Qt::AlignHCenter, m_annotation);
			}
		...
		default:
		{
			...
			for (int numtask = 0; it != m_tasks.end(); it++, numtask++)
			{
				...
				drawTask(i_painter, i_option, *it, task_percent, x + 5, y_cur, w - tkhost_width - 10,
						 HeightTask - 2);
...
void ItemRender::drawTask(QPainter *i_painter, const QStyleOptionViewItem &i_option,
						  const af::TaskExec *i_exec, int i_percent, int i_x, int i_y, int i_w, int i_h) const
{
	...
	taskstr += QString(": %1[%2][%3]")
				   .arg(QString::fromUtf8(i_exec->getJobName().c_str()))
	...
	for (auto const &tIt : i_exec->getTickets())
	{
		tw += Item::drawTicket(i_painter, pen, i_x + 5 + tw, i_y + 1, i_w - 5 - tw, Item::HeightTickets - 5,
							   Item::TKD_LEFT, afqt::stoq(tIt.first), tIt.second);
	...
	QRect rect_usertime;
	i_painter->drawText(i_x + 5, i_y, i_w - 10, i_h, Qt::AlignVCenter | Qt::AlignRight, user_time,
						&rect_usertime);
...
void ItemRender::v_setSortType(int i_type1, int i_type2)
{
	...
	switch (i_type1)
	{
		...
		case CtrlSortFilter::TCAPACITY: m_sort_int1 = m_capacity; break;
		case CtrlSortFilter::TELDERTASKTIME: m_sort_int1 = m_elder_task_time; break;
		case CtrlSortFilter::TTIMELAUNCHED: m_sort_int1 = m_time_launched; break;
		case CtrlSortFilter::TTIMEREGISTERED: m_sort_int1 = m_time_registered; break;
		case CtrlSortFilter::TNAME: m_sort_str1 = m_name; break;
		case CtrlSortFilter::TUSERNAME: m_sort_str1 = m_username; break;
		case CtrlSortFilter::TTASKUSER: m_sort_str1 = m_tasksusers; break;
		case CtrlSortFilter::TENGINE: m_sort_str1 = m_engine; break;
		case CtrlSortFilter::THWINFO: m_sort_str1 = m_hw_info; break;
		case CtrlSortFilter::TADDRESS: m_sort_str1 = m_address_str; break;
	...
	switch (i_type2)
	{
		...
		case CtrlSortFilter::TCAPACITY: m_sort_int2 = m_capacity; break;
		case CtrlSortFilter::TELDERTASKTIME: m_sort_int2 = m_elder_task_time; break;
		case CtrlSortFilter::TTIMELAUNCHED: m_sort_int2 = m_time_launched; break;
		case CtrlSortFilter::TTIMEREGISTERED: m_sort_int2 = m_time_registered; break;
		case CtrlSortFilter::TNAME: m_sort_str2 = m_name; break;
		case CtrlSortFilter::TUSERNAME: m_sort_str2 = m_username; break;
		case CtrlSortFilter::TTASKUSER: m_sort_str2 = m_tasksusers; break;
		case CtrlSortFilter::TENGINE: m_sort_str2 = m_engine; break;
		case CtrlSortFilter::THWINFO: m_sort_str2 = m_hw_info; break;
		case CtrlSortFilter::TADDRESS: m_sort_str2 = m_address_str; break;
...
void ItemRender::v_setFilterType(int i_type)
{
	...
	switch (i_type)
	{
		...
		case CtrlSortFilter::TUSERNAME:
			...
			break;
		case CtrlSortFilter::TTASKUSER: m_filter_str = afqt::qtos(m_tasksusers); break;
		case CtrlSortFilter::TENGINE: m_filter_str = afqt::qtos(m_engine); break;
		case CtrlSortFilter::THWINFO: m_filter_str = afqt::qtos(m_hw_info); break;
		case CtrlSortFilter::TADDRESS: m_filter_str = afqt::qtos(m_address_str); break;

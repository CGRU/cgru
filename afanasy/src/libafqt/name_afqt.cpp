...
const QHostAddress afqt::toQAddress(const af::Address &address)
{
	...
	switch (address.getFamily())
	{
		...
		case af::Address::IPv6: qaddr.setAddress((quint8 *)(address.getAddrData())); break;
		default: AFERROR("Address::setQAddress: Unknown address family.\n");
	}
	return qaddr;
}

const QString afqt::stoq(const std::string &str) { return QString::fromUtf8(str.data(), str.size()); }
...
const std::string afqt::qtos(const QString &str) { return std::string(str.toUtf8().data()); }
...
const QString afqt::time2Qstr(time_t time_sec) { return stoq(af::time2str(time_sec)); }

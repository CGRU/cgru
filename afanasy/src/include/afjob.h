...
const int TASK_PROGRESS_CHANGE_TIMEOUT =
	-1; ///< If task progress did not change within this time, consider that it is erroneous
...
const int SYSJOB_TASKSNAMEMAX =
	32; // Maximum number of characters in system task name (it is constructed from command)
...
const int64_t ASCII_PROGRESS_MASK = STATE_DONE_MASK | STATE_SKIPPED_MASK | STATE_WAITDEP_MASK |
									STATE_WARNING_MASK | STATE_READY_MASK | STATE_TRYTHISTASKNEXT_MASK |
									STATE_RUNNING_MASK | STATE_ERROR_READY_MASK | STATE_ERROR_MASK |
									STATE_WAITRECONNECT_MASK | STATE_SUSPENDED_MASK;

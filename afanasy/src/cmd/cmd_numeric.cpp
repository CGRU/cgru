...
CmdNumericCmd::CmdNumericCmd()
{
	...
	setHelp("numcmd [service] [frame_start] [frame_end] [command] Fill service numeric command with start "
			"and end frames.");
}
CmdNumericCmd::~CmdNumericCmd() {}
...
CmdNumeric::CmdNumeric()
{
	...
	setHelp("numeric [start] [end] [pertask] [increment(by)] [sequential] [frame to find] Generate numeric "
			"tasks, find task by frame.");
}
CmdNumeric::~CmdNumeric() {}

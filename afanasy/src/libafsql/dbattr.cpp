...
void DBAttr::init()
{
	...
	for (int i = _INTEGER_BEGIN_ + 1; i < _INTEGER_END_; i++)
		DBType[i] = "integer DEFAULT 0";

	for (int i = _BIGINT_BEGIN_ + 1; i < _BIGINT_END_; i++)
		DBType[i] = "bigint DEFAULT 0";
...
const std::string DBAttr::createLine() const { return DBName[type] + " " + DBType[type]; }

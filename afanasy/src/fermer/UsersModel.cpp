...
UsersModel::UsersModel(QObject *i_parent) : QAbstractListModel(i_parent)
{
...
UsersModel *UsersModel::usersModel() { return this; }
...
void UsersModel::updateInteraction(const QString &i_filter)
{
	...
	if (i_filter.size())
	{
		...
		for (int i = 0; i < m_users->size(); i++)
		{
			...
			found_index = (user->user_name().indexOf(i_filter) > found_index)
							  ? user->user_name().indexOf(i_filter)
							  : found_index;
			if (found_index < 0)
			{
...
void UsersModel::setBladeExcludeMask(const QString &i_value)
{
	m_users->setBladeExcludeMask(getSelectedIds(), i_value);
}

QString UsersModel::getUserInfo(const QString &i_value)
{
	return m_users->getInfo(getSelectedIds()[0], i_value);
}
...
QVariant UsersModel::data(const QModelIndex &i_index, int i_role) const
{
	...
	switch (i_role)
	{
		...
		case JobsSizeRole: return QVariant::fromValue(dobj->m_jobs_size);
		case TasksSizeRole: return QVariant::fromValue(dobj->m_tasks_size);
		case PriorityRole: return QVariant::fromValue(dobj->m_priority);
		case UserMachineNameRole: return QVariant::fromValue(dobj->m_user_machine_name);
		case UserMachineIPRole: return QVariant::fromValue(dobj->m_user_machine_ip);

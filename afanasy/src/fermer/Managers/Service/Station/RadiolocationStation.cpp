...
void RadiolocationStation::getItemInfo(std::ostringstream &o_str, const std::string &i_mode,
									   const std::string &i_type, int i_item_id)
{
	o_str.str("");
	o_str << "{\"get\":{";
	o_str << "\"binary\":true";
...
int RadiolocationStation::getAvalibleSlotsAndJobNames(af::Render *i_render, int i_total_slots,
													  QString &o_job_names, QList<int> &o_job_hash)
{
		 ...
		 it != active_task_list.end(); it++)
	{
		...
		if (it_group_jobs != all_group_jobs.end())
			all_group_jobs.insert(std::pair<std::string, std::vector<std::string>>(
				group_name, std::vector<std::string>(1, job_name)));
		else
...
void RadiolocationStation::getUserName(std::string &o_username) { o_username.assign(user_name); }

void RadiolocationStation::getComputerName(std::string &o_compname) { o_compname.assign(comp_name); }
...
void RadiolocationStation::QStringFromMsg(QString &o_ret, Waves::Ptr i_answer)
{
	switch (i_answer.get()->type())
	{
		...
		case af::Msg::TStringList:
		{
			...
			if (strlist.size() == 0)
				strlist.push_back("An empty list received.");
...
RadiolocationStation::RadiolocationStation()
	: m_qafclient(this, af::Environment::getWatchConnectionLostTime()), m_connected(false), user_id(0)
{
	...
	uint32_t env_flags =
		af::Environment::Quiet | af::Environment::SolveServerName; //   Silent environment initialization
...
size_t RadiolocationStation::getId() { return monitor_id; }

size_t RadiolocationStation::getUserId() { return user_id; }
...
bool RadiolocationStation::isConnected() { return m_connected; }
...
bool RadiolocationStation::setParameter(const std::string &i_name_object, const std::vector<int> &i_indeces,
										const std::string &i_param_name, const std::string &i_param_value,
										bool i_quoted)
{
...
bool RadiolocationStation::setOperation(const std::string &i_name_object, const std::vector<int> &i_indeces,

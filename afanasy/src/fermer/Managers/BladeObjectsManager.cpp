...
size_t BladeObjectsManager::size() { return m_lorry->m_objects.size(); }
...
BladeObjectPtrIt BladeObjectsManager::begin() { return m_lorry->m_objects.begin(); }

BladeObjectPtrIt BladeObjectsManager::end() { return m_lorry->m_objects.end(); }
...
bool BladeObjectsManager::setBladeService(int i_index, bool i_enable, const QString &i_name)
{
	return m_RLS->setBladeService(i_index, i_enable, i_name);
}
bool BladeObjectsManager::actLaunchCmd(int i_index, bool i_exit, const QString &i_cmd)
{
	return m_RLS->actLaunchCmd(i_index, i_exit, i_cmd);
}
void BladeObjectsManager::actRequestLog(int i_index, QString &i_val) { m_RLS->actRequestLog(i_index, i_val); }
void BladeObjectsManager::actRequestTasksLog(int i_index, QString &i_val)
{
	m_RLS->actRequestTasksLog(i_index, i_val);
}
void BladeObjectsManager::actRequestInfo(int i_index, QString &i_val)
{
	m_RLS->actRequestInfo(i_index, i_val);
}
void BladeObjectsManager::actCapacity(int i_index, const QString &i_val)
{
	m_RLS->actCapacity(i_index, i_val);
}
void BladeObjectsManager::actMaxTasks(int i_index, const QString &i_val)
{
	m_RLS->actMaxTasks(i_index, i_val);
}

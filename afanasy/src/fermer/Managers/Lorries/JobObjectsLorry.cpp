...
JobObject::Ptr
JobObjectsLorry::insert(const QString &user_name, JobState::State status, const QString &time_creation,
						int blocks_num, const QString &elapsed_time, const QString &blade_mask,
						...
						const QList<QString> &blades, const QString &block_name, int job_id,
						int blades_length, const QString &approx_time, const QString &depends,
						const QString &output_folder, const QString &user_color, int errors_avoid_blades,
						const std::string &json_represent, int elapsed_time_int, const QString &annotation)
{
	...
	{
		ret = (*it);
		(*it)->update(user_name, status, time_creation, blocks_num, elapsed_time, blade_mask,
					  exclude_blade_mask, software, priority, slot, progress, name, id, block_order, blades,
					  block_name, job_id, blades_length, approx_time, depends, user_color,
					  errors_avoid_blades, json_represent, elapsed_time_int, annotation);
	...
	else if (it_cached != m_cache.end())
	{
		ret = (*it_cached);
		(*it_cached)
			->update(user_name, status, time_creation, blocks_num, elapsed_time, blade_mask,
					 exclude_blade_mask, software, priority, slot, progress, name, id, block_order, blades,
					 block_name, job_id, blades_length, approx_time, depends, user_color, errors_avoid_blades,
					 json_represent, elapsed_time_int, annotation);
	...
	{
		JobObject::Ptr b =
			JobObject::create(user_name, status, time_creation, blocks_num, elapsed_time, blade_mask,
							  exclude_blade_mask, software, priority, slot, progress, name, id, block_order,
							  blades, block_name, job_id, blades_length, approx_time, depends, user_color,
							  errors_avoid_blades, json_represent, elapsed_time_int);

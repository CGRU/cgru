...
UserObject::Ptr UserObjectsLorry::insert(const QString &user_name, int jobs_size, int tasks_size,
										 const QString &user_machine_name, const QString &user_machine_ip,
										 int priority, int id)
{
	...
	if (it != m_objects.end())
	{
		(*it)->update(user_name, jobs_size, tasks_size, user_machine_name, user_machine_ip, priority, id);
		ret = (*it);
	}
	else
	{
		UserObject::Ptr b = UserObject::create(user_name, jobs_size, tasks_size, user_machine_name,
											   user_machine_ip, priority, id);

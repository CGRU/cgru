...
BladeObject::Ptr BladeObjectsLorry::insert(
	const QString &name, const QString &ip_address, const QString &base_os, const QString &blades_group,
	const QString &working_time, int loaded_cpu, const QList<int> &loaded_mem, int loaded_net,
	BladeState::State state, int performance_slots, int avalible_performance_slots, size_t max_tasks,
	size_t capacity, int blade_id, const QString &job_names, const QString &user, int hdd_busy)
{
	...
	if (it != m_objects.end())
	{
		(*it)->update(name, ip_address, base_os, blades_group, working_time, loaded_cpu, loaded_mem,
					  loaded_net, state, performance_slots, avalible_performance_slots, max_tasks, capacity,
					  blade_id, job_names, hdd_busy, user);
	...
	{
		BladeObject::Ptr b =
			BladeObject::create(name, ip_address, base_os, blades_group, working_time, loaded_cpu, loaded_mem,
								loaded_net, state, performance_slots, avalible_performance_slots, max_tasks,
								capacity, blade_id, job_names, hdd_busy, user);

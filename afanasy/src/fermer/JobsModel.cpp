...
JobsModel::JobsModel(QObject *i_parent) : QAbstractListModel(i_parent)
{
...
JobsModel *JobsModel::jobsModel() { return this; }

void JobsModel::updateInteraction(const QString &i_filter)
{
	if (m_pass_update == true)
	{
		m_pass_update = false;
		return;
	}
	...
	if (i_filter.size())
	{
		...
		for (int i = 0; i < m_job->size(); i++)
		{
			...
			found_index = (job->block_name().indexOf(i_filter) > found_index)
							  ? job->block_name().indexOf(i_filter)
							  : found_index;
			found_index =
				(job->name().indexOf(i_filter) > found_index) ? job->name().indexOf(i_filter) : found_index;
			found_index = (job->user_name().indexOf(i_filter) > found_index)
							  ? job->user_name().indexOf(i_filter)
							  : found_index;
			if (found_index < 0)
			{
				int found_id = job->id();
...
void JobsModel::passUpdate() { m_pass_update = true; }
void JobsModel::setShowAllJobs(bool val) { m_show_all_jobs = val; }
...
void JobsModel::setBladeMaskExclude(const QString &i_value)
{
	m_job->setExcludeBladeMask(getSelectedIds(), i_value);
}
void JobsModel::setJobDependMask(const QString &i_value) { m_job->setDependMask(getSelectedIds(), i_value); }
void JobsModel::setJobSetAnnotation(const QString &i_value)
{
	m_job->setAnnotation(getSelectedIds(), i_value);
}
...
void JobsModel::setJobPostCommand(const QString &i_value)
{
	m_job->setPostCommand(getSelectedIds(), i_value);
}
...
void JobsModel::setTasksErrorForgiveTime(int i_value)
{
	m_job->setErrorForgiveTime(getSelectedIds(), i_value);
}
...
void JobsModel::setTasksRunningPerBlades(int i_value)
{
	m_job->setMaxRunningTaskPerBlades(getSelectedIds(), i_value);
}
...
void JobsModel::multiSorting(int i_sort_type)
{
	...
	switch (i_sort_type)
	{
		case 0: std::sort(m_job->begin(), m_job->end(), blockCompare); break;
		case 1:
			if (m_state_sort)
			{
		...
		case 9:
			...
			break;
		default: qDebug() << "JobsModel::multiSorting default";
	}
	emit dataChanged(index(0, 0), index(rowCount() - 1, 0));
}
void JobsModel::sortingChangeState() { m_state_sort = (!m_state_sort); }
...
bool JobsModel::isDrawSelection() { return m_draw_selection; }
...
int JobsModel::multiselected() { return m_multiselected_state; }
...
int JobsModel::getNodePosX(int i) { return m_job->at(i)->m_node_posx; }
int JobsModel::getNodePosY(int i) { return m_job->at(i)->m_node_posy; }
...
QVariant JobsModel::data(const QModelIndex &i_index, int i_role) const
{
	...
	switch (i_role)
	{
		case StateRole: return QVariant::fromValue(dobj->m_status);
		case AnnotationRole: return QVariant::fromValue(dobj->m_annotation);
		case ProgressRole: return QVariant::fromValue(dobj->m_progress);
		case ElapsedRole: return QVariant::fromValue(dobj->m_time_elapsed);
		case UserNameRole: return QVariant::fromValue(dobj->m_user_name);
		case JobNameRole: return QVariant::fromValue(dobj->m_name);
		case BlockNameRole: return QVariant::fromValue(dobj->m_block_name);
		case SlotRole: return QVariant::fromValue(dobj->m_slot);
		case IDRole: return QVariant::fromValue(dobj->m_id);
		case PriorityRole: return QVariant::fromValue(dobj->m_priority); // m_priority//m_id
		case CreatedRole: return QVariant::fromValue(dobj->m_time_creation);
		case SoftwareRole: return QVariant::fromValue(dobj->m_software);
		case BlockNumberRole: return QVariant::fromValue(dobj->m_block_order);
		case BladesMaskRole: return QVariant::fromValue(dobj->m_blade_mask);
		case BladesMaskExludeRole: return QVariant::fromValue(dobj->m_exclude_blade_mask);
		case BladesRole: return QVariant::fromValue(dobj->m_blades);
		case GroupSizeRole: return QVariant::fromValue(dobj->m_blocks_num);
		case UserColorRole: return QVariant::fromValue(dobj->m_user_color);
		case GroupIDRole: return QVariant::fromValue(dobj->m_job_id);
		case GroupRandRedValueRole: return QVariant::fromValue(job_rand_red);
		case GroupRandGreenValueRole: return QVariant::fromValue(job_rand_green);
		case GroupRandBlueValueRole: return QVariant::fromValue(job_rand_blue);
		case BladesLenRole: return QVariant::fromValue(dobj->m_blades_length);
		case ApproxTimeRole: return QVariant::fromValue(dobj->m_approx_time);
		case SelectsRole: return QVariant::fromValue(dobj->m_selected);
		case DependsRole: return QVariant::fromValue(dobj->m_depends);
		...
		case GroupNodePoseXRole: return QVariant::fromValue(dobj->m_group_node_posx);
		case GroupNodePoseYRole: return QVariant::fromValue(dobj->m_group_node_posy);
		case GroupNodeWidthXRole: return QVariant::fromValue(dobj->m_group_node_width);
		case GroupNodeHeightXRole: return QVariant::fromValue(dobj->m_group_node_height);
		case ErrorsAvoidBladesRole: return QVariant::fromValue(dobj->m_errors_avoid_blades);
		case ExpandRole: return QVariant::fromValue(dobj->m_expand);

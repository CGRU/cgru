...
TaskItem.prototype.init = function() {
	...
	this.elThumbBtn.onclick = function(e) {
		e.stopPropagation();
		e.currentTarget.m_task.showThumbs();
	};
...
TaskItem.prototype.thumbsReceived = function(i_obj) {
	if (i_obj == null)
		return;
	if (i_obj.task_files == null)
		return;
	if (i_obj.task_files.files == null)
		return;
...
TaskItem.createPanels = function(i_monitor) {
	...
	acts.skip = {
		'handle': 'mh_Oper',
		'label': 'SKIP',
		'tooltip': 'Double click to skip selected task(s).',
		'ondblclick': true
	};
	acts.restart = {
		'handle': 'mh_Oper',
...
TaskItem.prototype.updatePanels = function() {
	...
	if (this.params.str)
		info += 'Starts ' + this.params.str + ', last on ' + this.params.hst + ' at:<br> ' +
			cm_DateTimeStrFromSec(this.params.tst) + '<br>';

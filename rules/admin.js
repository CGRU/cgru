...
function ad_ConstructDigest(i_password, i_user_id)
{
	...
	if (i_user_id == null)
	{
		if ((localStorage.auth_user == null) || (localStorage.auth_user.length == 0) ||
			(localStorage.auth_digest == null) || (localStorage.auth_digest.length == 0))
			return null;
	...
	obj.response = c_MD5(
		localStorage.auth_digest + ':' + SERVER.nonce + ':' + obj.nc + ':' + obj.cnonce + ':' + obj.qop +
		':' + c_MD5('POST:' + obj.uri));
...
function ad_WndDrawUsers()
{
	...
	users.sort(function(a, b) {
		...
		*/
		...
		if ((val_a > val_b) == ad_wnd_sort_dir)
			return -1;
		if ((val_a < val_b) == ad_wnd_sort_dir)
			return 1;
		...
		{
			if ((a.role > b.role) == ad_wnd_sort_dir)
				return -1;
			if ((a.role < b.role) == ad_wnd_sort_dir)
				return 1;
		}

		if ((a.disabled && (!b.disabled)) == ad_wnd_sort_dir)
			return -1;
		if ((b.disabled && (!a.disabled)) == ad_wnd_sort_dir)
			return 1;

		if ((a.title > b.title) == ad_wnd_sort_dir)
			return -1;
		if ((a.title < b.title) == ad_wnd_sort_dir)
			return 1;
...
function ad_WndAddUser(i_el, i_user, i_row)
{
	...
	if (i_row % 2)
		elTr.style.backgroundColor = 'rgba(0,0,0,.1)';
	else
		elTr.style.backgroundColor = 'rgba(255,255,255,.1)';
	...
	var states = '';
	if (i_user.states)
		for (var i = 0; i < i_user.states.length; i++)
...
function ad_UserShowEntries(i_el, i_show_ips)
{
	...
	if (user.ips && user.ips.length && i_show_ips && (i_el.m_ips_shown != true))
	{
		for (var i = 0; i < user.ips.length; i++)
			text += '<br><br>' + user.ips[i].ip + ' : ' + c_DT_StrFromSec(user.ips[i].time) + '<br>' +
				user.ips[i].url;

...
function u_ShowFile( i_element, i_path, i_file, i_walk)
{
	...
	u_MakeFileThumbEl( elFile, path, i_walk, 'file');
...
function u_MakeFileThumbEl( i_el, i_path, i_walk, i_type)
{
...
function u_BodyLoad( i_nockeck)
{
	...
	if( RULES.status && RULES.status.body )
	{
		...
		if( avatar )
		{
			$('body_avatar_c').style.display = 'inline';
		...
		if( RULES.status.body.muser )
		{
			if( RULES.status.body.cuser != RULES.status.body.muser )
			{
				...
				if( avatar )
				{
					$('body_avatar_m').style.display = 'inline';

 ...
 *    -Mmo.  -omM:                                                      Copyright © 2014-17 by The CGRU team
 *    '          '
...
function stcs_Show(i_args)
{
	...
	var tasks_t = [];
	var tasks_a = [];
	for (var ttype in tasks_types)
		tasks_t.push(tasks_types[ttype]);
	for (var ttype in tasks_artists)
		tasks_a.push(tasks_artists[ttype]);
	if (tasks_t.length)
	{
	...
	for (var rtype in reps_types)
		reports_t.push(reps_types[rtype]);
	for (var rtype in reps_artists)
		reports_a.push(reps_artists[rtype]);
...
function stcs_ShowTable(i_args)
{
	...
	var sort = 'duration';
	if (has_price && (i_args.main == 'artists'))
		sort = 'price';

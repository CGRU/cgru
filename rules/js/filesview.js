...
/*
	filesview.js - TODO: description
*/

'use strict';

var fv_views = [];
var fv_thumbnails_tomake = 0;
var fv_thumbnails_tomake_files = [];
...
function FilesView(i_args)
{
	...
	if (i_args.limits === false)
		this.has_limits = false;

	this.has_thumbs = true;
	if (i_args.thumbs === false)
		this.has_thumbs = false;
	...
	this.elClosedOpen.onclick =
		function(e) {
		e.currentTarget.m_view.open();
	}
...
generate thumbnails.';
	...
	if (this.can_refresh)
	{
		...
		el.onclick = function(e) {
			e.currentTarget.m_view.refresh()
		};
		...
		el.m_view = this;
		el.onclick = function(e) {
			e.currentTarget.m_view.makeFolder()
		};
	}
	...
	el.m_view = this;
	el.onclick = function(e) {
		e.currentTarget.m_view.selectAll()
	};
	...
	el.m_view = this;
	el.onclick = function(e) {
		e.currentTarget.m_view.selectInvert()
	};
	...
	el.m_view = this;
	el.onclick = function(e) {
		e.currentTarget.m_view.convert();
	};
	...
	el.m_view = this;
	el.onclick = function(e) {
		e.currentTarget.m_view.extractSound();
	};
	...
	el.m_view = this;
	el.onclick = function(e) {
		e.currentTarget.m_view.archivate();
	};
	...
	if (this.show_walk)
	{
		...
		el.m_path = this.path;
		el.onclick = function(e) {
			fu_Walk({'path': e.currentTarget.m_path});
		};
	...
	if (g_admin)
	{
		...
		el.m_view = this;
		el.onclick = function(e) {
			fu_TmpFio({'fview': e.currentTarget.m_view});
		};
	...
	if (g_admin)
	{
		...
		el.m_view = this;
		el.onclick = function(e) {
			e.currentTarget.m_view.bufferAdd();
		};
		...
		el.m_view = this;
		el.onclick = function(e) {
			e.currentTarget.m_view.bufferPut();
		};
	...
	if (this.has_thumbs)
	{
		...
		el.m_view = this;
		el.onclick = function(e) {
			e.currentTarget.m_view.thumbsBigger()
		};
		...
		el.m_view = this;
		el.onclick = function(e) {
			e.currentTarget.m_view.thumbsSmaller()
		};
		...
		el.m_view = this;
		el.onclick = function(e) {
			e.currentTarget.m_view.thumbsCrop()
		};
		...
		this.elThumbsBtn.onclick = function(e) {
			e.currentTarget.m_view.thumbsMake()
		};
...
FilesView.prototype.limitsAdd = function() {
	...
	elClose.onclick = function(e) {
		e.currentTarget.m_view.close();
	}
};

FilesView.prototype.close =
	function() {
	this.setLocalStorageAttr('closed', 'ON');
	this.processOpenClose();
} FilesView.prototype.open =
		function() {
	this.setLocalStorageAttr('closed', 'OFF');
	this.processOpenClose();
} FilesView.prototype.processOpenClose =
			function() {
...
FilesView.prototype.showCounts = function() {
	...
	if (size_count)
		counts += ' Size:' + c_Bytes2KMG(size_count);
...
FilesView.prototype.showAttrs = function(i_el, i_obj) {
	...
	if (num_files != null)
	{
		...
		i_el.m_el_num_files.onclick = function(e) {
			e.stopPropagation();
		};
	...
	if (video)
	{
		if (video.frame_count)
		{
			...
			i_el.m_el_num_files.title =
				'Frames quantity: ' + video.frame_count + '\nDouble click to store status frames number.';
...
FilesView.prototype.showItem = function(i_obj, i_isFolder) {
	...
	elItem.onclick = function(e) {
		e.currentTarget.m_view.onClick(e);
	};
	var type = 'file';
	if (i_isFolder)
	{
	...
	elItem.m_elName.ondragstart =
		function(e) {
		c_FileDragStart(e, e.currentTarget.m_path);
	}
...
FilesView.prototype.deleteFilesDialog = function(i_path) {
	new cgru_Dialog({
		...
		'name': 'delete',
		'title': 'Delete',
		'info': '<span style="font-size:20px;font-weight:bold;">' + i_path +
...
FilesView.prototype.deleteFiles = function(i_value, i_path) {
	...
	var cmd = 'rules/bin/rm -rf "' + RULES.root + i_path + '"';
	// console.log(cmd);
...
FilesView.prototype.bufferPut = function() {
	...
	for (var i = 0; i < paths.length; i++)
	{
		...
		var cmd = 'rules/bin/move.py "' + src + '" "' + dest + '"';
		cmds.push(cmd);
	}
...
function fv_GetFileIcon(i_name, i_folder)
{
	...
	else if (c_FileIsMovie(i_name))
		icon = 'file_movie.png';
	else if (c_FileIsImage(i_name))
		icon = 'file_image.png';
	else if (c_FileIsArchive(i_name))
		icon = 'file_archive.png';
	return icon;
}

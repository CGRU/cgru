/* ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' *\
 *        .NN.        _____ _____ _____  _    _                 This file is part of CGRU
 *        hMMh       / ____/ ____|  __ \| |  | |       - The Free And Open Source CG Tools Pack.
 *       sMMMMs     | |   | |  __| |__) | |  | |  CGRU is licensed under the terms of LGPLv3, see files
 * <yMMMMMMMMMMMMMMy> |   | | |_ |  _  /| |  | |    COPYING and COPYING.lesser inside of this folder.
 *   `+mMMMMMMMMNo` | |___| |__| | | \ \| |__| |          Project-Homepage: http://cgru.info
 *     :MMMMMMMM:    \_____\_____|_|  \_\\____/        Sourcecode: https://github.com/CGRU/cgru
 *     dMMMdmMMMd     A   F   A   N   A   S   Y
 *    -Mmo.  -omM:                                           Copyright © by The CGRU team
 *    '          '
\* ....................................................................................................... */

/*
	search.js - TODO: description
*/

"use strict";

function s_TabBtnCicked(i_name)
{
	for (let el of document.getElementsByClassName('search_tab_btn'))
		el.classList.remove('selected');
	for (let el of document.getElementsByClassName('search_tab_div'))
		el.style.display = 'none';

	let el = $('search_tab_btn_' + i_name);
	if (el == null)
	{
		c_Error('Invalid tab name: ' + i_name);
		return;
	}
	$('search_tab_div_' + i_name).style.display = 'block';

	el.classList.add('selected');
}

function s_FirstConstruct(i_tab)
{
	$('search_artists_' + i_tab).m_elRoles = [];
	$('search_artists_' + i_tab).m_elArtists = [];
	$('search_artists_' + i_tab).m_show_disabled = false;
	$('search_artists_' + i_tab).m_show_not_artists = false;

	// Construct artists just once, as they are not depending on location:
	let roles = c_GetRolesArtists(g_users);

	for (let r = 0; r < roles.length; r++)
	{
		let elRole = document.createElement('div');
		$('search_artists_' + i_tab).appendChild(elRole);
		$('search_artists_' + i_tab).m_elRoles.push(elRole);
		elRole.classList.add('role');
		elRole.m_elTags = [];
		elRole.m_elArtists = [];

		let elLabel = document.createElement('div');
		elRole.appendChild(elLabel);
		elLabel.m_elRole = elRole;
		elLabel.classList.add('label');
		elLabel.textContent = roles[r].role + ':';
		elLabel.title = 'Click to (un)select all role artists.';
		elLabel.onclick = function(e) {
			let el = e.currentTarget.m_elRole;
			for (let a = 0; a < el.m_elArtists.length; a++)
				if (false == el.m_elArtists[a].m_hidden)
					c_ElToggleSelected(el.m_elArtists[a]);
			if (ASSET && ASSET.filter)
				s_ProcessGUI();
		};

		for (let t = 0; t < roles[r].tags.length; t++)
		{
			let tag = roles[r].tags[t].tag;

			let elTag = document.createElement('div');
			elTag.classList.add('role_tag');
			elRole.appendChild(elTag);
			elTag.m_elArtists = [];
			elRole.m_elTags.push(elTag);

			let elLabel = document.createElement('div');
			elLabel.textContent = c_GetTagTitle(tag) + ':';
			elLabel.classList.add('label');
			elTag.appendChild(elLabel);
			elLabel.title = c_GetTagTip(tag)
				+ '\nClick to (un)select all artists with "' + c_GetTagTitle(tag) + '" tag.';
			elLabel.m_elTag = elTag;
			elLabel.onclick = function(e) {
				let el = e.currentTarget.m_elTag;
				for (let a = 0; a < el.m_elArtists.length; a++)
					if (false == el.m_elArtists[a].m_hidden)
						c_ElToggleSelected(el.m_elArtists[a]);
				if (ASSET && ASSET.filter)
					s_ProcessGUI();
			};

			for (let a = 0; a < roles[r].tags[t].artists.length; a++)
			{
				let artist = roles[r].tags[t].artists[a];

				let el = document.createElement('div');
				el.m_hidden = false;
				//elRole.appendChild(el);
				elTag.appendChild(el);
				elRole.m_elArtists.push(el);
				elTag.m_elArtists.push(el);
				$('search_artists_' + i_tab).m_elArtists.push(el);
				el.style.cssFloat = 'left';
				el.textContent = c_GetUserTitle(artist.id);
				el.m_user = artist
				el.classList.add('tag');
				el.classList.add('artist');

				if (artist.id == g_auth_user.id)
					el.classList.add('me');

				if (artist.disabled)
					el.classList.add('disabled');

				if (c_IsNotAnArtist(artist))
					el.classList.add('notartist');

				let avatar = c_GetAvatar(artist.id);
				if (avatar)
				{
					el.classList.add('with_icon');
					el.style.backgroundImage = 'url(' + avatar + ')';
				}

				el.onclick = function(e) {
					c_ElToggleSelected(e);
					if (ASSET && ASSET.filter)
						s_ProcessGUI();
				};
			}
		}

		s_ShowHideRoles(i_tab);
	}

	let el = $('search_artists_notassigned_' + i_tab);
	$('search_artists_' + i_tab).m_elArtists.push(el);
	el.m_user = {'id':'_null_'};
	el.classList.add('tag');
	el.classList.add('artist');
	el.onclick = function(e) {
		c_ElToggleSelected(e);
		if (ASSET && ASSET.filter)
			s_ProcessGUI();
	};

	for (let name of ['modified','bodymod','finish'])
	{
		let el = $('search_' + name + 'min_' + i_tab);
		if (el == null)
			continue;
		el.onfocus = c_ElCurDateSet;
		el.onfocus = c_ElCurDateSet;
	}
}

function s_CreateFlagsAndTags(i_tab)
{
	let specialElements = [];

	// Flags:
	// Remove old, as a new location can has own flags.
	if ($('search_flags_' + i_tab).m_elFlags)
		for (let i = 0; i < $('search_flags_' + i_tab).m_elFlags.length; i++)
			if ($('search_flags_' + i_tab).m_elFlags[i].m_flag.indexOf('_') != 0)
				$('search_flags_' + i_tab).removeChild($('search_flags_' + i_tab).m_elFlags[i]);
	$('search_flags_' + i_tab).m_elFlags = [];

	// Create new:
	for (let flag in RULES.flags)
	{
		let el = document.createElement('div');
		$('search_flags_' + i_tab).appendChild(el);
		el.style.cssFloat = 'left';
		el.textContent = c_GetFlagTitle(flag);
		el.title = c_GetFlagTip(flag);
		el.m_flag = flag;
		el.classList.add('flag');
		if (RULES.flags[flag].clr)
		{
			let c = RULES.flags[flag].clr;
			el.style.borderColor = 'rgb(' + c[0] + ',' + c[1] + ',' + c[2] + ')';
		}
		el.onclick = function(e) {
			c_ElToggleSelected(e);
			if (ASSET && ASSET.filter)
				s_ProcessGUI();
		};
		$('search_flags_' + i_tab).m_elFlags.push(el);
	}
	{// Search no flags:
		let el = $('search_noflags_' + i_tab);
		$('search_flags_' + i_tab).m_elFlags.push(el);
		el.m_flag = '_null_';
		specialElements.push(el);
	}
	{// Search AND flags:
		let el = $('search_flags_and_' + i_tab);
		$('search_flags_' + i_tab).m_elFlags.push(el);
		el.m_flag = '_AND_';
		specialElements.push(el);
	}

	// Tags:
	if ($('search_tags_' + i_tab).m_elTags)
	// Remove old, as a new location can has own tags.
		for (let i = 0; i < $('search_tags_' + i_tab).m_elTags.length; i++)
			if ($('search_tags_' + i_tab).m_elTags[i].m_tag.indexOf('_') != 0)
				$('search_tags_' + i_tab).removeChild($('search_tags_' + i_tab).m_elTags[i]);
	$('search_tags_' + i_tab).m_elTags = [];
	for (let tag in RULES.tags)
	{
		let el = document.createElement('div');
		$('search_tags_' + i_tab).appendChild(el);
		el.style.cssFloat = 'left';
		el.textContent = c_GetTagTitle(tag);
		el.m_tag = tag;
		el.classList.add('tag');
		el.onclick = function(e) {
			c_ElToggleSelected(e);
			if (ASSET && ASSET.filter)
				s_ProcessGUI();
		};
		$('search_tags_' + i_tab).m_elTags.push(el);
	}
	{// Search no tags
		let el = $('search_notags_' + i_tab);
		$('search_tags_' + i_tab).m_elTags.push(el);
		el.m_tag = '_null_';
		specialElements.push(el);
	}
	{// Search AND tags
		let el = $('search_tags_and_' + i_tab);
		$('search_tags_' + i_tab).m_elTags.push(el);
		el.m_tag = '_AND_';
		specialElements.push(el);
	}

	for (let el of specialElements)
	{
		el.onclick = function(e)
		{
			c_ElToggleSelected(e);
			if (ASSET && ASSET.filter)
				s_ProcessGUI();
		};
	}
}

function s_SearchOnClick()
{
	$('search_artists_btn_status').style.display = 'block';
	$('search_artists_div_status').style.display = 'none';
	$('search_artists_btn_task').style.display = 'block';
	$('search_artists_div_task').style.display = 'none';

	// First time search operations:
	if ($('search').m_constcted != true)
	{
		$('search').m_constcted = true;
		s_FirstConstruct('status');
		s_FirstConstruct('task');
	}


	$('search_result').textContent = '';
	$('search_result_div').style.display = 'none';
	$('search_result_none').style.display = 'none';

	if ($('search_btn').m_opened)
	{
		$('search_btn').m_opened = false;
		$('search_btn').textContent = 'Search';
		$('search').style.display = 'none';
		$('search_btn_process').style.display = 'none';
		g_ClearLocationArgs();
		if (ASSET && window[ASSET.filter])
			s_Found(window[ASSET.filter]());
		$('search').m_path = null;
	}
	else
	{
		$('search_btn').m_opened = true;
		$('search_btn').textContent = 'Close';
		$('search').style.display = 'block';
		$('search_btn_process').style.display = 'block';

		s_CreateFlagsAndTags('status');
		s_CreateFlagsAndTags('task');

		if (ASSET && (ASSET.thumbnails != null))
			$('search_comment_div').style.display = 'none';
		else
			$('search_comment_div').style.display = 'block';
	}

	if (ASSET && ASSET.filter)
	{
		$('search_depth_div').style.display = 'none';
		$('search_nofilter_div').style.display = 'none';
	}
	else
	{
		$('search_depth_div').style.display = 'block';
		$('search_nofilter_div').style.display = 'block';
	}

	if (ASSET && window[ASSET.filter])
		s_Found(window[ASSET.filter]());

	s_TabBtnCicked('task');
}

function s_ShowArtists(i_tab)
{
	$('search_artists_btn_'+i_tab).style.display = 'none';
	$('search_artists_div_'+i_tab).style.display = 'block';
}

function s_ShowHideRoles(i_tab)
{
	const show_disabled    = $('search_artists_' + i_tab).m_show_disabled;
	const show_not_artists = $('search_artists_' + i_tab).m_show_not_artists;
	const elRoles          = $('search_artists_' + i_tab).m_elRoles;

	for (let r = 0; r < elRoles.length; r++)
	{
		let role_hidden = true;

		for (let t= 0; t < elRoles[r].m_elTags.length; t++)
		{
			let tag_hidden = true;

			for (let a = 0; a < elRoles[r].m_elTags[t].m_elArtists.length; a++)
			{

				let el = elRoles[r].m_elTags[t].m_elArtists[a];
				el.m_hidden = false;

				if (el.m_user.disabled && (false == show_disabled))
					el.m_hidden = true;

				if (c_IsNotAnArtist(el.m_user) && (false == show_not_artists))
					el.m_hidden = true;

				if (el.m_hidden)
					el.style.display = 'none';
				else
				{
					el.style.display = 'block';
					tag_hidden = false;
				}
			}

			if (tag_hidden)
				elRoles[r].m_elTags[t].style.display = 'none';
			else
			{
				elRoles[r].m_elTags[t].style.display = 'block';
				role_hidden = false;
			}
		}

		if (role_hidden)
			elRoles[r].style.display = 'none';
		else
			elRoles[r].style.display = 'block';
	}
}

function s_ShowDisabledArtists(i_el, i_tab)
{
	i_el.classList.toggle('pushed');

	if (i_el.classList.contains('pushed'))
		$('search_artists_'+i_tab).m_show_disabled = true;
	else
		$('search_artists_'+i_tab).m_show_disabled = false;

	s_ShowHideRoles(i_tab);
}

function s_ShowNotArtists(i_el, i_tab)
{
	i_el.classList.toggle('pushed');

	if (i_el.classList.contains('pushed'))
		$('search_artists_'+i_tab).m_show_not_artists = true;
	else
		$('search_artists_'+i_tab).m_show_not_artists = false;

	s_ShowHideRoles(i_tab);
}

function s_ProcessGUI()
{
	if ($('search').m_path && ($('search').m_path != g_CurPath()))
	{
		g_GO($('search').m_path);
		g_PathChanged();
	}
	$('search').m_path = g_CurPath();

	let args = {};

	// Depth:
	if (c_Strip($('search_depth').textContent).length)
		args.depth = parseInt(c_Strip($('search_depth').textContent));

	for (let tab of ['status','task'])
	{
		args[tab] = {};

		// Annotation:
		if (c_Strip($('search_annotation_'+tab).textContent).length)
			args[tab].ann = c_Strip($('search_annotation_'+tab).textContent);

		// Artists:
		if ($('search_artists_notassigned_'+tab).m_selected)
		{
			if (args[tab].artists == null)
				args[tab].artists = [];

			for (let i = 0; i < $('search_artists_'+tab).m_elArtists.length; i++)
				c_ElSetSelected($('search_artists_'+tab).m_elArtists[i], false);
			c_ElSetSelected($('search_artists_notassigned_' + tab), true);

			args[tab].artists.push($('search_artists_notassigned_'+ tab).m_user.id);
		}
		else
			for (let i = 0; i < $('search_artists_'+tab).m_elArtists.length; i++)
				if ($('search_artists_'+tab).m_elArtists[i].m_selected)
				{
					if (args[tab].artists == null)
						args[tab].artists = [];
					if (false == $('search_artists_'+tab).m_elArtists[i].m_hidden)
						args[tab].artists.push($('search_artists_'+tab).m_elArtists[i].m_user.id);
				}

		// Flags:
		if ($('search_noflags_'+tab).m_selected)
		{
			if (args[tab].flags == null)
				args[tab].flags = [];
			for (let i = 0; i < $('search_flags_'+tab).m_elFlags.length; i++)
				c_ElSetSelected($('search_flags_'+tab).m_elFlags[i], false);
			c_ElSetSelected($('search_noflags_'+tab), true);

			args[tab].flags.push($('search_noflags_'+tab).m_flag);
		}
		else
			for (let i = 0; i < $('search_flags_'+tab).m_elFlags.length; i++)
				if ($('search_flags_'+tab).m_elFlags[i].m_selected)
				{
					if (args[tab].flags == null)
						args[tab].flags = [];
					args[tab].flags.push($('search_flags_'+tab).m_elFlags[i].m_flag);
				}

		// Tags:
		if ($('search_notags_'+tab).m_selected)
		{
			if (args[tab].tags == null)
				args[tab].tags = [];
			for (let i = 0; i < $('search_tags_'+tab).m_elTags.length; i++)
				c_ElSetSelected($('search_tags_'+tab).m_elTags[i], false);
			c_ElSetSelected($('search_notags_'+tab), true);

			args[tab].tags.push($('search_notags_'+tab).m_tag);
		}
		else
			for (let i = 0; i < $('search_tags_'+tab).m_elTags.length; i++)
				if ($('search_tags_'+tab).m_elTags[i].m_selected)
				{
					if (args[tab].tags == null)
						args[tab].tags = [];
					args[tab].tags.push($('search_tags_'+tab).m_elTags[i].m_tag);
				}

		let parm = ['priority','percent'];
		for (let i = 0; i < parm.length; i++)
		{
			let min = $('search_' + parm[i] + 'min_'+tab);
			let max = $('search_' + parm[i] + 'max_'+tab);
			if ((min == null) || (max == null))
				continue;
			min = c_GetElInteger(min);
			max = c_GetElInteger(max);
			if ((min != null) || (max != null))
				args[tab][parm[i]] = [min, max];
		}

		parm = ['modified','bodymod','finish'];
		for (let i = 0; i < parm.length; i++)
		{
			let min = $('search_' + parm[i] + 'min_'+tab);
			let max = $('search_' + parm[i] + 'max_'+tab);
			if ((min == null) || (max == null))
				continue;
			min = c_GetElTime(min);
			max = c_GetElTime(max);
			if ((min != null) || (max != null))
				args[tab][parm[i]] = [min, max];
		}
	}

	if (c_Strip($('search_body').textContent).length)
		args['status'].body = c_Strip($('search_body').textContent);
	if (c_Strip($('search_comment').textContent).length)
		args['status'].comment = c_Strip($('search_comment').textContent);

	for (let tab of ['status','task'])
		if (Object.keys(args[tab]).length == 0)
			delete args[tab];

	g_SetLocationArgs({"s_Search": args});
}

function s_Search(i_args)
{
	$('search_result').textContent = '';
	$('search_result_div').style.display = 'none';
	$('search_result_none').style.display = 'none';

	if ($('search_btn').m_opened !== true)
		s_SearchOnClick();

	if (i_args == null)
		i_args = {};

	if (i_args.depth)
		$('search_depth').textContent = i_args.depth;
	else
		i_args.depth = 1;

	for (let tab of ['status','task'])
	{
		let args = i_args[tab];
		if (args == null)
		{
			$('search_tab_btn_'+tab).classList.remove('enabled');
			continue;
		}

		$('search_tab_btn_'+tab).classList.add('enabled');

		let anns = null;
		if (args.ann)
			$('search_annotation_'+tab).textContent = args.ann;
		if (args.artists)
			for (let i = 0; i < $('search_artists_'+tab).m_elArtists.length; i++)
				c_ElSetSelected(
					$('search_artists_'+tab).m_elArtists[i],
					args.artists.indexOf($('search_artists_'+tab).m_elArtists[i].m_user.id) != -1);
		if (args.flags)
			for (let i = 0; i < $('search_flags_'+tab).m_elFlags.length; i++)
				c_ElSetSelected(
					$('search_flags_'+tab).m_elFlags[i],
					args.flags.indexOf($('search_flags_'+tab).m_elFlags[i].m_flag) != -1);
		if (args.tags)
			for (let i = 0; i < $('search_tags_'+tab).m_elTags.length; i++)
				c_ElSetSelected(
					$('search_tags_'+tab).m_elTags[i], args.tags.indexOf($('search_tags_'+tab).m_elTags[i].m_tag) != -1)

		let parm = ['priority','percent'];
		for (let i = 0; i < parm.length; i++)
			if (args[parm[i]])
			{
				if ((args[parm[i]][0] != null) && (args[parm[i]][1] != null) &&
					(args[parm[i]][0] > args[parm[i]][1]))
				{
					args[parm[i]][0] += args[parm[i]][1];
					args[parm[i]][1] = args[parm[i]][0] - args[parm[i]][1];
					args[parm[i]][0] = args[parm[i]][0] - args[parm[i]][1];
				}
				$('search_' + parm[i] + 'min_'+tab).textContent = args[parm[i]][0];
				$('search_' + parm[i] + 'max_'+tab).textContent = args[parm[i]][1];
			}

		parm = ['modified', 'bodymod','finish'];
		for (let i = 0; i < parm.length; i++)
			if (args[parm[i]])
			{
				if (args[parm[i]][0])
				{
					let min = new Date(1000 * args[parm[i]][0]);
					min = min.toISOString();
					$('search_' + parm[i] + 'min_'+tab).textContent = min.substr(0, min.indexOf('T'));
				}

				if (args[parm[i]][1])
				{
					let max = new Date(1000 * args[parm[i]][1]);
					max = max.toISOString();
					$('search_' + parm[i] + 'max_'+tab).textContent = max.substr(0, max.indexOf('T'));
				}
			}
	}

	if (i_args.status)
	{
		if (i_args.status.body)
			$('search_body').textContent = i_args.status.body;
		if (i_args.status.comment)
			$('search_comment').textContent = i_args.status.comment;
	}

	if (ASSET && ASSET.filter)
	{
		if (window[ASSET.filter])
			s_Found(window[ASSET.filter](i_args));
		return;
	}

	$('search_path').textContent = g_CurPath();
	$('search_path').href = '#' + g_CurPath();

	let args = {};
	for (let arg in i_args)
	{
		if (arg == 'comment')
		{
			args.comment = i_args['comment'];
			continue;
		}
		if (arg == 'body')
		{
			args.body = i_args['body'];
			continue;
		}
		if (arg == 'depth')
		{
			args.depth = i_args['depth'];
			continue;
		}
		if (args.status == null)
			args.status = {};
		args.status[arg] = i_args[arg];
	}
	args.path = RULES.root + g_CurPath();

	n_Request({"send": {"search": args}, "func": s_ResultReceived});
	$('search').classList.add('waiting');
}

function s_ResultReceived(i_data)
{
	$('search').classList.remove('waiting');

	var res = i_data;

	if (res.error)
	{
		c_Error(res.error);
		return;
	}

	if (res.result == null)
	{
		c_Error('Search returned null result.');
		return;
	}

	$('search_results_count').textContent = res.result.length;

	$('search_result_div').style.display = 'block';
	if (res.result.length == 0)
	{
		$('search_result').style.display = 'none';
		$('search_result_none').style.display = 'block';
		return;
	}
	$('search_result').style.display = 'block';

	res.result.sort();
	for (var i = 0; i < res.result.length; i++)
	{
		var path = res.result[i];
		path = path.replace(RULES.root, '');
		path = path.replace(/\/\//g, '/');
		var el = document.createElement('div');
		$('search_result').appendChild(el);
		var elLink = document.createElement('a');
		el.appendChild(elLink);
		elLink.href = '#' + path;
		elLink.textContent = path;
	}
}

function s_Found(i_args)
{
	if (i_args == null)
		return;
//console.log(JSON.stringify(i_args));
	s_SetNotFoundElements(i_args, 'status');
	s_SetNotFoundElements(i_args, 'task');
}

function s_SetNotFoundElements(i_args, i_tab)
{
//console.log(JSON.stringify(i_args));
	if (i_args[i_tab] == null)
		return;

	const artists = i_args[i_tab].artists;
	const   flags = i_args[i_tab].flags;
	const    tags = i_args[i_tab].tags;

	const elArtists = $('search_artists_' + i_tab).m_elArtists;
	const elFlags   = $('search_flags_'   + i_tab).m_elFlags;
	const elTags    = $('search_tags_'    + i_tab).m_elTags;

	for (const el of elArtists)
	{
		let artist = el.m_user.id;
		// Skip special elements:
		if (artist.charAt(0) == '_') continue;

		if (artists.indexOf(artist) == -1)
			el.classList.add('notfound');
		else
			el.classList.remove('notfound');
	}

	for (const el of elFlags)
	{
		let flag = el.m_flag;
		// Skip special elements:
		if (flag.charAt(0) == '_') continue;

		if (flags.indexOf(flag) == -1)
			el.classList.add('notfound');
		else
			el.classList.remove('notfound');
	}

	for (const el of elTags)
	{
		let tag = el.m_tag;
		// Skip special elements:
		if (tag.charAt(0) == '_') continue;

		if (tags.indexOf(tag) == -1)
			el.classList.add('notfound');
		else
			el.classList.remove('notfound');
	}
}


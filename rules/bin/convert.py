...
Parser.add_option(      '--renumfirst', dest='renumfirst', type  ='int',        default=None,   help='Renumerate first frame')
...
for inarg in Args:
    ...
    for seq in seqs:
        ...
        # Resize sequence:
        if Options.resize != '':
            wh = Options.resize.split('x')
            if len(wh) != 2:
                errorExit('Invalid resize argument: "%s"' % Options.resize)
            cmd += ' -resize %s' % wh[0]
            cmd += ' -gravity center -background black -extent %s +repage' % Options.resize
        ...
        if seq['seq']:
            if Options.renumpad:
                ...
                if Options.afanasy:
                    ...
                    scene = 1
                    if Options.renumfirst is not None:

...
function forum_Init()
{
	...
	n_Request({
		'send': {'getfile': 'rules/assets/forum.html'},
		'func': forum_InitHTML,
		'info': 'get forum.html',
		'parse': false
	});
...
function forum_InitHTML(i_data)
{
	...
	if (g_auth_user == null)
		$('forum_guest_form').style.display = 'block';
...
function forum_NewQuestionOnClick()
{
	...
	if ($('forum_question_id').classList.contains('error'))
		return;
	...
	n_Request({
		'send': {'save': {'file': path, 'data': btoa(question.body), 'type': 'base64'}},
		'func': forum_BodySaved,
		'question': question
	});
}
function forum_BodySaved(i_data, i_args)
{
	...
	if (question.guest)
		status.body.guest = question.guest;
...
function forum_StatusSaved(i_data, i_args)
{
	...
	nw_MakeNews(
		{'title': 'question', 'path': question.folder, 'user': question.user_id, 'guest': question.guest});
	g_GO(question.folder);
}

if (ASSETS.topic)
{
	if (ASSETS.topic.path == g_CurPath())
		forum_Init();
	else

...
/*
	for( var i = 0; i < cgru_WindowsAll.length; i++)
		if( cgru_WindowsAll[i].name == i_name )
			cgru_WindowsAll[i].close();

	var wnd = window.open('lib/' + i_file, name, 'location=no,scrollbars=yes,resizable=yes,menubar=no');
	if( wnd == null ) return;
	cgru_WindowsAll.push( wnd);
	wnd.name = i_name;
	wnd.focus();

window.console.log('cgru_WindowsAll.length = '+cgru_WindowsAll.length);
	wnd.onclose = function(){	
		var index = cgru_WindowsAll.indexOf( this);
window.console.log('closing '+this.name);
...
function cgru_ShowObject( obj)
{
	var title = 'Object';
	if( obj.name ) title = obj.name;
	var wnd = cgru_OpenWindow( title);

	var el = document.createElement('p');
	el.innerHTML = JSON.stringify( obj, null, '&nbsp&nbsp&nbsp&nbsp').replace(/\n/g,'<br/>');
	wnd.content.appendChild( el);
/*	wnd.onload = function(){
...
function cgru_ConfigShow() { cgru_ShowObject( cgru_Config);}
function cgru_LocalStorageShow() { cgru_ShowObject( localStorage);}

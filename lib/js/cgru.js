...
{
	var link = i_path;
	for( var i = 0; i < cgru_Config.rules_projects.length; i++)
		if( link.indexOf(cgru_Config.rules_projects[i]) == 0 )
			link = link.replace( cgru_Config.rules_projects[i], '')
	...
	link = cgru_Config.rules_url + link;
	return link;
}

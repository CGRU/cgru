...
{
	let i_elParent = i_args.parent;
	let i_elType = i_args.type;
	let i_label = i_args.label;
	let i_tooltip = i_args.tooltip;
	let i_open = i_args.open;
	let i_cmd = i_args.cmd;
	let i_cmds = i_args.cmds;
	let i_cwd = i_args.cwd;
	let i_terminal = i_args.terminal;

	if (i_elType == null)
		i_elType = 'div';
	let el = document.createElement(i_elType);
	...
	cgru_CmdExecProcess({"element": el, "open": i_open, "cmd": i_cmd, "cmds": i_cmds, "cwd": i_cwd, "terminal": i_terminal});

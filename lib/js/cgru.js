...
function cgru_OpenFolderCmd( i_path)
{
	var cmd = cgru_Config.open_folder_cmd;
	cmd = cmd.replace(/@PATH@/g, i_path);
	return cmd;
}
...
function cgru_CmdExecCreate( i_args)
{
	...
	cgru_CmdExecProcess({"element":el,"cmd":i_cmd});
...
function cgru_CmdExecCreateOpen( i_args)
{
	...
	var cmd = cgru_OpenFolderCmd( i_path);
	...
	if( typeof RULES !== 'undefined')
	{
		...
		cmd = cgru_OpenFolderCmd( cgru_PM('/'+RULES.root + i_path))
	}

	var tooltip = 'Open location in a file browser.\nDouble click to run.'

	var el = cgru_CmdExecCreate({"parent":i_parent,"type":i_type,"cmd":cmd,"tooltip":tooltip});
...
cgru_Menu.prototype.addItem = function( i_args)
{
	...
	if( i_args.receiver == 'cmdexec')
	{
		el.classList.add('cmdexec');
		if( i_args.handle )
			el.setAttribute('cmdexec', JSON.stringify( i_args.handle));
		return;
	}

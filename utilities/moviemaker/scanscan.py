...
Parser = OptionParser(
	usage="%prog [options] scan_folder result_folder\ntype \"%prog -h\" for help",
	version="%prog 1.  0"
)
...
if len(args) != 2:
	Parser.error('Not enough or too many arguments provided.')
Folder = args[0]
Output = args[1]
if not os.path.isdir(Folder):
	Parser.error('Scan folder "%s" does not exist.' % Folder)
...
REexclude = None
if Options.include != '':
	REinclude = re.compile(Options.include)
if Options.exclude != '':
...
def genMovieName(path):
	...
	if lastdotpos > 0:
		path = path[:lastdotpos - 1]
	for char in BadFileCharacters:
...
if Options.afanasy != 0:
	...
	if Options.maxhosts != -1:
		job.setMaxHosts(Options.maxhosts)
	if Options.pause:
		job.offline()
	job.setNeedOS('')
...
for dirpath, dirnames, filenames in os.walk(Folder):
	...
	if REexclude is not None:
		...
		if len(excludes) != 0:
			continue
	if Options.after > 0:
		if os.path.getmtime(dirpath) < Options.after:
			continue
	...
	for pattern in patterns:
		...
		if Options.aspect_in > 0:
			cmd += ' --aspect_in %f' % Options.aspect_in
		if Options.aspect_auto > 0:
			cmd += ' --aspect_auto %f' % Options.aspect_auto
		if Options.template != '':
...
if not Options.debug and not Options.test and Options.afanasy != 0:
	job.send()

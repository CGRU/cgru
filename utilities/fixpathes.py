...
if not options.capital and not options.spaces and not options.invalid:
	parser.error('What to check, if no options provided?')
...
for dirpath, dirnames, filenames in os.walk(Root, False, None):
	for dirname in dirnames:
		...
		result = rename(os.path.join(dirpath, dirname),
						os.path.join(dirpath, fixedname))
		if not result:
			sys.exit(1)
